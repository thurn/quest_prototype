...
Today, Firebase RTDB stores **materialized game state** (`journeyState`,
...
rooms/{roomId}/
  log/                      ← the ONLY game-state location clients write
    genesis:  { seed, reducerVersion, journeyConfig, createdAt }
...
- **Divergence tripwire**: appenders include `stateHashAfter` (hash of their
  ...
  `fold_divergence` event to `journey-log.jsonl`. Note the failure mode is
...
- **Replay CI**: because the log is a complete, portable record, record real
  ...
  Mirror appended events into `journey-log.jsonl` so existing log-analysis
...
**Phase 4 — migrate journey-level state, subsystem by subsystem**
The ~5,000-line `multiplayer-journey-context.tsx` transaction writers convert
...
legacy `journeyState`; the boundary is the existing battle-entry/exit seam.
