...
The `battleStart.firstBattle` and `battleStart.secondBattle` tables author Mira
guidance for the first two battle previews in the tutorial journey. Each delay
begins when its Battle Start screen loads, and the guidance remains visible
until the player begins the battle.
