# Dream Avatar Selection Carousel Implementation Plan
...
- Identify cards/dream avatars by **UUID/id, never by name**. Never key a map/set on a display name.
 ...
 * dreamsign reveal (`DreamsignInfoCard`), and the DreamAvatar ability reveal.
 *
...
```tsx
// TideCluster — the collapsed tide disclosure used on the DreamAvatar-select
...
import { GLYPHS } from "../primitives/glyph";
import { token } from "../primitives/tokens";
import { dreamAvatarImageSrc } from "../components/hud/DreamAvatarPortrait";
import { extractGlossaryTerms } from "../../data/glossary-terms";

/** One tide shown on a DreamAvatar, already resolved to display copy. It is
 * exactly the cluster's tide view, re-exported under the screen's own name so
 * the view-model builder keeps importing `DreamAvatarTideView` from here. */
export type DreamAvatarTideView = TideClusterTideView;
 ...
 * fills the page and needs no frame, unlike the shared DreamAvatarPortrait. */
function FullBleedPortrait({
  ...
  return (
    <img
      src={dreamAvatarImageSrc(dreamAvatar.imageNumber)}
...
function EssenceReveal({
  ...
  return (
    ...
    >
      ...
      >
        <ResourceChip kind="essence" value={dreamAvatar.startingEssence} />
      </span>
    </InfoCard.PressInfo>
  );
}
...
/** One DreamAvatar page: portrait + title + console. */
function DreamAvatarPage({
  ...
  return (
    ...
    >
      <FullBleedPortrait dreamAvatar={dreamAvatar} />
      ...
      >
        ...
        >
          {dreamAvatar.name}, {dreamAvatar.title}
        </h1>
      </div>
      ...
      >
        <GroupPanel>
          <AbilityReveal text={dreamAvatar.renderedText} stageRef={stageRef} />
          ...
          >
            {dreamAvatar.tides.length > 0 ? (
              <span data-dream-avatar-tides={dreamAvatar.id}>
                <TideCluster tides={dreamAvatar.tides} stageRef={stageRef} />
              </span>
            ) : (
              <span />
            )}
            <EssenceReveal dreamAvatar={dreamAvatar} stageRef={stageRef} />
          </div>
...
- **Spec coverage:** carousel screen (Task 3), `TideCluster` full container-transform (Task 2), screen-local full-bleed portrait — `DreamAvatarPortrait` untouched (Task 3, `FullBleedPortrait`), `CardTermDefinitions` evolved in place with Dreamsign/GameCard inheriting (Task 1), ability reveal via `InfoCard.PressInfo` guarded on term presence (Task 3, `AbilityReveal`), hide-tides-when-empty (Task 3), registration unchanged/verified (Task 3 interfaces + Task 4), QA (Task 4). All spec sections map to a task.
- **Placeholder scan:** none — every code step carries full source.
- **Type consistency:** `DreamAvatarOfferView.tides` is `DreamAvatarTideView[]`, which extends `TideClusterTideView` (`{id,label,description,tide}`), so it feeds `TideCluster` directly. `tideVisual(tide)` (Task 2) is consumed only inside `TideCluster`. `CardTermDefinitions({text,testId,side})` keeps its signature (Task 1) and is consumed by `AbilityReveal` (Task 3) with just `text`.
