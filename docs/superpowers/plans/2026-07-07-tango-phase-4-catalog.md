# Tango Revisions Phase 4: Deletions and Catalog Honesty — Implementation Plan
...
Phase 4 is the final phase of the Tango system-revisions program. It makes the
...
The Tango doc catalog is **generated**, not hand-written. This is the single
most important fact for this phase:

- **Component reference pages** (`.llms/skills/tango/components/<id>.md`) are
  produced by `scripts/generate-tango-docs.mjs` from three sources:
  `src/tango/docs/registry.ts` (the `TANGO_COMPONENTS` array + its imports, in
  catalog order), `src/tango/docs/demos/<id>.tsx` (the authored prose: `id`,
  `title`, `blurb`, optional `callout`, `group`, `docName`, `usage[]`), and
  `src/tango/metadata/tango-metadata.json` (the docgen `PropMeta[]`, keyed by
...
- **Metadata** (`tango-metadata.json`) is produced by
  `scripts/generate-tango-metadata.mjs`, which globs **all** `.tsx` under
  `src/tango/components/` and `src/tango/primitives/` (skipping `__*__`) and runs
...
- **Full-screen mockups** are a separate wiring: `src/tango/docs/mockups/<id>.tsx`
  registered in `src/tango/docs/mockups/registry.ts` (`MOCKUPS` map + import).
...
- **Regeneration:** `npm run regenerate-assets` runs metadata + docs (among
  others). Two drift tests — `scripts/generate-tango-metadata.test.mjs` /
  `scripts/tango-generated-docs-drift.test.mjs` — fail the build if the committed
...
The Tango isolation boundary (`eslint-rules/no-external-ui-imports.js`) is
fail-closed: `src/tango/**` may import only from `src/tango/`, `src/data/`,
...
`src/screens/tango_adapters/atlas-view-model.ts`; the atlas demo (under
`src/tango/docs/`) therefore cannot import them directly — Phase 4 relocates the
node-size constants into the in-Tango `atlas-display.ts` so both the adapter and
the demo read one source.

## Tech Stack

TypeScript / React (Vite), ESLint flat config with custom `tango/*` rules,
Vitest, react-docgen-typescript. Node ESM scripts under `scripts/`. Docs are
Markdown generated into `.llms/skills/tango/`. Browser QA via `agent-browser`
...
- **Regenerate + commit drift:** any task touching a Tango component, demo,
...
- **Browser QA for visual demo/screen changes** via the doc catalog route
  (`/tango#/<id>` for a component page, `/tango#/<id>/mockup` for a full-screen
...
**Files**
- `src/tango/components/controls/StatTile.tsx` (delete)
- `src/tango/docs/demos/stat-tile.tsx` (delete)
- `src/tango/docs/mockups/stat-tile.tsx` (delete)
- `src/tango/docs/registry.ts` (remove the `statTileDemo` import + its `TANGO_COMPONENTS` entry)
- `src/tango/docs/mockups/registry.ts` (remove the `StatTileMockup` import + its `MOCKUPS` entry)
- `src/tango/primitives/color.ts` (comment referencing StatTile)
- `src/tango/components/controls/SegmentedControl.tsx` (comment referencing StatTile)
- `scripts/tango-ghost-components.test.mjs` (remove `"StatTile"` from the exported `BASELINE`)
- Generated (auto): `src/tango/metadata/tango-metadata.json` (StatTile key drops), `.llms/skills/tango/components/stat-tile.md` (swept)
...
LOCAL function named `StatTile` that is unrelated to the Tango component; leave it
entirely alone. The only `StatTile` being deleted is
`src/tango/components/controls/StatTile.tsx` and its docs wiring.
...
- [ ] In `scripts/tango-ghost-components.test.mjs`, remove `"StatTile"` from the
...
- [ ] Grep guard — expect **empty**: `grep -rn "StatTile" src/tango/ .llms/skills/tango/`.
...
- [ ] Browser QA (catalog): dev server on 5174, isolated session; open
      `http://localhost:5174/tango#/` — the index no longer lists "Stat Tile", and
      `#/stat-tile` shows the graceful not-found note; no console errors.
- [ ] Commit: `refactor(tango): delete unused StatTile component and its catalog entry` + push.
...
**Files**
- `src/screens/tango_adapters/quest-start-view-model.ts:11` (redirect the `Tide` type import)
- `src/tango/components/hud/TidePill.tsx` (delete)
- `src/tango/docs/demos/tide-pill.tsx` (delete)
- `src/tango/docs/mockups/tide-pill.tsx` (delete)
- `src/tango/docs/registry.ts` (remove the `tidePillDemo` import + entry)
- `src/tango/docs/mockups/registry.ts` (remove the `TidePillMockup` import + entry)
- `src/tango/docs/demos/pressable.tsx` (prose string listing "TidePill" as an example)
- `src/tango/components/controls/SegmentedControl.tsx` (comment — if it still names TidePill after Task 1)
- `src/tango/components/overlay/InfoCard.tsx` (comment "TidePill, a HUD chip")
- `scripts/tango-ghost-components.test.mjs` (remove `"TidePill"` → BASELINE now `[]`)
- Generated (auto): `tango-metadata.json` (`TidePill` and `Tide` keys drop), `tide-pill.md` (swept)

**Interfaces / key finding:** the `Tide` type **already lives in
`src/tango/components/hud/tide-spec.ts`** (`export type Tide = …`). `TidePill.tsx`
...
- `src/screens/tango_adapters/quest-start-view-model.ts:11` — production. **Redirect this one** to `../../tango/components/hud/tide-spec`.
- `src/tango/docs/demos/tide-pill.tsx`, `src/tango/docs/mockups/tide-pill.tsx` — both deleted here.
...
- [ ] `scripts/tango-ghost-components.test.mjs`: remove `"TidePill"` from
...
- [ ] Grep guard — expect **empty**: `grep -rn "TidePill" src/ .llms/skills/tango/`.
...
- [ ] Commit: `refactor(tango): delete TidePill, repoint Tide type onto tide-spec, empty ghost baseline` + push.
...
**Files**
- `src/tango/components/dreamscape/SiteNode.tsx` (post-Phase-3 path; the visited branch of `siteRevealNote`, the visited `opacity`, the battle-pulse/badge visited guards, and the `data-site-visited` attribute)
- `src/tango/components/dreamscape/site-node.css` (the `.ds-node-badge.visited` rule)
- `src/tango/docs/demos/site-node.tsx` (delete the `s-visited` fixture; fix the `s-locked` blurb)
- `src/tango/docs/mockups/site-node.tsx` (delete the `s-draft` `isVisited: true` fixture)

> Phase 3 moved `SiteNode` / `site-node.css` / `dreamscape-scatter.ts` to
> `src/tango/components/dreamscape/`. Write these edits against that path. If a
...
- [ ] Grep guard — expect **empty**:
      `grep -rn "isVisited" src/tango/components/dreamscape/SiteNode.tsx` and
      `grep -rn "\.visited\b\|Already visited" src/tango/components/dreamscape/`.
- [ ] Browser QA: `/tango#/site-node` and `/tango#/site-node/mockup` at desktop +
...
- [ ] Commit: `refactor(tango): remove SiteNode's unreachable visited state and repair its demo fixtures` + push.
...
**Files**
- `src/tango/components/atlas/AtlasNodeReveal.tsx` (the `AtlasNodeCard.eyebrow` field + the `meta={card.eyebrow ?? undefined}` wiring in `AtlasMainCard`)
- `src/screens/tango_adapters/atlas-view-model.ts` (the three `eyebrow: null,` lines in `buildNodeCard` — boss / unrevealed / revealed branches)
- `src/tango/components/atlas/AtlasNode.tsx` (the `node-forgone` fade coupling)
- `src/tango/components/atlas/atlas.css` (the shared `.node-forgone, .node-unreachable` fade rule + its paired `.node-glow` hide rule)
...
- [ ] Grep guard — expect **empty**:
      `grep -rn "eyebrow" src/tango/components/atlas/ src/screens/tango_adapters/atlas-view-model.ts`
      and `grep -rn "node-forgone" src/tango/components/atlas/atlas.css`.
- [ ] Browser QA: `/tango#/atlas-node/mockup` (or the atlas mockup after Task 7) at
...
- [ ] Commit: `refactor(tango): drop AtlasNode's dead eyebrow/meta wiring; fade unreachable nodes off isReachable` + push.
...
**Files**
- `src/tango/components/atlas/atlas-display.ts` (add exported node-size constants)
- `src/screens/tango_adapters/atlas-view-model.ts` (`ATLAS_LAYOUT_DESKTOP` / `ATLAS_LAYOUT_MOBILE` reference the new constants)
- `src/tango/docs/demos/atlas-node.tsx` (rewrite fixtures + mount `AtlasNodeReveal`)
...
The `ATLAS_LAYOUT_*` profiles live in `src/screens/tango_adapters/` and **cannot be
imported across the Tango boundary** (`src/tango` may not import `src/screens`), so
the size numbers move into the in-Tango `atlas-display.ts` as the single source
both sides read.

**Contract — the constants (names + values are load-bearing):**
```ts
// src/tango/components/atlas/atlas-display.ts
...
- [ ] Browser QA: `/tango#/atlas-node` at desktop + mobile — nodes render at
...
- [ ] Commit: `refactor(tango): atlas demo mounts AtlasNodeReveal and uses production node sizes/forgone shape` + push.
...
**Files**
- `src/tango/docs/demos/tide-disc.tsx` (add the canonical `InfoCard.PressInfo` usage; document the `tide-spec` palette as TideDisc's source)
...
- [ ] The demo's `blurb` (or an added `callout`, plain string literal) names
      ...
      glyphs live in `src/tango/components/hud/tide-spec.ts` (`TIDES` / `tideVisual`
...
**Verify**
- [ ] `npm run regenerate-assets`; `npm run lint && npm run typecheck && npm test` — green.
- [ ] Browser QA: `/tango#/tide-disc` — the palette prose renders; press/hover a
      disc and the tide reveal appears; no console errors.
- [ ] Commit: `docs(tango): show canonical TideDisc-in-reveal usage and document the tide-spec palette` + push.
...
**Files**
- `src/tango/docs/mockups/atlas-map.tsx` (`AtlasMapMockup` — rebuild to mount the real `AtlasMap`)
...
`AtlasMapEdge[]` fixtures in-Tango (like the atlas-node demo builds `AtlasNodeView`s),
...
**Verify**
- [ ] `npm run regenerate-assets`; `npm run lint && npm run typecheck && npm test` — green.
- [ ] Browser QA: `/tango#/atlas-node/mockup` (and `#/atlas-map/mockup` after Task
...
- [ ] Commit: `refactor(tango): rebuild the atlas mockup on the real AtlasMap surface` + push.
...
`TangoComponent`, append it to `TANGO_COMPONENTS` in `registry.ts` in its stated
...
**Files:** `src/tango/docs/demos/dreamcaller-portrait.tsx` (new); `registry.ts` (import + append, Components group).
...
**Verify:** regenerate; lint/typecheck/test green; Browser QA `/tango#/dreamcaller-portrait`
(three framings + monogram fallback if pointed at a missing `imageNumber`;
adoption count > 0; no console errors). Commit: `docs(tango): document DreamcallerPortrait` + push.

---

### Task 9 — Document HoverPopover (13 consumers) with the HoverPopover-vs-InfoCard decision rule

**Files:** `src/tango/docs/demos/hover-popover.tsx` (new); `registry.ts` (import + append).
...
**Verify:** regenerate; green; Browser QA `/tango#/hover-popover` (hover shows the
tooltip, stays on-screen near a viewport edge; no console errors). Commit:
`docs(tango): document HoverPopover with the InfoCard-vs-HoverPopover decision rule` + push.

---

### Task 10 — Document HoverZoomCard (8 consumers)

**Files:** `src/tango/docs/demos/hover-zoom-card.tsx` (new); `registry.ts` (import + append).
...
**Verify:** regenerate; green; Browser QA `/tango#/hover-zoom-card` (hover grows the
card, neighbours pop in turn, no layout shift; no console errors). Commit:
`docs(tango): document HoverZoomCard` + push.

---

### Task 11 — Document GlowIcon (8 consumers)

**Files:** `src/tango/docs/demos/glow-icon.tsx` (new); `registry.ts` (import + append).
...
**Verify:** regenerate; green; Browser QA `/tango#/glow-icon` (marks render in the
resource hues with shadow/glow; no console errors). Commit: `docs(tango): document GlowIcon` + push.

---

### Task 12 — Document rich-text (8 consumers)

**Files:** `src/tango/docs/demos/rich-text.tsx` (new); `registry.ts` (import + append).
...
**Verify:** regenerate; green; Browser QA `/tango#/rich-text` (the four kinds render
— keyword emphasis + inline glyphs on `rules`, muted italic on `note`; no console
errors). Commit: `docs(tango): document the rich-text model (RichTextView)` + push.

---

### Task 13 — Document GlossaryDefinitionCard (6) and CardTermDefinitions (5)

**Files:** `src/tango/docs/demos/glossary-definition-card.tsx` (new);
`src/tango/docs/demos/card-term-definitions.tsx` (new); `registry.ts` (import +
...
- `GlossaryDefinitionCard({ entry: GlossaryEntry })` — the one keyword-definition
  ...
  headline, rules text as a `richText.rules` body; establishes its own `.tango`
  token scope so it renders inside popovers portaled outside the Tango subtree;
...
- glossary-definition-card: live `Component` renders one `GlossaryDefinitionCard`
  ...
  its own `.tango` scope so it works inside a portaled popover. `usage[]`: one entry
...
**Verify:** regenerate; green; Browser QA `/tango#/glossary-definition-card` and
`/tango#/card-term-definitions` (the tile and the stack render; no console errors).
Commit: `docs(tango): document GlossaryDefinitionCard and CardTermDefinitions` + push.

---

### Task 14 — Document PipBadge (3 consumers)

**Files:** `src/tango/docs/demos/pip-badge.tsx` (new); `registry.ts` (import + append).
...
**Verify:** regenerate; green; Browser QA `/tango#/pip-badge` (pips at both sizes;
tooltip on long hover; no console errors). Commit: `docs(tango): document PipBadge` + push.

---

### Task 15 — Document AtlasMap (the documentable atlas surface)

**Files:** `src/tango/docs/demos/atlas-map.tsx` (new); `registry.ts` (import +
append); `src/tango/docs/mockups/registry.ts` (add `"atlas-map": AtlasMapMockup`).
...
**Verify:** regenerate; green; Browser QA `/tango#/atlas-map` and `/tango#/atlas-map/mockup`
at desktop + mobile (the map scales to fit, nodes reveal live; no console errors).
Commit: `docs(tango): document AtlasMap` + push.
...
**Files**
- `src/tango/docs/demos/info-card.tsx` (add a `callout` + a usage entry — the generated `info-card.md` picks these up)
...
- [ ] Commit: `docs(tango): document InfoCard's mobile scale and above-only placement` + push.
...
**Files**
- `.llms/skills/tango-migrate/SKILL.md` (add the floating pick-counter idiom + draft-screen note in the idioms section)
...
- `DeviceFrameDemo` (`src/tango/screens/devtools/DeviceFrameDemo.tsx`) is a
...
**End state**
- [ ] `.llms/skills/tango-migrate/SKILL.md` gains a short "Floating pick counter"
...
**Files**
- `src/tango/screens/MobileDeckViewer.tsx` (add the card count to the `TopBand`)
- `src/tango/screens/desktop-deck-filter.ts` / `src/tango/screens/mobile-deck-filter.ts` (a cross-referencing divergence comment, if not already present)
...
- [ ] Commit: `feat(tango): show the card count in the mobile deck viewer header; document the deck-filter divergence` + push.
...
**Files**
- `src/tango/internal/glass-surface.ts` (post-Phase-0 path; the doctrine comment's consumer list)
- `src/tango/components/overlay/InfoCard.tsx` (`SITE_DISC` comment — verify only)
...
0–3 changed consumers again (recipe moved to `src/tango/internal/`; `IconButton` /
`GlassButton` / `GlassDialog` now consume it; `GlassDialog` owns the shared backdrop
the deck viewers render through; StartingDeckModal migrated into the Tango tier).
The comment must state the **final, grepped truth**.

> If `glass-surface.ts` is still under `src/tango/components/controls/` at execution
> time, Phase 0 has not landed — resolve the phase ordering first.

**End state**
- [ ] Re-grep the real consumers: `grep -rn "glassSurfaceStyle(\|glassTrack" src/tango/`.
...
**Verify**
- [ ] `npm run lint && npm run typecheck && npm test` — green.
- [ ] Grep guard: `grep -rn "no longer\|used to\|unlike before" src/tango/internal/glass-surface.ts` — expect **empty**.
- [ ] Commit: `docs(tango): refresh the glass-surface doctrine consumer list to the current tree` + push.
...
- **"Tango design-system audit follow-ups (2026-07-06)"** — the entire prioritized
  action-item list is executed by this program → **delete the section**.
- **"Tango readiness gaps" item 2** (popup-rule conflict: bless HoverPopover or
  migrate) — resolved by Task 9's documented decision rule → **delete item 2**.
- **"Tango readiness gaps" item 1** (~9 undocumented components) — this phase
...
- [ ] Read `pre-existing-issues.txt` at execution time and reconcile with the above
      (Phases 0–3 may have added/removed entries). Delete the "Tango design-system
...
- [ ] Final program-wide grep sweep — deleted components leave **zero** references
      ...
      ```
      grep -rn "StatTile"  src/tango/ src/screens/ .llms/skills/tango/
      grep -rn "TidePill"  src/ .llms/skills/tango/
      ```
      Confirm the only surviving `StatTile` hit repo-wide is the unrelated
      `OpponentDebugApp.tsx` local function.
- [ ] `npm run lint && npm run typecheck && npm test` — green.
- [ ] Commit: `docs: resolve the Tango audit-follow-up and readiness-gap items closed by the revisions program` + push.
...
- [ ] `npm run lint && npm run typecheck && npm test` — all green, including:
      - `scripts/tango-ghost-components.test.mjs` with the **empty** `BASELINE`
        (every registry entry has a real consumer; no ghosts).
      - `scripts/tango-orphan-tokens.test.mjs` (no orphaned tokens introduced).
      - `scripts/generate-tango-metadata.test.mjs` / `tango-generated-docs-drift.test.mjs`
...
- [ ] Commit (if drift only): `chore(tango): regenerate assets for Phase 4 boundary` + push.
