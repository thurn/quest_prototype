...
- `cumulus/scripts/verify-tango-mvp.sh` is the authoritative completion gate. It must run without an open Unity Editor or human input and return nonzero for any failed stage, malformed/missing result, compiler error, shader error, unexpected exception, or failed acceptance metric.
...
- `cumulus/Assets/TangoMvp/Runtime/Diagnostics/TangoGlassDiagnostics.cs` — per-camera, per-frame render facts exposed to tests and development tooling.
...
- `cumulus/Assets/TangoMvp/Runtime/Demo/TangoVerificationMarkers.cs` — named world-space regions used to derive GPU-test pixel bounds.
...
- `cumulus/Assets/TangoMvp/Editor/TangoMvpBatchVerification.cs` — shader inspection, deterministic player build, and batch exit-code ownership.
...
- `cumulus/Assets/TangoMvp/Tests/EditMode/TangoImageMetricsTests.cs` — boundary and negative-control tests for every GPU acceptance metric.
- `cumulus/Assets/TangoMvp/Tests/Support/TangoMvp.TestSupport.asmdef` — test-only shared image-analysis assembly.
- `cumulus/Assets/TangoMvp/Tests/Support/TangoImageMetrics.cs` — deterministic region readback, luminance, edge-energy, contrast, and image-difference functions.
- `cumulus/Assets/TangoMvp/Tests/Support/TangoGpuAcceptance.cs` — named relational thresholds and JSON result model.
...
- `cumulus/Assets/TangoMvp/Tests/PlayMode/TangoGlassGpuTests.cs` — deterministic render readback and relational material assertions.

### Agent verification infrastructure

- `cumulus/scripts/verify-tango-mvp.sh` — one-command autonomous verification orchestrator.
...
- `cumulus/.gitignore` — ignores `/Artifacts/TangoMvpVerification/` and `/Builds/TangoMvpVerification/`.
- `cumulus/Artifacts/TangoMvpVerification/summary.json` — ignored aggregate verdict and stage evidence.
- `cumulus/Artifacts/TangoMvpVerification/render-metrics.json` — ignored GPU measurements and threshold verdicts.
- `cumulus/Artifacts/TangoMvpVerification/*.png` — ignored deterministic captures for optional agent or human inspection.
...
- Produces: ignored `cumulus/Artifacts/TangoMvpVerification/stages/<stage-name>/` directories for logs and result XML.
...
- [ ] **Step 3: Run the targeted EditMode tests and confirm the intended failure**
  ...
  ```bash
  source cumulus/scripts/lib/unity-run.sh
  run_unity_stage task1-mesh-tests nographics -runTests -testPlatform EditMode -testFilter TangoMvp.Tests.TangoRoundedPanelMeshTests -testResults "$PWD/cumulus/Artifacts/TangoMvpVerification/stages/task1-mesh-tests/results.xml"
  ```
...
- [ ] **Step 5: Run the targeted tests and inspect the generated mesh in an editor test**
  ...
  Repeat the Task 1 harness command. Expected: exit code `0`, all `TangoRoundedPanelMeshTests` pass, and the stage's validated NUnit XML reports zero failures.
...
- Create: `cumulus/Assets/TangoMvp/Runtime/Diagnostics/TangoGlassDiagnostics.cs`
...
- Produces: immutable `TangoGlassFrameFacts` values from `TangoGlassDiagnostics.TryGetFrameFacts(int cameraInstanceId, int frameCount, out TangoGlassFrameFacts facts)` with input/output dimensions, graph-record count, horizontal-pass count, vertical-pass count, and availability.
...
- [ ] **Step 6: Run the targeted tests and perform a Unity compile smoke test**
  ...
  ```bash
  source cumulus/scripts/lib/unity-run.sh
  run_unity_stage task2-rendering-tests nographics -runTests -testPlatform EditMode -testFilter TangoMvp.Tests.TangoGlassRenderingTests -testResults "$PWD/cumulus/Artifacts/TangoMvpVerification/stages/task2-rendering-tests/results.xml"
...
- [ ] **Step 6: Rebuild assets and run the tests**
  ...
  ```bash
  source cumulus/scripts/lib/unity-run.sh
  run_unity_stage task3-build-materials nographics -quit -executeMethod TangoMvp.Editor.TangoGlassLabBuilder.RebuildMaterials
  find cumulus/Assets/TangoMvp/Materials -type f -print0 | sort -z | xargs -0 shasum > /tmp/cumulus-tango-task3-before.sha
  run_unity_stage task3-rebuild-materials nographics -quit -executeMethod TangoMvp.Editor.TangoGlassLabBuilder.RebuildMaterials
  ...
  run_unity_stage task3-rendering-tests nographics -runTests -testPlatform EditMode -testFilter TangoMvp.Tests.TangoGlassRenderingTests -testResults "$PWD/cumulus/Artifacts/TangoMvpVerification/stages/task3-rendering-tests/results.xml"
  ```
...
- [ ] **Step 6: Run targeted EditMode and PlayMode tests**
  ...
  ```bash
  source cumulus/scripts/lib/unity-run.sh
  run_unity_stage task4-edit-tests nographics -runTests -testPlatform EditMode -testFilter TangoMvp.Tests.TangoCubicBezierTests -testResults "$PWD/cumulus/Artifacts/TangoMvpVerification/stages/task4-edit-tests/results.xml"
  run_unity_stage task4-play-tests graphics -runTests -testPlatform PlayMode -testFilter TangoMvp.Tests.PlayMode -testResults "$PWD/cumulus/Artifacts/TangoMvpVerification/stages/task4-play-tests/results.xml"
  ```
...
- Create: `cumulus/Assets/TangoMvp/Runtime/Demo/TangoVerificationMarkers.cs`
...
- Produces: deterministic `SetPhase(float normalizedPhase)` methods on the spinner and light plus named `Rect GetViewportRegion(TangoVerificationRegion region, Camera camera)` probes for `LiveGlassA`, `LiveGlassB`, `UncoveredPattern`, `OnGlassButton`, `SolidBevel`, `FrameShadowReceiver`, and `PrimaryLabel`.

- [ ] **Step 1: Write failing lab-asset tests**

  Run `Rebuild()` twice and assert stable GUIDs for the mesh, four materials, library, prefab, and scene. Assert `PC_Renderer.asset` contains exactly one active `TangoGlassRendererFeature` with the shared blur material. Open the scene additively and assert exact object names and counts, two scene-glass renderers share the same material object, the nested button uses the on-glass material, the solid frame uses an opaque material with `ShadowCastingMode.On`, both labels are `TextMesh` components outside any Canvas, every pressable has one stable root collider, all seven verification regions project inside the camera viewport without overlap, and `EditorBuildSettings` enables only `Assets/Scenes/TangoGlassLab.unity` for the MVP.
...
- [ ] **Step 3: Implement deterministic background motion and lighting**
  ...
  `TangoSpinner` rotates a striped opaque object at a fixed angular velocity behind both panes. `TangoLightOrbit` changes the directional light orientation on a fixed loop so bevel highlights visibly travel. Both expose deterministic normalized phase setters used by tests; ordinary Play Mode advances the same phase from elapsed time. Neither component uses randomness or allocates per frame.

- [ ] **Step 4: Complete the idempotent scene builder**

  Build a fixed camera, bright/dark/gold background geometry, ground receiver, moving striped object, directional light, source/destination anchors, main panel prefab instance, and second independent glass pane. The main panel hierarchy contains the shallow glass face, opaque bevel/frame, warm-white mesh label, raised on-glass button visual, stable button collider, pressable, and travel component. Wire button activation to `ToggleDestination()` in serialized scene data. Add one `TangoPointerInteractor` to the camera, one `TangoVerificationMarkers` object whose named regions are fully visible at `512 × 288`, including a clean ground patch receiving the frame shadow, and log Unity/URP versions plus `live-shared-blur` mode at scene start.
...
- [ ] **Step 6: Generate and validate committed assets**
  ...
  ```bash
  source cumulus/scripts/lib/unity-run.sh
  run_unity_stage task5-build-scene nographics -quit -executeMethod TangoMvp.Editor.TangoGlassLabBuilder.Rebuild
  ...
  run_unity_stage task5-rebuild-scene nographics -quit -executeMethod TangoMvp.Editor.TangoGlassLabBuilder.Rebuild
  ...
  run_unity_stage task5-asset-tests nographics -runTests -testPlatform EditMode -testFilter TangoMvp.Tests.TangoGlassLabAssetTests -testResults "$PWD/cumulus/Artifacts/TangoMvpVerification/stages/task5-asset-tests/results.xml"
  ```
...
- Create: `cumulus/scripts/verify-tango-mvp.sh`
- Modify: `cumulus/.gitignore`
- Create: `cumulus/Assets/TangoMvp/Editor/TangoMvpBatchVerification.cs`
- Create: `cumulus/Assets/TangoMvp/Tests/Support/TangoMvp.TestSupport.asmdef`
- Create: `cumulus/Assets/TangoMvp/Tests/Support/TangoImageMetrics.cs`
- Create: `cumulus/Assets/TangoMvp/Tests/Support/TangoGpuAcceptance.cs`
- Create: `cumulus/Assets/TangoMvp/Tests/EditMode/TangoImageMetricsTests.cs`
- Create: `cumulus/Assets/TangoMvp/Tests/PlayMode/TangoGlassGpuTests.cs`
- Modify: `cumulus/Assets/TangoMvp/Tests/EditMode/TangoMvp.EditModeTests.asmdef`
- Modify: `cumulus/Assets/TangoMvp/Tests/PlayMode/TangoMvp.PlayModeTests.asmdef`

**Interfaces:**
- Produces: `cumulus/scripts/verify-tango-mvp.sh`, invoked from the repository root with no arguments and returning `0` only when every stage passes.
...
- Produces: `TangoMvp.Editor.TangoMvpBatchVerification.InspectShadersAndBuildPlayer()`; it exits Unity nonzero on any shader error or unsuccessful build.
- Produces: `TangoImageMetrics` functions for mean absolute RGB difference, luminance edge energy, percentile contrast, and region PNG encoding.
- Produces: `TangoGpuAcceptanceResult` records containing metric name, measured value, comparison, threshold, pass/fail, frame phases, graphics API, and device name.
- Produces ignored evidence under `cumulus/Artifacts/TangoMvpVerification/` and ignored player output under `cumulus/Builds/TangoMvpVerification/`.
...
- [ ] **Step 2: Run the image-metric tests and confirm the intended failure**
  ...
  ```bash
  source cumulus/scripts/lib/unity-run.sh
  run_unity_stage task6-metric-tests-red nographics -runTests -testPlatform EditMode -testFilter TangoMvp.Tests.TangoImageMetricsTests -testResults "$PWD/cumulus/Artifacts/TangoMvpVerification/stages/task6-metric-tests-red/results.xml"
  ```
...
- [ ] **Step 4: Write GPU-backed PlayMode acceptance tests**
  ...
  Load `TangoGlassLab.unity`, target its camera at a `512 × 288` `ARGB32` render texture, set spinner/light phases explicitly, render, and read pixels back from the GPU. Use `TangoVerificationMarkers` to derive regions rather than hard-coded pixel coordinates. Save phase captures and `render-metrics.json` even on failure. Assert:
  ...
  - `TangoGlassDiagnostics` reports one graph record, one horizontal pass, and one vertical pass for a rendered camera frame with both panes enabled;
...
- [ ] **Step 5: Implement shader inspection and standalone build verification**
  ...
  `InspectShadersAndBuildPlayer()` loads `TangoMvp/SceneGlass`, `TangoMvp/OnGlass`, and `Hidden/TangoMvp/SeparableBlur`, calls `ShaderUtil.GetShaderMessages`, writes every message to `shader-report.json`, and exits with code `21` if any message has error severity. It then builds the enabled scene for `BuildTarget.StandaloneOSX` into `cumulus/Builds/TangoMvpVerification/CumulusTangoMvp.app`, writes `BuildReport.summary` fields to `build-report.json`, and exits with code `22` unless the result is `Succeeded`. This player build is the platform shader/serialization/stripping compilation check; editor import alone is not sufficient.

- [ ] **Step 6: Implement the one-command orchestrator**

  `verify-tango-mvp.sh` removes and recreates the ignored artifact directory, installs a trap that always writes the failing stage to `summary.json`, and runs these stages in order:

  1. shell harness self-tests;
  2. clean Unity import/compilation with `-batchmode -nographics -quit`;
  3. `TangoGlassLabBuilder.Rebuild` twice with a before/after hash-manifest equality check;
  ...
  6. `TangoMvpBatchVerification.InspectShadersAndBuildPlayer` with shader/build JSON validation;
...
- [ ] **Step 7: Run the complete gate twice from a clean process state**
  ...
  cp cumulus/Artifacts/TangoMvpVerification/summary.json /tmp/cumulus-tango-first-summary.json
  bash cumulus/scripts/verify-tango-mvp.sh
  ```
...
- [ ] **Step 8: Prove the live gate rejects a controlled failure without modifying production files**
  ...
  Invoke the harness validator against one synthetic compiler-error log, invoke `TangoGpuAcceptance` against one synthetic metric outside its threshold, and feed the scope guard a synthetic forbidden path/runtime allocation. Expected: all three return nonzero and identify the exact rejected signature, metric, or scope rule. Do not introduce and revert a compiler error in tracked Unity source; committed negative controls are the safe proof that the gate fails closed.

- [ ] **Step 9: Commit and push**

  ```bash
  git add cumulus/.gitignore cumulus/scripts cumulus/Assets/TangoMvp
  git commit -m "test(cumulus): add autonomous Tango MVP verification gate" -m "Fail closed on Unity and shader compilation errors, validate NUnit outputs and strict logs, exercise deterministic GPU rendering metrics, build a standalone player, and emit machine-readable evidence through one agent-safe command."
  git push
  ```
...
**Interfaces:**
- Consumes: `cumulus/scripts/verify-tango-mvp.sh` and its ignored evidence directory.
...
- [ ] **Step 2: Run the authoritative gate from the README command**
  ...
  Expected: exit `0` and `cumulus/Artifacts/TangoMvpVerification/summary.json` reports `overall: "passed"`. This command supersedes ad hoc compile/test commands for completion claims.
...
- [ ] **Step 4: Perform optional visual review and retain artifacts**
  ...
  An agent may inspect the generated phase PNGs and open `TangoGlassLab.unity` at `1920 × 1080` to catch aesthetic issues outside the MVP gate. Record observations separately from the automated verdict. Do not convert an aesthetic preference into a passing result when an automated criterion failed.
...
- [ ] **Step 6: Commit and push**
  ...
  git commit -m "docs(cumulus): document autonomous Tango MVP verification" -m "Lead with the fail-closed agent command, define evidence and threshold interpretation, map acceptance criteria to automated checks, and retain visual review as optional supporting evidence."
  git push
  ```
...
- [ ] `verify-tango-mvp.sh` exits `0` twice consecutively and both summaries report `overall: "passed"`.
...
- [ ] Asset/shader contract tests prove fixed Tango tint, saturation, rim, sheen, and lit-shell roles.
