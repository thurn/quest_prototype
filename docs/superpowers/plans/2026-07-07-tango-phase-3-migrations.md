# Tango Revisions Phase 3: Migrations — Implementation Plan
...
> the worktree's existing branch (`worktree-tango-audit-revisions`).
>
...
> - [ ] Task 8 — StartingDeckModal → Tango tier (overlay + view-model + adapter)
...
- **Consumes Phase 0–2 outputs (do not recreate):** `src/tango/internal/`
  (`glass-surface.ts`, `control-treatment.ts`); the `--glass-*` tokens;
  `--surface-chrome`/`--surface-chrome-strong` (renamed from `--surface-glass*`);
  `IconButton` (`src/tango/components/controls/IconButton.tsx`, sizes `sm`=40/22,
  `md`=48/26); `GlassButton`; `GlassDialog` + `GlassBackdrop`
  (`src/tango/components/overlay/GlassDialog.tsx`); `economy-spec.ts`;
  `useScaleToFit`; the integrity-test `BASELINE` arrays in `scripts/tango-*.test.mjs`;
  and the `eslint.config.js` `tango/internal` `no-restricted-imports` block whose
...
- **Tiers:** presentational Tango screens/components live in `src/tango/**`
  (pure, props-driven, cannot import `src/components/**` — enforced by
  `no-external-ui-imports`, `ALLOWED_PREFIXES` = `src/tango/`, `src/data/`,
  `src/types/`, `src/runtime/`). Adapters live in `src/screens/tango_adapters/**`,
  are wiring-only (`tango/thin-adapters` + `max-lines` ≤120 on `*Adapter.tsx`),
  and delegate all domain→view mapping to a pure `*-view-model.ts` builder
  (`no-restricted-imports` bans `react`/state imports there).
- **Registry:** `tangoScreenFor` / `tangoSiteScreenFor`
  (`src/screens/tango_adapters/registry.tsx`) resolve site screens; the
...
with the local `tango` plugin (`eslint-rules/`) + cross-file integrity tests
(`scripts/tango-*.test.mjs`). Boxicons v3 vendored at `src/vendor/boxicons/`.
...
- **Docs describe the current state**, never removed behavior ("no longer",
  "unlike before" are banned in `docs/` and `src/tango/docs/`).
...
**Files**
- `src/tango/screens/DesktopDeckViewer.tsx` — the bespoke corner close disc, the
  local `CLOSE_BUTTON_PX = 40` constant, and their imports.
- `src/tango/docs/` — the `IconButton` registry entry (delete `incubating`).

**Interfaces**
- Consumes: `IconButton({ glyph, size?, label, onPress, disabled? })` from
  `src/tango/components/controls/IconButton.tsx`; `GLYPHS.close`.
...
**Files**
- `src/tango/screens/MobileDeckViewer.tsx` — the bespoke absolutely-positioned
...
- `src/tango/primitives/glyph.ts` — add `gear`, `menu` to `GLYPHS`.
- `src/tango/components/controls/IconButton.tsx` — add optional `testId?` +
  `ariaExpanded?` passthrough (see note).
- `eslint.config.js` — delete the `DreamscapeQuestMenu.tsx` entry from the
  `tango/internal` `no-restricted-imports` `ignores` list.

**Interfaces**
- Consumes: `IconButton` (`md`), new `GLYPHS.gear` (`bxf bx-cog`) /
  `GLYPHS.menu` (`bxf bx-menu`).
- Produces: `DreamscapeQuestMenu` no longer imports any `tango/internal` recipe
...
- `DreamscapeQuestMenu.tsx` imports `IconButton` and `GLYPHS` from `src/tango/**`
  (legal — legacy screens may render public Tango components). Delete the
...
- Delete the `DreamscapeQuestMenu.tsx` `ignores` entry from `eslint.config.js`;
  confirm the file now imports nothing from `tango/internal`.
...
**Files**
- `src/tango/screens/quest-start-mobile.tsx` — the `EdgeChevron` component body.
...
**Files**
- `src/tango/components/hud/QuestStatusBar.tsx` — the dreamsigns-window close
  button.
...
**Files**
- `src/tango/screens/DesktopDeckViewer.tsx` — the sort block (`Select` +
  `SegmentedControl` wrapped in layout-glue) and its imports.
- `src/tango/screens/desktop-deck-filter.ts` — `SORT_DIRECTION_OPTIONS` (remove
  if unused after this change; the `SortDirection` type and `direction: "asc"`
  default stay).
- `src/tango/primitives/glyph.ts` — add `chevronUp`.
...
**Files**
- `src/tango/screens/DesktopDeckViewer.tsx` — the local `GlassBackdrop` and local
  `GridPlaceholder`.
- `src/tango/screens/MobileDeckViewer.tsx` — the local `GlassBackdrop` and local
  `GridPlaceholder`.
- `src/tango/screens/deck-viewer-shared.tsx` (**new**) — the single
  `GridPlaceholder`.
- `src/tango/components/overlay/GlassDialog.tsx` — already exports `GlassBackdrop`.

**Interfaces**
- Consumes: `GlassBackdrop` from `GlassDialog.tsx`.
- Produces: `export function GridPlaceholder({ message }: { message: string }): ReactElement`
  from `src/tango/screens/deck-viewer-shared.tsx` — the centered muted message
...
**Files**
- `src/tango/screens/StartingDeckOverlay.tsx` (**new** — presentational)
- `src/screens/tango_adapters/starting-deck-view-model.ts` (**new** — pure)
- `src/screens/tango_adapters/starting-deck-view-model.test.ts` (**new**)
- `src/screens/tango_adapters/StartingDeckOverlayAdapter.tsx` (**new** — thin, ≤120 lines)
- `src/screens/tango_adapters/StartingDeckOverlay.test.tsx` (**new** — ported behavior test)
- `src/tango/components/overlay/GlassDialog.tsx` — extend with optional `cutoutAwareClose`.
...
- `eslint.config.js` — delete the `StartingDeckModal.tsx` `tango/internal` `ignores` entry.
- `scripts/tango-legacy-ratchet.test.mjs` — delete the `StartingDeckModal` `BASELINE` entry.
- `src/runtime/qa-scenes.ts` + `docs/quest_prototype/qa_scenes.md` — keep/extend
  the `startingdeck` scene.
- `src/tango/docs/` — the `GlassDialog` registry entry (delete `incubating`).
...
- Delete the `StartingDeckModal.tsx` `ignores` entry (`eslint.config.js`) and the
  `StartingDeckModal` `BASELINE` entry (`scripts/tango-legacy-ratchet.test.mjs`).
...
**Commit:** `refactor(starting-deck): migrate modal to Tango overlay + adapter`
...
**Files**
- `src/tango/components/hud/QuestStatusBar.tsx` — the `QsbDreamsign` type,
  `QsbSignObject`, its use in the strip and the window, and the `DS_SHADOW` const.
- `src/tango/components/hud/Dreamsign.tsx` — extend with a `variant`; receive
  `DS_SHADOW`.
- `src/screens/tango_adapters/dreamscape-view-model.ts` — `toQsbDreamsigns`.

**Interfaces**
- Consumes: shared `Dreamsign` (`src/tango/components/hud/Dreamsign.tsx`).
...
**Files**
- `src/tango/components/atlas/SiteNode.tsx` — the `DreamscapeMotes` component.
- `src/tango/components/atlas/site-node.css` — the `.ds-motes` rules.
- `src/tango/components/hud/Motes.tsx` — `MoteTint`, `TINTS`.
- `src/tango/primitives/tango-tokens.css` + the `tokens.ts` mirror — new
...
- Read the `.ds-motes span` background/box-shadow color out of `site-node.css`;
  ...
  `tango-tokens.css` with those exact values; regenerate the `tokens.ts` mirror
...
- `src/tango/components/atlas/SiteNode.tsx` → `src/tango/components/dreamscape/SiteNode.tsx`
- `src/tango/components/atlas/site-node.css` → `src/tango/components/dreamscape/site-node.css`
- `src/tango/components/atlas/dreamscape-scatter.ts` → `src/tango/components/dreamscape/dreamscape-scatter.ts`
- Importers to update (enumerated by grep):
  - `src/tango/screens/DreamscapeScreen.tsx` (`../components/atlas/SiteNode`)
  - `src/tango/screens/DreamscapeScreen.test.tsx`
  - `src/screens/DreamscapeScreen.tsx` (`../tango/components/atlas/SiteNode` + `…/dreamscape-scatter`)
  - `src/screens/tango_adapters/dreamscape-view-model.ts`
  - `src/tango/docs/mockups/site-node.tsx`
  - `src/tango/docs/demos/site-node.tsx` (and the demo code-string sample inside it)
  - registry/metadata references: `src/tango/docs/mockups/registry.ts`,
    `src/tango/metadata/tango-metadata.json` (regenerated), and any
...
**Contract**
- `git mv` the three files into `src/tango/components/dreamscape/`
...
- Run `scripts/regenerate-assets.sh` (regenerates `tango-metadata.json`); stage
...
**Commit:** `refactor(tango): move SiteNode + scatter to components/dreamscape/`
...
**Files**
- `src/tango/screens/DesktopDeckViewer.tsx` — the `HOVER_TARGET_WIDTH_PX` map.
- `src/tango/components/card/HoverZoomCard.tsx` — `export const MAX_SCALE = 1.5`.
...
**Files**
- `src/tango/components/atlas/AtlasNode.tsx` — the raw
  `role={isAvailable ? "button" : "img"}` div with `onClick` + `tabIndex` +
  hand-rolled `onKeyDown` Enter/Space.
- `src/tango/components/atlas/atlas.css` — node centering
  (`transform: translate(-50%,-50%)`).

**Interfaces**
- Consumes: `Pressable` (`src/tango/primitives/Pressable.tsx`).
...
**Files**
- `src/tango/primitives/tango-tokens.css` + the `tokens.ts` mirror —
  `--node-hover-scale`.
- `src/tango/components/atlas/atlas.css` — the `.is-hover .node-art` `scale(1.07)`.
- `src/tango/components/dreamscape/SiteNode.tsx` — the node hover `scale(1.08)`
  (post-move path).
...
**Contract**
- Add `--node-hover-scale: 1.08;` to `tango-tokens.css` (comment: the map-node
...
**Files**
- `src/tango/screens/chrome-geometry.ts` (**new** — shared source of truth)
- `src/components/DreamscapeQuestMenu.tsx` — consume the shared consts.
- `src/tango/screens/DraftScreen.tsx` — the top-band clearance math (`TOP_SAFE_OP`).
...
`src/tango/**` and **cannot import from `src/components/**`** (`no-external-ui-imports`,
`ALLOWED_PREFIXES`). So the pinned constants `MENU_BUTTON_PX` /
`MENU_EDGE_INSET_MOBILE_PX` live in a shared **Tango** module
(`src/tango/screens/chrome-geometry.ts`) that BOTH the legacy menu (allowed to
import from `src/tango/`) and `DraftScreen` import. The pinned names are
preserved verbatim; only the export location is a shared Tango spec (the §162
...
- Browser QA the draft screen top band clears the hamburger:
  ```bash
  agent-browser --session t15 goto "http://localhost:5174/?goto=draft&ui=tango&deviceFrame=iphone16"
...
`PoolDraftState` with `activeSiteId: null`. So the Tango `DraftSiteScreenAdapter`
(mobile-gated via `tangoSiteScreenFor`) can
...
**Contract**
- QA `?goto=draft&ui=tango` at a mobile viewport and confirm the pack paints a
...
- Add a `?goto=draft` row to `docs/quest_prototype/qa_scenes.md`: what it shows
  (the Tango draft site with a rolled first offer + the floating
  `Draft (n/total)` pick counter over the starter dreamscape), that it is
  mobile-gated for the Tango screen (desktop falls back to the legacy draft
  screen), and the `&ui=tango` + `&deviceFrame=` usage. Current behavior only
...
- Browser QA:
  ```bash
  agent-browser --session t16 goto "http://localhost:5174/?goto=draft&ui=tango&deviceFrame=iphone16"
...
- `eslint.config.js` — register + apply under the `src/tango/**` +
  `src/screens/tango_adapters/**` block.
- `scripts/tango-glyphs-exist.test.mjs` (**new** — generation-time companion) OR
  a check folded into `scripts/generate-tango-metadata.mjs`.
- (bonus) `src/screens/ShopScreen.tsx` + `pre-existing-issues.txt`.

**Rule spec.** A Boxicons class string (`bxf …` or a token containing `bx-*`)
literal is legal **only** in `src/tango/primitives/glyph.ts`. Everywhere else
under `src/tango/**` and `src/screens/tango_adapters/**` a glyph must arrive as a
...
binds `src/tango/**` + `src/screens/tango_adapters/**`). Detect string/template
literals matching `/\bbxf?\b|\bbx-[a-z-]+/` and report unless the file is
`src/tango/primitives/glyph.ts`.
...
- Author `no-raw-icon-classes.js` mirroring `valid-token-references.js`'s
  ...
  `src/tango/primitives/glyph.ts`, scan `Literal` + `TemplateElement`. The message
...
- Register in the `tango` plugin and add `"tango/no-raw-icon-classes": "error"` to
  the `src/tango/**` + `tango_adapters/**` block in `eslint.config.js`.
- Add `scripts/tango-glyphs-exist.test.mjs` (glyph→css existence) running under
...
- `scripts/tango-css-press-scale.test.mjs` (**new**) — the CSS companion scan
  (decision below).

**Rule spec (JS).** A `scale(`/`scaleX(`/`scaleY(` call inside a `transform`
value, in any file under `src/tango/**` **except** `Pressable.tsx`, must not take
...
`scripts/tango-duplicate-literals.test.mjs` is a Phase-2 artifact whose job is
duplicate detection; adding a scale scan there overloads it. Add a dedicated
`scripts/tango-css-press-scale.test.mjs` that greps every `src/tango/**/*.css`
...
**Contract**
- Author `no-adhoc-press-scale.js` (self-scoped to `src/tango/**` minus
...
- Register + apply `"tango/no-adhoc-press-scale": "error"`.
- Add `scripts/tango-css-press-scale.test.mjs` running under `npm test`.
...
`src/tango/**`; the rule self-exempts the components tier via
`EXEMPT_PREFIXES` (the `"src/tango/components/"` entry). **Delete that entry** so
the rule also polices `src/tango/components/**`. Keep the
...
**Contract**
- Remove `"src/tango/components/"` from `EXEMPT_PREFIXES`.
- Add an invalid RuleTester case with a `src/tango/components/**` filename (a raw
...
- `scripts/regenerate-assets.sh` → run it; `git status` must show either no drift
  or only intended generated files (tokens mirror, `tango-metadata.json`, docs
...
- **Sweep assertion — no `glassIconButtonChrome()` call site survives outside
  ...
  grep -rn "glassIconButtonChrome" src --include="*.ts" --include="*.tsx" \
    | grep -v "src/tango/components/controls/IconButton.tsx" \
    | grep -v "src/tango/internal/"
  ```
  Expected output: **empty** (the recipe is referenced only by `IconButton` and
  its own `src/tango/internal/` definition). Any printed line is a missed call
  site — migrate it before closing the phase.
- **Incubating-flag audit:** `grep -rn "incubating" src/tango/docs` — confirm
...
- Confirm both `tango/internal` `ignores` entries (`StartingDeckModal.tsx`,
  `DreamscapeQuestMenu.tsx`) and the `StartingDeckModal` legacy-ratchet `BASELINE`
  entry are gone.
- Final `git push`.

**Commit:** `chore(tango): Phase 3 boundary — checks green, generated drift committed`
...
`MENU_EDGE_INSET_MOBILE_PX` constants (Task 15) are exported from a shared Tango
module (`src/tango/screens/chrome-geometry.ts`) rather than literally from
`src/components/DreamscapeQuestMenu.tsx`, because `no-external-ui-imports`
forbids `DraftScreen` (`src/tango/**`) importing from `src/components/**`. The
