...
- [ ] **Step 8: Three stub sites.** Add `StubSiteScreen.tsx` rendering the guide framing + a short "coming soon" body + a Continue button that calls `completeJourneySite`. Add `ScreenRouter` cases for `TemptingOffer`, `Gamble`, `TemporalFork` → `StubSiteScreen`.
