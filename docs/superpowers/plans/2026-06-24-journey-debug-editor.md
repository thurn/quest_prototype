# Journey State Debug Editor Implementation Plan
...
**Tech Stack:** TypeScript, React, Framer Motion, Tailwind + inline styles, Vitest. Firebase RTDB persistence via the existing multiplayer journey context.
...
**Files:**
- Modify: `src/types/journey.ts` (DeckEntry, ~line 91)
- Modify: `src/card-type-change.ts`
- Test: `src/card-type-change.test.ts`

- [ ] **Step 1: Add the `statOverride` field to `DeckEntry`.**

In `src/types/journey.ts`, add to `DeckEntry` (keep the existing fields):
...
- [ ] **Step 6: Commit.** `git add -A && git commit -m "feat(journey-editor): add DeckEntry statOverride and resolveDeckEntryCard"`
...
- [ ] **Step 6: Commit.** `git add -A && git commit -m "feat(journey-editor): apply statOverride in battle and deck display paths"`
...
**Files:**
- Modify: `src/state/journey-context.tsx` (`JourneyMutations` interface)
- Modify: `src/state/multiplayer-journey-context.tsx` (implementations + provider value object)
- Test: `src/state/multiplayer-journey-context.test.tsx`

Seven new mutations, all declared **optional** (`?`) on `JourneyMutations` to match the existing debug-only pattern. Reuse semantics from the spec; replace-not-merge for the keyword/type setters so the editor can clear back to printed.

- [ ] **Step 1: Add the optional signatures to `JourneyMutations` in `journey-context.tsx`:**
...
- [ ] **Step 2: Write failing tests in `multiplayer-journey-context.test.tsx`.** Use the existing `onJourney`/captured-`JourneyContextValue` harness in that file (see its `onJourney` helper around line 486). Specify these guarantees:
  ...
  3. **Debug edits preserve run invariants** (catches accidental invariant break): after a `setDeckEntryStatOverride` edit on a state that has a `dreamAvatar`, `state.dreamAvatar` is still non-null (the edit goes through `writeRoomTransaction`'s validator without tripping it).
...
- [ ] **Step 3: Run, verify failures** (mutations undefined). `npm test -- src/state/multiplayer-journey-context.test.tsx`

- [ ] **Step 4: Implement the seven mutations in `multiplayer-journey-context.tsx`.**

Each is a `useCallback` following the established `changeDeckEntryType` shape: read `currentRef.current`, `writeRoomTransaction` with an updater that no-ops when `room`/`journeyState` is null (or the target entry/index is missing), maps/sets the field immutably, bumps `metadata.updatedAt`. Add each to the provider's `mutations` value object (near the existing deck/essence mutations). Representative implementation for the deck-entry stat override (the others follow the same shape over their respective field; the scalar setters write the scalar directly):
...
const setDeckEntryStatOverride = useCallback(
  (
    ...
    writeRoomTransaction({
      ...
      updater: (room) => {
        if (room === null || room.journeyState === null) return room ?? undefined;
        const entry = room.journeyState.deck.find((c) => c.entryId === entryId);
        if (entry === undefined) return room;
        logEvent("journey_debug_edit", {
          target: "deckEntry",
          field: "statOverride",
          entryId,
        ...
        return {
          ...
          journeyState: {
            ...room.journeyState,
            deck: room.journeyState.deck.map((c) =>
              c.entryId === entryId
                ? statOverride === null
                  ? omitStatOverride(c)
                  : { ...c, statOverride }
...
- [ ] **Step 6: Commit.** `git add -A && git commit -m "feat(journey-editor): add debug-only journey-state mutations with journey_debug_edit logging"`

---

## Task 4: `JourneyDebugEditor` overlay component

**Files:**
- Create: `src/screens/JourneyDebugEditor.tsx` (overlay shell + Resources + Dreamsigns sections)
- Create: `src/screens/JourneyDebugDeckSection.tsx` (add-card picker + deck list + per-entry editor)
...
Props: `{ isOpen: boolean; onClose: () => void }`. The component reads `useJourney()` for `state`, `mutations`, `cardDatabase`, and `journeyContent`. Resolve card/dreamsign **names only at render time**; key everything internally by `cardNumber` / `entryId` / `dreamsign id` (AGENTS.md: never key logic by card name).

- [ ] **Step 1: Build the overlay shell + Resources section in `JourneyDebugEditor.tsx`.**

Copy the `DebugScreen.tsx` overlay scaffold (AnimatePresence + `motion.div` full-screen backdrop, header with title "Edit Journey State (debug)" and a × close button, `Escape`-to-close effect, scrollable body). Resources section: numeric `<input type="number">` rows for `essence`, `essenceCap`, `maxDreamsigns`, `completionLevel`, each committing on blur / Enter via the matching mutation (`setEssence`, `setEssenceCap`, `setMaxDreamsigns`, `setCompletionLevel`) with `source: "journey_debug_editor"`. Mirror the label/input styling from `src/editor/TidesDetailView.tsx`.
...
- [ ] **Step 5: Commit.** `git add -A && git commit -m "feat(journey-editor): add JourneyDebugEditor overlay and deck section"`
...
Guarantee (catches the menu entry being absent or mis-wired): rendering the HUD with an `onOpenJourneyEditor` spy, opening the utility menu (click `hud-utility-menu-button`), and clicking the "Edit Journey State" item (`hud-journey-editor-button`) calls the spy once. Follow the existing menu-interaction tests in this file for the open-menu setup.

- [ ] **Step 2: Run it, verify it fails.** `npm test -- src/components/HUD.test.tsx`

- [ ] **Step 3: Add the HUD prop and menu item.**

In `HUD.tsx`, add `onOpenJourneyEditor: () => void` to the props interface (near `onOpenDebugScreen` at line 65) and to the destructured params (near line 86). In the `menuView === "root"` block, add (placement near "Package Debug" is fine):
...
<UtilityMenuButton
  ...
  testId="hud-journey-editor-button"
/>
```

- [ ] **Step 4: Wire `App.tsx`.** Mirror the `debugScreen` wiring exactly: add `const [journeyEditorOpen, setJourneyEditorOpen] = useState(false);` (near line 64), `handleOpenJourneyEditor`/`handleCloseJourneyEditor` callbacks (near line 315), pass `onOpenJourneyEditor={handleOpenJourneyEditor}` to `<HUD>` (near line 417), and render the overlay near the other overlays (line ~468):

```tsx
<ErrorBoundary scope="overlay:journey-editor" onClose={handleCloseJourneyEditor}>
  <JourneyDebugEditor isOpen={journeyEditorOpen} onClose={handleCloseJourneyEditor} />
</ErrorBoundary>
```

Import `JourneyDebugEditor` from `./screens/JourneyDebugEditor`. If any other component constructs HUD props (e.g. `HudDreamsignLayoutDemo.tsx`), add a no-op `onOpenJourneyEditor`.

- [ ] **Step 5: Run tests + typecheck, verify pass.** `npm test -- src/components/HUD.test.tsx && npm run typecheck`

- [ ] **Step 6: Commit.** `git add -A && git commit -m "feat(journey-editor): surface JourneyDebugEditor in the HUD utility menu"`
...
- [ ] **Step 2: Browser QA** per AGENTS.md. Start a QA server on a non-default port (`npm run dev -- --port 5174`, capture its PID). Boot into a live journey (e.g. `http://localhost:5174/?startInBattle=1` or a `?goto=` scene that yields a populated deck/dreamsigns). Open the "..." menu, choose "Edit Journey State", and verify: essence/cap/maxDreamsigns/completionLevel edits persist and reflect in the HUD; adding/removing a dreamsign updates the list; adding a card by search and removing a deck entry update the deck; editing a card's energy cost / spark / transfiguration / subtype / keywords shows in the deck viewer and (spot-check) carries into a battle. Inspect the captured error buffer for render errors, unhandled rejections, and console errors. Confirm layout is coherent (no clipping/overlap) at the tested viewport. Tear down by killing only your PID / `pkill -f "vite --port 5174"` (never a broad `pkill -f vite`).

- [ ] **Step 3: Confirm logging.** Trigger several edits, then verify `logs/journey-log.jsonl` contains `journey_debug_edit` entries with `target`/`field`/`before`/`after`/`source` so the session's edits are reconstructable.
