...
- `data/dreamsign_signatures.toml` — generated by the dreamsign-classification workflow (Task 6).
...
**Files:**
- Create: `data/dreamsign_signatures.toml`
...
For every returned `signatureCards[].id`: lowercase, confirm it exists in `public/cards_v2-data.json`; drop/flag any that don't and report. Then emit `data/dreamsign_signatures.toml`:
...
```bash
git add data/dreamsign_signatures.toml scripts/config-data.mjs scripts/setup-assets.mjs src/data/journey-content.ts public/dreamsign-signatures-data.json src/data/journey-content.dreamsign-signatures.test.ts
