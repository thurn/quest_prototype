...
**Goal:** Build `localhost/tango`, a self-contained, interactive documentation endpoint for the Tango UI design system, and make `src/tango/` the home of the shared UI component library.

**Architecture:** A new `/tango` pathname branch in `src/main.tsx` lazy-loads a `TangoApp` that uses a tiny custom hash router. Design tokens are imported from Claude Design as CSS custom properties (source of truth) plus a generated typed mirror. A standalone docgen script emits one metadata JSON that drives both the programmatic props tables and the auto-generated interactive controls. All UI lives under `src/tango/`, isolated from the rest of the app by a fail-closed custom ESLint rule; reused components (game card, rules text, atlas) physically move into Tango and external call sites re-point.
...
src/tango/
  primitives/
    tango-tokens.css        # imported CSS custom properties (source of truth), scoped .tango
    tokens.ts               # GENERATED typed mirror of tango-tokens.css
  ...
  docs/                      # the /tango page itself (Tango-only UI)
    TangoApp.tsx             # route shell + hash router mount
    route.ts                 # parseTangoRoute (pure) + types
    route.test.ts
  ...
  metadata/
    tango-metadata.json      # GENERATED docgen output

scripts/
  generate-tango-metadata.mjs   # react-docgen-typescript -> tango-metadata.json
  lib/tango-css-tokens.mjs      # pure CSS-var parser (shared by generator + tests)
  generate-tango-tokens.mjs     # tango-tokens.css -> tokens.ts
...
Files that change together live together: each component's demo and mockup sit under `src/tango/docs/demos` and `src/tango/docs/mockups`, keyed by the same id used in `registry.ts`.

---

## Decisions already made (do not re-open)

- **Route:** `/tango` in `src/main.tsx`, following the `/editor` / `/dreamsigns` lazy-import pattern. Internal nav = hash router. No react-router, no server rewrites.
- **Isolation:** `src/tango/**` may import only `src/tango/**`, bare `node_modules`, and the allowlist `src/data/`, `src/types/`, `src/runtime/`, `src/logging.ts`. Everything else under `src/` is denied. Reused components move in; external call sites re-point.
- **Tokens:** CSS custom properties from the design (`tokens/*.css`) are the source of truth; `tokens.ts` is generated from them.
- **Docgen:** `react-docgen-typescript` → `tango-metadata.json`; the same JSON drives props tables and control inference.
...
Create `TangoApp` as a default-exported component rendering a single `<h1>Tango Design System</h1>` inside a `.tango` root element. This is a placeholder replaced in later tasks; keep it tiny.

- [ ] **Step 3: Verify it loads**

Run: `npm run dev:vite -- --port 5174` then load `http://localhost:5174/tango` via `agent-browser`. Expected: the heading renders, no console errors. Tear down only your own server (`pkill -f "vite --port 5174"`).
...
```bash
git add src/main.tsx src/tango/docs/TangoApp.tsx
git commit -m "feat(tango): add /tango route with placeholder shell"
git push
```
...
**Files:**
- Create: `src/tango/primitives/tango-tokens.css`
...
- [ ] **Step 2: Assemble `tango-tokens.css`**

Concatenate the fetched custom-property declarations into `src/tango/primitives/tango-tokens.css`, wrapping the `:root` blocks in a `.tango` selector so tokens are scoped to the Tango subtree (not leaked app-wide). Keep the values **verbatim**; do not rename tokens. Preserve the `@kind` trailing comments (`/* @kind radius */`, etc.) — the token-doc generator reads them to group tokens.
...
Add `import "../primitives/tango-tokens.css";` to `TangoApp.tsx`.

- [ ] **Step 4: Verify tokens resolve**

Run the dev server, load `/tango`, and in the browser console confirm `getComputedStyle(document.querySelector('.tango')).getPropertyValue('--accent')` returns `#a855f7` (via its alias chain). Expected: non-empty.

- [ ] **Step 5: Commit** (`feat(tango): import design tokens as scoped CSS custom properties`, then push)
...
**Files:**
- Create: `scripts/lib/tango-css-tokens.mjs`
- Test: `scripts/lib/tango-css-tokens.test.mjs`
...
- [ ] **Step 2: Run → FAIL** (`node --test scripts/lib/tango-css-tokens.test.mjs`; expected: cannot find export `parseCssTokens`)
...
- [ ] **Step 5: Commit** (`feat(tango): add CSS custom-property parser`, push)

### Task 0.4: Generate `tokens.ts` from the CSS

**Files:**
- Create: `scripts/generate-tango-tokens.mjs`
- Create (generated, committed): `src/tango/primitives/tokens.ts`
- Modify: `scripts/regenerate-assets.sh`

- [ ] **Step 1: Write the generator**

`generate-tango-tokens.mjs` reads `src/tango/primitives/tango-tokens.css`, calls `parseCssTokens`, and writes `tokens.ts` exporting a typed const object mapping each token name (as a `var(--name)` string, and its raw value) so TS callers use `token("--accent")` or a typed `TOKENS` record. Include a generated-file header comment. The generated file is committed (like `public/*-data.json` peers are baked, but this one is tracked).

- [ ] **Step 2: Wire into regenerate-assets**

Add a numbered step to `scripts/regenerate-assets.sh` (renumber the header comment + `step` labels) that runs `node scripts/generate-tango-tokens.mjs` and `node scripts/generate-tango-metadata.mjs` (the latter created in Phase 1). Place it after setup-assets step 6.

- [ ] **Step 3: Run it**

Run: `node scripts/generate-tango-tokens.mjs`. Expected: `src/tango/primitives/tokens.ts` written, `npm run typecheck` PASS.

- [ ] **Step 4: Commit** (`feat(tango): generate typed tokens.ts mirror`, push)
...
tester.run("no-external-ui-imports", rule, {
  valid: [
    // tango-internal relative import
    { code: `import { Button } from "./Button";`, filename: "src/tango/components/Foo.tsx" },
    // allowlisted non-UI infra
    { code: `import type { CardData } from "../../types/cards";`, filename: "src/tango/components/GameCard.tsx" },
    { code: `import { logEvent } from "../../logging";`, filename: "src/tango/docs/demos/x.tsx" },
    // bare node_modules
    { code: `import React from "react";`, filename: "src/tango/components/Foo.tsx" },
    // files OUTSIDE tango are unrestricted (rule is a no-op there)
    { code: `import { HUD } from "./HUD";`, filename: "src/components/App.tsx" },
  ],
  invalid: [
    // external UI dir
    { code: `import { HUD } from "../../components/HUD";`, filename: "src/tango/components/Foo.tsx", errors: 1 },
    { code: `import { AtlasScreen } from "../../screens/AtlasScreen";`, filename: "src/tango/docs/TangoApp.tsx", errors: 1 },
  ],
});
```

Bug classes pinned: **fail-closed default** (an import to a non-allowlisted `src/` path errors — catches a denylist that misses new UI dirs), **allowlist correctness** (types/logging/runtime/data resolve clean — catches an over-strict rule that breaks legitimate infra use), **scope** (the rule is inert outside `src/tango/` — catches a rule accidentally applied repo-wide), **bare-module pass-through** (node_modules never flagged).

- [ ] **Step 2: Run → FAIL** (`npx vitest run eslint-rules/no-external-ui-imports.test.ts`; expected: module not found)

- [ ] **Step 3: Implement the rule**

The rule's decision (embedded because it defines the allowlist contract): for each `ImportDeclaration`/`ExportNamedDeclaration` with a source in a file under `src/tango/`, resolve the source relative to the file; **allow** iff the resolved path is under `src/tango/`, OR the specifier is a bare module (does not start with `.`), OR the resolved path is under one of `ALLOWLIST = ["src/data/", "src/types/", "src/runtime/"]` or equals `"src/logging.ts"`. Otherwise report. Everything else about the rule (message text, `meta`) is boilerplate — describe, don't embed.

- [ ] **Step 4: Run → PASS**

- [ ] **Step 5: Register in `eslint.config.js`**

Add a flat-config object scoped to `files: ["src/tango/**/*.{ts,tsx}"]` that registers the local rule under a `tango` plugin namespace and sets it to `error`. Remove `eslint-rules/` from being linted with the type-checked project if it trips `projectService` (add to the config's ignore list if needed, since it is Node tooling, not app `src`).

- [ ] **Step 6: Prove the gate works end-to-end**

Temporarily add `import { HUD } from "../../components/HUD";` to a scratch file under `src/tango/`, run `npm run lint`, confirm it errors, then remove it. Expected: lint fails on the bad import, passes once removed.

- [ ] **Step 7: Commit** (`feat(tango): enforce fail-closed UI-import boundary via custom eslint rule`, push)
...
- Create: `scripts/generate-tango-metadata.mjs`
- Create (generated, committed): `src/tango/metadata/tango-metadata.json`
- Create (fixture): `src/tango/components/__docgen_fixture__.tsx`
- Test: `scripts/generate-tango-metadata.test.mjs`

- [ ] **Step 1: Install the dev dep**

Run: `npm install -D react-docgen-typescript`. Add an `npm run tango-metadata` script pointing at the generator.
...
- [ ] **Step 4: Run → FAIL**, then implement the generator: parse with `react-docgen-typescript` (using the repo `tsconfig.json`), map each component's `.props` into `PropMeta[]`, keyed by component display name, and write `tango-metadata.json`. Normalizing union members from the parser's enum `type.value` is the one non-obvious step — everything else follows from the shape above.

- [ ] **Step 5: Run → PASS**

- [ ] **Step 6: Commit** (`feat(tango): generate component prop metadata via react-docgen-typescript`, push)

### Task 1.2: Control inference (TDD)

**Files:**
- Create: `src/tango/docs/controls.ts`
- Test: `src/tango/docs/controls.test.ts`
...
- [ ] **Step 5: Commit** (`feat(tango): infer demo controls from prop metadata`, push)

### Task 1.3: Hash router (TDD)

**Files:**
- Create: `src/tango/docs/route.ts`
- Test: `src/tango/docs/route.test.ts`

- [ ] **Step 1: Write the failing test for `parseTangoRoute(hash: string): TangoRoute`**

`TangoRoute` (embedded contract):
...
- [ ] **Step 2–4:** Run → FAIL; implement `parseTangoRoute` (split the hash on `/`, branch on segment count and the `mockup` suffix); Run → PASS.

- [ ] **Step 5:** Add a `useTangoRoute()` hook wrapping `parseTangoRoute` over a `hashchange` listener. No test (thin DOM-event wrapper; the parse logic — the part that breaks — is already covered).

- [ ] **Step 6: Commit** (`feat(tango): add hash router for the tango doc site`, push)

### Task 1.4: Component registry, props table, control panel, demo stage

**Files:**
- Create: `src/tango/docs/registry.ts`, `PropsTable.tsx`, `ControlPanel.tsx`, `DemoStage.tsx`, `ComponentPage.tsx`

- [ ] **Step 1: Registry**

`registry.ts` maps a component id → `{ title, group, Component, docName, demo }`, where `docName` is the key into `tango-metadata.json` and `demo` supplies default args + content-typed prop values (for `none`-control props like `icon`). Registry starts empty; each component task appends its entry.

- [ ] **Step 2: PropsTable**

Renders `tango-metadata.json[docName]` as a table (prop, type, required, default, description). Pure presentation from generated data — no test (it renders generated JSON; a snapshot would pin volatile design data, which AGENTS.md forbids). Verified by QA.

- [ ] **Step 3: ControlPanel + DemoStage**

`ControlPanel` builds an input per prop from `controlForProp`, holding live arg state; `DemoStage` renders `<Component {...args} />` inside a `.tango` surface. `ComponentPage` composes DemoStage + ControlPanel + PropsTable for the routed id.

- [ ] **Step 4: Wire routing**

`TangoApp` switches on `useTangoRoute()`: `overview` → TOC; `component` → `ComponentPage`; `mockup` → the component's mockup (Phase 6). TOC lists registry entries grouped by `group`.

- [ ] **Step 5: QA + typecheck + lint + commit**

Run dev server, load `/tango`, confirm TOC renders and (after Phase 2 adds the first component) a component page shows demo + live controls + props table. `npm run typecheck && npm run lint`. Commit (`feat(tango): component page with generated props table and controls`, push).
...
2. Writes `src/tango/<primitives|components>/<Name>.tsx` — a faithful `.tsx` port: same props as the `.d.ts` (preserve its JSDoc verbatim so docgen populates the table), styling driven by `tango-tokens.css` custom properties (never raw hex), imports compliant with the isolation allowlist.
3. Writes `src/tango/docs/demos/<id>.tsx` with representative default args (and sample nodes for `none`-control props).
...
- `node scripts/generate-tango-metadata.mjs` then load `/tango#/<id>`: props table populated, every inferred control drives a visible change, component renders coherently at desktop and at an emulated 390×844 (DevTools). No console errors.
- Commit per component (`feat(tango): import <Name> component`), push.

### Components to import (Phase 2)

| id | Name | group / location | design path | Notes that constrain the port |
| --- | --- | --- | --- | --- |
| `pressable` | Pressable | primitives | `components/primitives/Pressable.jsx` | The one press-feedback primitive; scale-down `--press-scale` (0.94) on pointer-down; every interactive Tango control routes through it. Also expose `usePress`. |
...
| `button` | Button | components | `components/buttons/Button.jsx` | Beveled purple 9-patch. Background = `src/tango/assets/Button_Purple.png` bundled via Vite import, rendered as `border-image` 9-slice. Props `size` (`sm/md/lg`, `lg`=taller commit), `full`, `icon`, `cost`, `frameScale`. No color variants. Press via Pressable. |
...
**Files:** Create `src/tango/assets/Button_Purple.png`, `src/tango/assets/phosphor-fill.css` (+ font file)

- [ ] Copy `~/Documents/UI/ClassicFantasyRPG_UI/ARTWORKS/UIelements/Buttons/Medium/Button_Purple.png` into `src/tango/assets/`. Confirm it exists and is committed.
...
- [ ] Commit (`feat(tango): bundle button sprite and phosphor fill face`, push).
...
**Files:** Create `src/tango/components/InfoCard.tsx`; demo + registry entry.
...
**Files:** Create `src/tango/components/GroupPanel.tsx`; demo + registry entry.

Port the design's `components/surfaces/GroupPanel.jsx` **as-is** — it is already CSS-only (`backdrop-filter: blur(22px) saturate(1.5)`, specular gradient, inset rim/wash, drop shadow). Replace its literal rgba/px values with the corresponding Tango tokens where a token exists (`--border-soft`, `--r-popover`, `--space-6`); keep the bespoke glass gradient/shadow literals that have no token. Expose `groupPanelStyle(radius)` and the `GroupPanel` wrapper (`as`, `padding`, `radius`). No dependency. Recipe verification + commit + push.

### Task 3.3: QuestStatusBar

**Files:** Create `src/tango/components/QuestStatusBar.tsx`; demo + registry entry.

Import per recipe from `components/quest/QuestStatusBar.jsx`. The transparent bottom HUD (no surface); text/icons use `.hud-outline` (rung 1 of the legibility ladder) — port that outline utility into Tango CSS. Recipe verification + commit + push.

---

## Phase 4 — Move production components into Tango

These are **moves**, not imports. The production component and its component-specific helper closure relocate under `src/tango/`; the file physically moves; every external call site re-points; the component's existing tests move with it. Verification is **the moved tests still pass** plus typecheck, lint (boundary gate), and QA.

### Task 4.1: Move RulesText (+ closure)

**Files:**
- Move into `src/tango/components/`: `RulesText.tsx`, `card-text.ts`, `PipBadge.tsx` (and their `.test.ts(x)` files: `RulesText.test.tsx`, `RulesText.nesting.test.ts`, `rules-text-highlight.test.ts`, `card-text.test.ts`).
...
- [ ] **Step 2: Move the files** (git mv) into `src/tango/components/`, moving their tests alongside.
- [ ] **Step 3: Fix imports inside the moved files** so their remaining dependencies are either tango-internal or allowlisted (types). Any UI helper they pull that is not yet in Tango moves too (RulesText's closure is small: `card-text`, `PipBadge`).
- [ ] **Step 4: Re-point external call sites** to the new `src/tango/components/...` paths.
- [ ] **Step 5: Run the moved tests** — `npx vitest run src/tango/components/RulesText.test.tsx src/tango/components/RulesText.nesting.test.ts src/tango/components/rules-text-highlight.test.ts src/tango/components/card-text.test.ts`. Expected: PASS unchanged. These existing tests are the regression guarantee for the move (they pin the tokenizer/highlight contract); no new tests are added.
- [ ] **Step 6:** `npm run typecheck && npm run lint && npm test`. Expected: PASS, boundary rule green.
- [ ] **Step 7:** Add RulesText to the Tango registry with a demo rendering real authored rules text (a curated card's text). Commit (`refactor(tango): move RulesText and card-text into tango`), push.

### Task 4.2: Move GameCard/CardView (+ closure)

**Files:**
- Move into `src/tango/components/`: `CardView.tsx` (→ exported as `GameCard`), `card-display-scale.ts`, `GlowIcon.tsx`, `CardStatOrb.tsx`, `useCardTermPopover.tsx`, `useFitText.ts` (and their tests: `CardView.test.tsx`, `CardView.artpan.test.ts`, `HoverZoomCard.test.tsx` if it exercises CardView, plus any helper tests).
...
- [ ] **Step 2:** `git mv` the files + their tests into `src/tango/components/`. Re-export `CardView` as `GameCard` (keep a `CardView` alias if external code uses that name, to minimize churn — or re-point all callers; prefer re-pointing).
...
- [ ] **Step 7:** Register GameCard with a demo rendering a few curated real card UUIDs (deterministic set), showing `compact`, `exhausted`, `counters` affordances. Commit (`refactor(tango): move CardView into tango as GameCard`), push.
...
**Files:**
- Move into `src/tango/components/`: `AtlasNode.tsx`, `atlas-display.ts`, plus `DreamscapeSiteNode.tsx`'s dependency `dreamscape-scatter.ts` (shared with SiteNode in 5.2).
- Create: `src/tango/components/AtlasEdge.tsx`, `AtlasEdgeDefs.tsx` — the four connector kinds (`traveled/open/dim/locked`) and the shared SVG `<defs>` (gradients / flow markers). Local atlas is authoritative; the design's reconstruction is reference only.
...
- [ ] Re-point call sites; run any moved atlas tests; typecheck + lint + QA `/tango#/atlas-node` against a real atlas fixture (`__test-helpers__/atlas-fixtures`). Commit (`refactor(tango): port atlas node/edge/defs into tango`), push.

### Task 5.2: Unify Dreamsign and SiteNode through InfoCard

**Files:**
- Create: `src/tango/components/Dreamsign.tsx` (unify local `DreamsignHoverCard` / `DreamsignArtTile` with design `components/entities/Dreamsign.jsx`).
- Create: `src/tango/components/SiteNode.tsx` (unify local `DreamscapeSiteNode` with design `components/quest/SiteNode.jsx`).
...
- [ ] Re-point external call sites of the local originals to the new Tango components; move/keep their tests.
...
**Files:** Create `src/tango/docs/IntroSection.tsx`.

Condense the design's governing principles into the Intro (per the spec's §6): material continuity, the legibility ladder, popup rule R-17, and content voice. Prose content, dogfooding Tango tokens for its own chrome. No test. QA + commit + push.

### Task 6.2: Primitives section

**Files:** Create `src/tango/docs/PrimitivesSection.tsx`.

Render color / typography / radius / spacing / iconography / motion / glow specimens from `parseCssTokens(tango-tokens.css)` grouped by the `@kind` markers (color, radius, shadow, other) and by token-name prefix. Because it is generated from the tokens, it never goes stale by hand. No golden test (would pin volatile design values). QA + commit + push.

### Task 6.3: Per-component full-screen mockups

**Files:** Create `src/tango/docs/mockups/<id>.tsx` for each registered component; wire the `mockup` route in `ComponentPage`/`TangoApp`.

Each mockup composes the component into a realistic full-screen (`100vw×100vh`, responsive) UI using real content where it exists (curated card UUIDs, atlas fixtures, production art from `public/`). Reachable at `/tango#/<id>/mockup` with a back affordance to the component page. No unit tests (visual); QA each at desktop + emulated mobile. Commit per mockup (or per small batch), push.

### Task 6.4: Final sweep

- [ ] `npm run regenerate-assets` (exercises the new token + metadata generation steps); confirm the generated `tokens.ts` and `tango-metadata.json` are up to date and committed.
- [ ] `npm run lint && npm run typecheck && npm test` all green.
- [ ] Full `/tango` QA pass: TOC → every component page (demo + controls + props table) → every mockup, desktop + 390×844, checking the captured error buffer for render errors / console errors / unhandled rejections.
- [ ] Commit any regenerated artifacts (`chore(tango): refresh generated tokens and metadata`), push.
...
**Type consistency:** `PropMeta` (1.1) is consumed unchanged by `controlForProp` (1.2) and `PropsTable` (1.4). `ControlSpec` (1.2) is consumed by `ControlPanel` (1.4). `TangoRoute` (1.3) is consumed by `TangoApp` (1.4). `ALLOWLIST` (0.5) is extended in lockstep with its test in 4.2. Registry `docName` keys the metadata JSON from 1.1.

**Snippet justification:** Embedded blocks are all contracts or exact insertion points — the `main.tsx` route branch (placement is semantic), `PropMeta`/`ControlSpec`/`TangoRoute` types (cross-task contracts), the control-mapping rule set (the decision itself), and the RuleTester valid/invalid cases (the boundary contract, where the exact import shapes are the point). Implementation bodies (parser, generator, rule logic, router) are described in prose because the signature plus stated guarantees determine them.
