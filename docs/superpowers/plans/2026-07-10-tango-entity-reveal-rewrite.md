# Tango Entity Reveal Rewrite Implementation Plan
...
**Goal:** Replace every Tango entity hover, focus, and press reveal with one
root-owned coordinator and strict self-revealing entity components.

**Architecture:** One coordinator per Tango application root owns the input
...
- [Approved design](../specs/2026-07-10-tango-entity-reveal-rewrite-design.md)
- [Normative interaction contract](../../tango/entity-reveal-interactions.md)
- [Browser QA tooling](../../quest_prototype/qa_tooling.md)
- [Registered QA scenes](../../quest_prototype/qa_scenes.md)

## Global constraints

- Install exactly one reveal coordinator per mounted Tango application root.
- Keep reveal models, source bindings, geometry, and overlay rendering under a
  Tango-internal import boundary. Product screens cannot import them.
...
- Existing Tango consumers include production quest screens, battle, Atlas,
  HUD, draft, deck and shop surfaces, docs, mockups, debug tools, and
  transitional screens that directly render Tango components.
...
- `src/tango/internal/reveal/model.ts`: semantic reveal models and UUID-backed
  source identity.
- `src/tango/internal/reveal/state-machine.ts`: pure coordinator state and event
  transitions.
- `src/tango/internal/reveal/geometry.ts`: immutable geometry inputs, candidate
  generation, scoring, and secondary-prefix fitting.
- `src/tango/internal/reveal/viewport.ts`: visual-viewport and safe-area
  snapshots.
- `src/tango/internal/reveal/feedback.ts`: size-aware press and hover scale
  calculations.
- `src/tango/internal/reveal/logging.ts`: typed open and close diagnostic
  payloads.
- `src/tango/internal/reveal/context.tsx`: root coordinator, registration, and
  private source binding.
- `src/tango/internal/reveal/RevealOverlay.tsx`: measurement and the one
  pointer-transparent portal.
- `src/tango/internal/reveal/render-reveal-card.tsx`: exhaustive rendering of
  `GameCard` and strict `InfoCardModel` variants.
- `src/tango/TangoRoot.tsx`: the public application-root shell; it exposes no
  reveal controls.
- `src/tango/internal/reveal/test-utils.tsx`: deterministic clocks, viewport
  fixtures, and a Tango-root render helper for component tests.
...
- Create: `src/tango/internal/reveal/model.ts`
- Create: `src/tango/internal/reveal/state-machine.ts`
- Create: `src/tango/internal/reveal/state-machine.test.ts`
- Create: `src/tango/internal/reveal/logging.ts`
- Create: `src/tango/internal/reveal/logging.test.ts`
- Create: `src/tango/internal/reveal/context.tsx`
- Create: `src/tango/internal/reveal/context.test.tsx`
- Create: `src/tango/internal/reveal/test-utils.tsx`
- Create: `src/tango/TangoRoot.tsx`
- Modify: `src/main.tsx`

**Interfaces:**

- Produces `RevealSpec`, `RevealSourceIdentity`, `RevealCoordinatorState`,
  `RevealCoordinatorEvent`, `reduceRevealState`, `TangoRoot`, and the private
...
- [ ] **Step 2: Run the focused tests and confirm the missing-module failure.**
  Run `npx vitest run src/tango/internal/reveal/state-machine.test.ts`. Expected:
...
- [ ] **Step 6: Implement typed diagnostic builders around `logEvent`.** Use
  stable event names `tango_entity_reveal_opened` and
  `tango_entity_reveal_closed`. The builders accept the same immutable geometry
  snapshot used for placement; do not re-read the DOM when logging.

- [ ] **Step 7: Write coordinator-root tests.** Prove one `TangoRoot` provides a
...
- [ ] **Step 8: Implement `TangoRoot`, coordinator context, and private source
  registration.** `TangoRoot` accepts children only. `useRevealSource` remains
...
- [ ] **Step 9: Install the root once for every application entry.** Wrap the
  `renderStrict` contents in `src/main.tsx` with `TangoRoot`, which covers the
  quest app, battle, docs, mockups, editors, and debug entries that render Tango
  entities. Keep `TangoApp`'s `.tango` styling root inside that shell. Update
...
- [ ] **Step 11: Commit and push.** Commit the internal models, state machine,
  ...
  `feat(tango): add entity reveal coordinator foundation`, then immediately run
  `git push`.
...
- Create: `src/tango/internal/reveal/geometry.ts`
- Create: `src/tango/internal/reveal/geometry.test.ts`
- Create: `src/tango/internal/reveal/viewport.ts`
- Create: `src/tango/internal/reveal/viewport.test.ts`
- Create: `src/tango/internal/reveal/feedback.ts`
- Create: `src/tango/internal/reveal/feedback.test.ts`
- Create: `src/tango/internal/reveal/RevealOverlay.tsx`
- Create: `src/tango/internal/reveal/RevealOverlay.test.tsx`
- Create: `src/tango/internal/reveal/render-reveal-card.tsx`
- Modify: `src/tango/internal/reveal/context.tsx`
- Modify: `src/tango/internal/reveal/logging.ts`
- Modify: `src/tango/primitives/Pressable.tsx`
- Modify: `src/tango/primitives/Pressable.test.tsx`
- Modify: `src/tango/primitives/tango-base.css`
- Modify: `src/tango/primitives/tango-tokens.css`
...
- [ ] **Step 3: Run the geometry tests and confirm missing exports.** Run
  `npx vitest run src/tango/internal/reveal/geometry.test.ts src/tango/internal/reveal/viewport.test.ts`.
...
- [ ] **Step 10: Run task verification.** Run all new reveal tests and
  `src/tango/primitives/Pressable.test.tsx`, then `npm run lint`,
...
- [ ] **Step 11: Commit and push.** Commit with message
  `feat(tango): add shared reveal geometry and overlay`, then immediately run
  `git push`.
...
- Create: `src/tango/components/card/GameCardReveal.test.tsx`
- Create: `src/battle/components/BattleGameCard.tsx`
- Create: `src/battle/components/BattleGameCard.test.tsx`
- Modify: `src/tango/components/card/CardView.tsx`
- Modify: `src/tango/components/card/CardView.hover-zoom.test.tsx`
- Modify: `src/tango/components/card/CardGalleryPanel.tsx`
- Modify: `src/tango/components/card/CardGalleryPanel.test.tsx`
- Modify: `src/tango/screens/DraftScreen.tsx`
- Modify: `src/tango/screens/DesktopDeckViewer.tsx`
- Modify: `src/tango/screens/MobileDeckViewer.tsx`
- Modify: `src/tango/screens/MobileDeckViewer.test.tsx`
- Modify: `src/tango/screens/StartingDeckOverlay.tsx`
- Modify: `src/tango/screens/StartingDeckOverlay.test.tsx`
- Modify: `src/tango/screens/CardShopSiteScreen.tsx`
- Modify: `src/tango/screens/CardShopSiteScreen.test.tsx`
- Modify: `src/tango/screens/PurgeSiteScreen.tsx`
- Modify: `src/tango/screens/PurgeSiteScreen.test.tsx`
- Modify: `src/screens/tango_adapters/mobile-deck-view-model.ts`
- Modify: `src/screens/tango_adapters/desktop-deck-view-model.ts`
- Modify: `src/screens/tango_adapters/starting-deck-view-model.ts`
- Modify: `src/screens/tango_adapters/draft-view-model.ts`
- Modify: `src/screens/tango_adapters/card-shop-view-model.ts`
- Modify: `src/screens/tango_adapters/purge-view-model.ts`
...
- Modify: `src/tango/docs/demos/game-card.tsx`
- Modify: `src/tango/docs/mockups/game-card.tsx`
- Modify: `src/debug/SignatureDecksApp.tsx`
- Delete after migration: `src/tango/components/card/MobileCardPeek.tsx`
- Delete after migration: `src/tango/components/card/MobileCardPeek.test.tsx`
- Delete after migration: `src/tango/components/card/mobile-card-peek-geometry.ts`
- Delete after migration: `src/tango/components/card/mobile-card-peek-geometry.test.ts`
- Delete after migration: `src/tango/components/card/useCardTermPopover.tsx`
...
- [ ] **Step 2: Run the focused GameCard tests and preserve the expected
  failure.** Run
  `npx vitest run src/tango/components/card/GameCardReveal.test.tsx src/tango/components/card/CardView.hover-zoom.test.tsx`.
...
- [ ] **Step 7: Migrate docs, mockups, debug, and transitional direct consumers.**
  ...
  inspect every result. Convert every Tango entity consumer to the strict model;
...
- [ ] **Step 10: Commit and push.** Commit with message
  `refactor(tango): route every game card through reveal coordinator`, then
...
- Create: `src/tango/components/card/GlossaryTerm.tsx`
- Create: `src/tango/components/card/GlossaryTerm.test.tsx`
- Modify: `src/tango/components/overlay/InfoCard.tsx`
- Modify: `src/tango/components/overlay/InfoCard.test.ts`
- Modify: `src/tango/components/hud/DreamcallerPortrait.tsx`
- Modify: `src/tango/components/hud/DreamcallerPortrait.test.tsx`
- Modify: `src/tango/components/hud/Dreamsign.tsx`
- Modify: `src/tango/components/hud/Dreamsign.test.tsx`
- Modify: `src/tango/components/hud/TideDisc.tsx`
- Modify: `src/tango/components/hud/ResourceChip.tsx`
- Modify: `src/tango/components/hud/EssenceValue.tsx`
- Modify: `src/tango/components/hud/QuestStatusBar.tsx`
- Modify: `src/tango/components/card/CardStatOrb.tsx`
- Modify: `src/tango/components/controls/PipBadge.tsx`
- Modify: `src/tango/components/controls/PipBadge.test.tsx`
- Modify: `src/tango/components/dreamscape/SiteNode.tsx`
- Modify: `src/tango/screens/quest-start-shared.tsx`
- Modify: `src/tango/screens/DesktopDeckViewer.tsx`
- Modify: `src/tango/docs/demos/info-card.tsx`
- Modify: `src/tango/docs/demos/tide-disc.tsx`
- Modify: `src/tango/docs/mockups/info-card.tsx`
- Modify: `src/tango/docs/mockups/rules-text.tsx`
...
- [ ] **Step 3: Run focused tests and confirm current wrapper failures.** Run
  `npx vitest run src/tango/components/card/GlossaryTerm.test.tsx src/tango/components/hud/DreamcallerPortrait.test.tsx src/tango/components/hud/Dreamsign.test.tsx src/tango/components/controls/PipBadge.test.tsx`.
...
- [ ] **Step 5: Replace screen-owned wrappers with named components.** Migrate
  ...
  shape remains, create a narrowly named Tango component instead of a generic
  reveal wrapper.

- [ ] **Step 6: Inventory all auxiliary Tango consumers.** Run
  ...
  actual Tango entity source; do not broaden the project into unrelated legacy
  popovers that render no Tango entity.
...
- [ ] **Step 9: Commit and push.** Commit with message
  `refactor(tango): make info entities own reveal semantics`, then immediately
  run `git push`.
...
- Modify: `src/tango/components/atlas/AtlasNode.tsx`
- Create: `src/tango/components/atlas/AtlasNode.test.tsx`
- Modify: `src/tango/components/atlas/AtlasMap.tsx`
- Modify: `src/tango/components/atlas/AtlasHoverCard.tsx`
- Modify: `src/tango/components/atlas/AtlasHoverCard.test.ts`
- Modify: `src/tango/components/atlas/atlas-display.ts`
- Modify: `src/tango/components/atlas/atlas-preflight.ts`
- Modify: `src/tango/components/atlas/atlas.css`
- Modify: `src/screens/tango_adapters/atlas-view-model.ts`
- Modify: `src/screens/tango_adapters/atlas-view-model.test.ts`
- Modify: `src/tango/screens/AtlasScreen.tsx`
- Modify: `src/tango/screens/AtlasScreen.test.tsx`
- Modify: `src/tango/docs/__atlas-fixtures__.tsx`
- Modify: `src/tango/docs/demos/atlas-node.tsx`
- Modify: `src/tango/docs/demos/atlas-map.tsx`
- Delete after migration: `src/tango/components/atlas/AtlasNodeReveal.tsx`
...
- [ ] **Step 3: Run focused Atlas tests and confirm failures.** Run
  `npx vitest run src/tango/components/atlas/AtlasNode.test.tsx src/tango/screens/AtlasScreen.test.tsx src/screens/tango_adapters/atlas-view-model.test.ts`.
...
- [ ] **Step 7: Delete AtlasNodeReveal and resolve all imports.** Run
  `rg 'AtlasNodeReveal|stageRef.*reveal|AtlasNodeCard|AtlasNodeRevealItem' src/tango src/screens/tango_adapters`.
...
- [ ] **Step 9: Commit and push.** Commit with message
  `refactor(tango): make atlas nodes own entity reveals`, then immediately run
  `git push`.
...
- Modify: `src/tango/components/overlay/InfoCard.tsx`
- Modify: `src/tango/components/overlay/InfoCard.test.ts`
- Modify: `src/tango/internal/reveal/context.test.tsx`
- Create: `src/tango/screens/devtools/EntityRevealConformanceDemo.tsx`
- Create: `src/tango/screens/devtools/EntityRevealConformanceDemo.test.tsx`
- Modify: `src/main.tsx`
- Modify: `docs/quest_prototype/qa_scenes.md`
- Modify: `docs/tango/entity-reveal-interactions.md`
- Modify generated: `src/tango/metadata/tango-metadata.json`
- Modify generated: `.llms/skills/tango/SKILL.md`
- Modify generated: `.llms/skills/tango/components/game-card.md`
- Modify generated: `.llms/skills/tango/components/info-card.md`
- Modify generated: `.llms/skills/tango/components/atlas-map.md`
- Modify generated: `.llms/skills/tango/components/atlas-node.md`
- Modify generated: `.llms/skills/tango/components/card-stat-orb.md`
- Modify generated: `.llms/skills/tango/components/dreamcaller-portrait.md`
- Modify generated: `.llms/skills/tango/components/dreamsign.md`
- Modify generated: `.llms/skills/tango/components/pip-badge.md`
- Modify generated: `.llms/skills/tango/components/quest-status-bar.md`
- Modify generated: `.llms/skills/tango/components/resource-chip.md`
- Modify generated: `.llms/skills/tango/components/site-node.md`
- Modify generated: `.llms/skills/tango/components/tide-disc.md`
- Delete generated: `.llms/skills/tango/components/hover-popover.md`
- Delete: `src/tango/components/overlay/HoverPopover.tsx`
- Delete: `src/tango/components/overlay/hover-popover-placement.ts`
- Delete: `src/tango/components/overlay/hover-popover-placement.test.ts`
- Delete: `src/tango/docs/demos/hover-popover.tsx`

**Interfaces:**

- Consumes the fully migrated coordinator and named components from Tasks 1–5.
- Produces the final public vocabulary: `TangoRoot`, visual `InfoCard` variants,
...
- [ ] **Step 1: Write lint-rule tests before the rule.** Reject product imports
  from `src/tango/internal/reveal`, `InfoCard.PressInfo`,
  ...
  named Tango components and internal implementation/tests.
...
- [ ] **Step 3: Implement and enable the repository boundary.** Register the
  rule in `eslint.config.js` for Tango, adapters, battle, docs, debug, editor,
...
- [ ] **Step 11: Spot-check every migrated entity type in its normal workflow.**
  ...
  where available. Use `/tango` docs or the conformance demo for auxiliary
...
- [ ] **Step 13: Commit and push.** Commit enforcement, cleanup, regenerated
  ...
  `refactor(tango): enforce unified entity reveal system`, then immediately run
  `git push`.
...
- Every named Tango entity derives its own primary and ordered secondaries.
