...
- **Persistence + invariants:** all writes go through `writeRoomTransaction` (`src/state/multiplayer-quest-context.tsx:169`), which runs `validateRoomTransitionInvariants` (forbids nulling `dreamAvatar`/`resolvedPackage`/`draftState`, per `src/state/quest-state-invariants.ts`). The editor never exposes clearing those fields.
