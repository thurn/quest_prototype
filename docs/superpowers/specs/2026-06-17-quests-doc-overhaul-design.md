# Quests Doc Overhaul → Prototype Implementation Spec

**Date:** 2026-06-17
**Source of truth:** `docs/quests/quests.md` (rewritten 2026-06-17)
**Mode:** Full doc in one pass. Subagent execution in a dedicated git worktree.

This spec adapts the rewritten quests design doc into the existing React/Vite
quest prototype. It is *not* a greenfield build: most sites, the draft engine,
...
integration, save/load (`saved-quests/`), logging (`src/logging.ts`), screen
...
- Types: `src/types/quest.ts`, `src/types/content.ts`
...
- State: `src/state/quest-context.tsx`, `src/state/quest-state-actions.ts`,
  `src/state/saved-quests.ts`
- Routing: `src/components/ScreenRouter.tsx`
...
- Extend `src/types/quest.ts` and `src/types/content.ts` with all new shared
...
- Integration tests against the real quest state-action / QuestView interface
  covering: quest start → atlas → dreamscape → each site → battle → reward →
  next layer → final boss → victory/defeat. Match battle-mode integration
  philosophy; no unit tests.
- Full manual QA playthrough of a complete quest in both orientations; inspect
...
To reach quest screens for QA, use the existing debug affordances: the Create
Game "Load Quest" dropdown / `?loadQuest=...`, the HUD "Load Quest" menu, and the
...
(`logEvent`) and land in `logs/quest-log.jsonl`.

## 9. Testing philosophy

Integration tests against the real QuestView / quest state-action interface,
...
- Final-boss unique abilities/cards beyond reusing the existing battle path
  (see `docs/quests/bosses.md` for future work).
- Meta-progression unlocks (`docs/quests/meta_progression.md`).
