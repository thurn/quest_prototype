# Journey Prototype V2 Overhaul — Design
...
**runtime journey path** do the same, replacing the tide-based package machinery
it uses today. Dreamsign pool *construction* is explicitly out of scope.

## Background: current data flow (what changes)

| Concern | Today (runtime journey play) | After this overhaul |
|---|---|---|
...
their generation stay in place for non-journey consumers (e.g. the card editor).
...
are pure libraries, not tests. Runtime journey code must not import from a `_test`
directory. Move:
...
## Section 3 — Journey content loading (`src/data/journey-content.ts`)

`loadJourneyContent` changes its inputs and drops tide-package resolution:
...
- **Decklist corpus**: `loadDecklists()` (`/decklists-data.json`, now sourced
  ...
  card-name → card-number index (`buildNameIndex`). Store both on `JourneyContent`
  for pool generation at journey start.
...
replaced by data produced at journey start (Section 4). Tide fields
(`mandatoryTides`, `optionalTides`, `selectedTides`, `optionalSubset`) are
removed from the runtime Dream Avatar/package types and from journey-start logging.

## Section 4 — Pool generation at journey start (`src/state/journey-context.tsx`)

When the player selects a Dream Avatar, `startJourney` generates the pool instead of
reading a precomputed package:

1. Seed an RNG from the journey seed + Dream Avatar id (deterministic per run).
...
chosen deck's card names (e.g. `starterDeck: string[]`). At journey start, resolve
those names to V2 card numbers and store them on journey state as the run's
**starter decklist** (used by specialty shops, Section 5).

Journey-start logging drops the removed tide fields and logs the chosen
...
- `ensureDreamsignOfferRuntime` (`journey-context.tsx`): drop `requiredTides`.
...
- **Unit**: journey-content V2 loading; pool generation at journey start
