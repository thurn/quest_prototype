# Quest State Debug Editor — Design

**Date:** 2026-06-24
**Status:** Approved, ready for implementation planning
**Worktree:** `.worktrees/quest-debug-editor` (branch `wt/quest-debug-editor`)

## Goal

A debug-mode editor that lets a developer freely reconfigure the live state of a
quest run during play. It is surfaced from the "..." utility menu at the bottom
right of the quest screen (the HUD), opens as a full-screen overlay, and edits
the live multiplayer quest state in place.
...
- **Persistence:** quest state lives in a Firebase RTDB room. All writes go
...
- **Deck-entry resolution (the chokepoint):** a `DeckEntry` is turned into an
  ...
  - `normalizePlayerDeckCard` in `src/battle/integration/create-battle-init.ts`
    (battle path), and
  - `effectiveCardFor` in `src/journey_v2/archetypes/improve.ts` (quest UI path).
...
- **HUD "..." menu:** `src/components/HUD.tsx` (lines ~390-495); nested menu with
  items like Pool Viewer, Package Debug, Save/Load Quest, Download Log. New items
...
```ts
// src/types/quest.ts — DeckEntry
statOverride?: { energyCost?: number; spark?: number };
```

Mirror the field on `BattleQuestDeckEntry` and `BattleDeckCardDefinition`
...
"Edit Quest State (debug)" entry in the HUD "..." menu. Open/close state is
...
Per AGENTS.md, every edit emits a `quest_debug_edit` event to
`logs/quest-log.jsonl` capturing `{ gameId, target, field, before, after }` so a
...
- `statOverride` flows through `normalizePlayerDeckCard` into the battle deck
  definition.
- Deck add / remove and `setDeckEntryOverrides` mutate quest state correctly and
  preserve invariants.
