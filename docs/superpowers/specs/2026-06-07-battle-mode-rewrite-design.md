...
- **The AI is coupled to the old model** and plays the 10 "Starter" cards
  (`cards.toml` numbers 510–519) through hand-written per-card models whose code
...
2. **AI (`?ai=1`):** adapt the existing AI architecture to the new model; the
   ...
   `cards.toml`, deliberately simple), with per-card models refreshed to current
...
(generated by the card-walk subagent over all 519 cards in `cards.toml` plus the
32 `dream_avatars.toml` identities). It is the coverage map for §4.4 and the
