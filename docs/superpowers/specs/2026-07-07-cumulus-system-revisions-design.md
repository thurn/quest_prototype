# Cumulus System Revisions — Design

**Date:** 2026-07-07
**Sources:** `docs/postmortems/2026-07-06-cumulus-system-audit.md` (original
...
3. **StartingDeckModal migrates into the Cumulus tier** as a proper
...
- **`src/cumulus/internal/`** — move `glass-surface.ts` and
...
- **Integrity trio** (unit tests, cross-file):
  - *Duplicate-literal detector* — an identical visual literal (color,
    ...
    more than one file under `src/cumulus/` fails, printing both sites.
  - *`no-orphan-tokens`* — every semantic token in `cumulus-tokens.css` has
    ...
    `--display-cutout-right` lesson). `src/cumulus/docs/` and tests are
...
- **New ESLint rules:** `no-raw-safe-area-env` (raw
  `env(safe-area-inset-*)` legal only in `cumulus-tokens.css`; baseline:
...
- **Legacy-tier ratchet** (integrity test): legacy files importing
  `cumulus/internal` pinned to the shrinking baseline; new files under
  `src/components/` flagged with a message pointing at the Cumulus tier.
- **Generated adoption counts** in `npm run cumulus-docs`: each reference
  page and the index print real-consumer counts (imports outside
  `src/cumulus/docs/` and tests).
...
- **Docs:** the **Materials** page (liquid glass — who wears it, the
  ...
  cumulus token docs (the one sanctioned mechanism, the `env()` ban, floors
...
- **`GlassDialog` adoption:** both deck viewers and the starting-deck
  ...
  modules are deleted.
- **StartingDeckModal → Cumulus tier:** rebuilt as a Cumulus overlay screen +
  adapter under `src/screens/cumulus_adapters/`, consuming `GlassDialog` +
  `IconButton`; the legacy file is deleted and the `cumulus/internal`
...
- **Document the new behavior:** InfoCard's mobile scale and above-only
  ...
  pick counter idiom) in the cumulus skill; `DeviceFrameDemo` and
...
- **`valid-token-references` ownership extension** — Phase 1, with the
  `--cv-textbox-blur` move into Cumulus (its only known violation).
...
- `EssenceValue` → `ResourceChip` call-site migrations in legacy screens
  (happens as those screens Cumulus-ify; the spec only lands the shared
