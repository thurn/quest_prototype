# Dream Avatar Selection — mobile swipe carousel (Cumulus)

**Date:** 2026-07-04
**Status:** Approved design, ready for implementation plan
**Source design:** `claude.ai/design` project `fa1281a7-9765-4ae4-832d-6c5bd86d9f0e`,
file `01-dream-avatar-selection.html` (imported via the design connector).

## Goal

Replace the current Cumulus Dream Avatar-selection screen (a static row of pickable
...
A horizontally swipeable carousel of the three offered Dream Avatars. Each page:

- A **full-bleed cinematic portrait** filling the viewport.
- Drifting **motes** over the portrait of the active page only.
- A screen header eyebrow — **"Choose Your Avatar"** — pinned at the top,
...
- A screen-local full-bleed portrait component (`DreamAvatarPortrait` is **not**
  modified).
...
| Full-bleed portrait | screen-local component (reuses `dreamAvatarImageSrc`) |
...
**`data-*` hooks**, keyed by Dream Avatar id, preserved / added for tests and QA:

- Per-page container: `data-dream-avatar-page={dreamAvatarId}`.
- Choose button: `data-choose-dream-avatar={dreamAvatarId}`.
- Essence value: `data-starting-essence-value={dreamAvatarId}`.
...
### A. Full-bleed portrait — a screen-local component (`DreamAvatarPortrait` untouched)
...
- **Reuses** the already-exported `dreamAvatarImageSrc(imageNumber)` helper from
  `DreamAvatarPortrait` for URL resolution (import only — no change to that
...
- **Model in, not markup:** takes a structured list of tides (each: stable
  ...
  the same data the screen already has from `DreamAvatarTideView`. It renders
...
**The one piece of NEW wiring** — the Dream Avatar ability reveal (this screen):
...
is `<CardTermDefinitions text={dreamAvatar.renderedText} />`. There is no object
