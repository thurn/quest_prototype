# Tango Entity Reveal Rewrite

## Context

The complete behavioral contract for this work is [Entity Reveal
Interactions](../../tango/entity-reveal-interactions.md). That document defines
...
render Tango entities. Unrelated legacy UI remains outside this project.

## Architecture and ownership

Each mounted Tango application root installs one `RevealCoordinator`. The
production application, component documentation, mockups, tests, and standalone
debug applications install it through their Tango root shell. Tests reject
nested or duplicate coordinators within a root. This preserves one active reveal
per visible Tango application without coupling independent React roots through a
...
- A private source adapter lets named Tango components register semantic intent
...
Screens render named semantic Tango components and supply domain data,
...
1. **Establish the internal coordinator foundation.** Add semantic reveal
   models, the private registration adapter, one host per Tango root, the
...
3. **Converge the GameCard ecosystem.** Make UUID-backed GameCard own desktop
   ...
   deck, shop, battle, docs, mockups, debug, and transitional Tango consumers.
4. **Converge named InfoCard entities.** Move Dreamcaller, Dreamsign, tide,
   ...
   strict semantic components and migrate all of their Tango consumers.
...
6. **Prove final conformance.** Delete compatibility surfaces and superseded
   ...
   representative browser matrix, inspect diagnostics, and update Tango
