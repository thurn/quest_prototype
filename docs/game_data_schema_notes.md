...
The ability variant determines the compatibility family, legacy runtime id,
selection policy, and available quantity fields. The `augury_v1` adapter lowers
the catalog to generated `data/augury.toml`; runtime code consumes the validated
`public/augury-data.json` and `src/generated/config/augury-data.json` projections.
The adapter also publishes the four registered inactive compatibility abilities
with fixed disabled definitions so the runtime registry remains exhaustive.
...
target limits, and Dreamsign chooser bounds are validated by their typed ability
variants. Presentation text accepts only the registered runtime placeholders.
