...
catalog to generated `data/augury_site.toml`; runtime code consumes the validated
...
Journey-wide limits are authored in `data/journey.ron`; shop configuration is
authored in `data/shop_site.ron`; shared rewards and Purge pricing are authored
in `data/sites.ron`; and battle rewards are authored in `data/battle.ron`.
Their typed adapters lower each source to its matching compatibility TOML. The
field anchors below describe the assembled browser economy contract.
