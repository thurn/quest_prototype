...
under `src/`, with emphasis on effects that append coop events, invoke quest
...
`logs/quest-log.jsonl` is a local production-game debugging artifact and is not
...
`src/state/coop-quest-context.tsx:363` to `SET_CARD_SOURCE_DEBUG`. The reducer in
`src/rules/quest/shop.ts:656` applies every valid show and clear, including
same-value updates.
...
`logs/quest-log.jsonl:35540`, `:35543`, `:35545`, and `:35547` show four applied
...
The normal single-client sequence appears in `logs/quest-log.jsonl:44507`
...
`src/rules/quest/sites.ts:759`. `SET_SCREEN` always applies in
`src/rules/quest/lifecycle.ts:214`, even when the requested screen is already
current. `logs/quest-log.jsonl:44204` and `:44205` demonstrate an applied
...
already-accepted guard in `src/rules/quest/sites.ts:428`.

The current log contains a single-client acceptance at
`logs/quest-log.jsonl:41733`.
...
Duplicate `OPEN_SITE` returns the same quest at
`src/rules/quest/sites.ts:310`. Duplicate `ENTER_DRAFT_SITE` returns the same
quest at `src/rules/quest/draft.ts:221`. `questCase` in
...
pairs, including `logs/quest-log.jsonl:44192` and `:44195`, and six adjacent
`OPEN_SITE` pairs, including `logs/quest-log.jsonl:44246` and `:44248`.
...
- The `?goto=` bootstrap in `src/App.tsx:123` combines a replay-surviving ref
  with the durable `dreamcaller` check.
- The asynchronous `?loadQuest=` path has no cancellation, but the codebase has
...
```text
Fix every confirmed issue in docs/quest_prototype/react_effect_coop_audit.md.
...
Because these changes affect quest UI lifecycle and coop behavior, also perform
