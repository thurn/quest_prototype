...
| **GroupPanel** | `components/` | A flat, solid deep-plum card (`--surface-card` fill, no border, `--shadow-card` drop shadow) — a distinct surface from InfoCard's glass. The liquid-glass recipe (`backdrop-filter` blur/saturate + specular gradient + inset rim/wash + drop shadow) lives in `glassSurfaceStyle` (`src/cumulus/internal/glass-surface.ts`), shared by InfoCard, GlassDialog, IconButton, GlassButton, and SpeechBubble. The deck viewer and full-bleed CardGalleryPanel use the 80%-black `--scrim-gallery` alpha overlay; floating CardGalleryPanels use glass. |
