...
sites across a quest. Each encounter is built from a structured *read* of the
current deck and quest state, voiced in procedurally generated dialogue, and
...
4. **He is recurring and remembers.** A small `MerchantState` persists in
   `QuestState`: encounter count, mood, past deals, and an anti-repetition
...
quest mutations in `src/state/quest-context.tsx`. The Reckoner's offers come
...
- **Deck** — `QuestState.deck: DeckEntry[]`. Each `DeckEntry` is a card instance:
  `{ entryId, cardNumber, transfiguration, typeChange?, keywordModification?, isBane }`
  (`src/types/quest.ts:53`). `cardNumber` resolves to `CardData`
...
function** of `(quest.seed, siteId, MerchantState)`. The `FitModel` is
...
must be registered in both `createDefaultState()` (`src/state/quest-context.tsx`)
and `normalizeQuestState()` (`src/multiplayer/room-service.ts`), because RTDB
...
- **`FitModel`** (`src/draft/replay/fit-model.ts`): exposes `prior:
  ...
  (`src/data/cards-v2-database.ts:87`) and threaded into the read via quest
  content. (Today the model is built only when a draft mode uses it,
  `src/data/quest-content.ts:425`; the Reckoner needs it unconditionally, so the
...
shared `EffectMutations` from `src/effects/` (which wrap the real quest
...
(`src/journeys/adapter/`), backed by `src/state/quest-context.tsx`:

```ts
interface EffectMutations {
  transfigureCard(entryId, type: TransfigurationType, effectDescription: string,
                  effectDetails: Record<string, unknown>): void;  // quest-context.tsx:1868
  changeEssence(delta: number, source: string): void;            // quest-context.tsx:704
...
mood registers, the grammar can stay fresh across a quest without a model.
...
- `src/effects/mutations.ts` — the `EffectMutations` interface + the adapter to
  `QuestMutations` (today's `journeyMutations`).
...
- Add `SiteType` `"Merchant"` in `src/types/quest.ts` (and a
...
- `MerchantState` registered in `createDefaultState()` and
  `normalizeQuestState()` with a `room-service.test.ts` round-trip case (§4).
- The Reckoner mutation path writes via `multiplayer-quest-context.tsx`: the
...
content bundle (`src/data/quest-content.ts`), cached and threaded into
...
5. **Dialogue grammar** behind `renderEncounterDialogue`, sized for low
   repetition across a quest; mood registers; memory callbacks.
