# Dreamcaller ↔ Dreamscape mapping

Every non-starter dreamscape in `data/tabula/dreamscapes.toml` lists the 3–4
Dreamcallers that call it home, under a `dreamcaller-ids` field. Together these
lists **partition** `data/tabula/dreamcallers_v2.toml`: each of the 32
Dreamcallers belongs to exactly one dreamscape, and no Dreamcaller is shared.
...
discard, …). A Dreamcaller's `rendered-text` describes what it wants to do, so
the mapping is built in two passes:

1. **Affiliation-driven (mechanical).** Where a Dreamcaller's ability names a
...
2. **Closest thematic fit (flavor).** Dreamcallers whose abilities are
...
32 Dreamcallers across 10 non-starter regions means **eight regions of 3 and two
...
| Dreamscape | Affiliation | Dreamcallers |
| --- | --- | --- |
...
(`validateDreamcallerMapping`). The structural checks — the same Dreamcaller
...
referential checks against the Dreamcaller set are **warnings**: a
`dreamcaller-id` that resolves to no Dreamcaller, and a Dreamcaller assigned to
no dreamscape. They warn rather than abort because the build may run against a
reduced Dreamcaller fixture (the asset tests swap one in). In a full production
build both files are real, so a stray id surfaces as paired warnings — the bad
id is "unknown" and the orphaned Dreamcaller is "unassigned". The per-region
