...
Set `QUEST_REVIEW_BASE=<revision>` to compare against a different revision.
...
`node_modules/.cache/quest-review/`. The cache is scoped to the worktree and is
...
- Set `QUEST_TEST_WORKERS=<count>` for an intentional local override.
- CI sets `QUEST_TEST_WORKERS=4`.
- Tests receive a 15-second ceiling so a loaded comprehensive run does not
  ...
  `QUEST_TEST_TIMEOUT_MS=<milliseconds>` for an intentional override.
- ESLint uses two typed workers by default. Set `QUEST_ESLINT_WORKERS=1` for a
