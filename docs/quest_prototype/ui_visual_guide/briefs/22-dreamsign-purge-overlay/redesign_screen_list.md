...
3. **Persistent HUD (bottom bar)** — always-on run state (essence, deck size, Dreamcaller, dreamsigns, progress) + global controls; surrounds every meta screen.
4. **Dreamcaller HUD popover** — the chip's portrait + name + title + full ability text.
5. **Dreamsign hover card** — the larger art + name + full effect-text card raised from a dreamsign thumbnail (shared with battle).
6. **Dream Atlas** — the between-dreamscapes world map; the 7-layer branching graph and the run's primary navigation hub.
7. **Dreamscape Atlas hover preview card** — the large floating dreamscape/boss/unrevealed preview (guide, site, bonus, affiliation, known dreamsign); the heart of Atlas decision-making.
8. **Dreamscape Screen** — inside-a-dreamscape view; the scatter of clickable site nodes over scene art.

### Site screens

9. **Draft** — pick 1 of 4 cards, 5 picks per visit, with the running deck tray.
10. **Card Shop** (Tobias) — buy cards with essence; restock; regular and enhanced forms.
11. **Dreamsign Market** (Amunet) — buy dreamsigns; same shop frame as Card Shop.
12. **Dreamsign Revelation** (Sigrún) — free dreamsign offer (single, or choose 3–4 enhanced).
13. **Dreamsign Reward site** — non-interactive grant of a pre-disclosed known dreamsign; accept/decline.
14. **Essence site** — non-interactive essence count-up grant (pure feedback, no decision).
15. **Transfiguration** (Durgan) — two-phase reforge: pick a card, then pick a form.
16. **Duplication** (Deacon Holt) — single-select-then-confirm; copy one deck card.
17. **Purge** (Master Takeshi) — deck-thinning grid with escalating cost meter and bane handling.
18. **Dream Augury / Merchant** (Aldric) — two deck-tuned offers side-by-side; ~a dozen bespoke reward-object treatments.
19. **Placeholder sites** — Tempting Offer (Maddox), Gamble (Gravok), Temporal Fork ("Layaway"); shared stub screen pending real encounters.

### Reference, overlays & end-of-run

20. **Deck Viewer** — full-screen deck browser (filter / sort / size) with Dreamcaller + dreamsigns sidebar; reused by Purge.
21. **Glossary** — centered modal listing every keyword and its definition.
22. **Dreamsign-purge overlay** — blocking cap-handling modal (drop one of 12 dreamsigns to accept a new one).
23. **Quest Complete (Victory)** — run victory end-state; stats grid + optional final-deck reveal + actions.
24. **Quest Failed (Defeat)** — run defeat/draw end-state; result reason + terminal-battle summary grid.

### Developer surfaces (re-home to a dedicated developer screen)

25. **"⋯" utility menu** *(developer)* — the HUD dropdown (Pool Viewer, Package Debug, Why Cards, Why Journey, Edit Quest State, Save/Load Quest, Download Log).
...
30. **Battle Start (pre-battle intro)** — scout the opposing Dreamcaller (ability, signature cards, dreamsigns, stakes); "Begin Battle".
31. **Battle Board (in-battle)** — the live match: status bar, phase rail, symmetric multi-rank battlefield, side columns, hand tray, action bar. The single hardest screen to fit to portrait.
...
39. **Card Hover Preview** — the pervasive enlarged-card + term-definitions read-this-card affordance.
...
45. **Card Context Menu (right-click)** *(developer)* — per-card debug/manual-play actions and card notes.
