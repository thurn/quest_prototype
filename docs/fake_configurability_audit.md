...
| Economy shop stock | Represent a total closed stock mapping as named fields. | `card_shop`, `specialty_shop`, and `dreamsign_market` own their stock values directly. Compatibility lowering emits the established keyed stock object. |
| Atlas profiles and fixed policies | Keep the seven-layer journey shape as a game rule; make profile names data-driven and encode fixed algorithms as code-owned behavior. | Layers remain One through Seven with starter, standard, and boss roles. Fill profiles are a nonempty string-keyed map, and layers may reference any defined profile. Compatibility lowering emits `allow-repeats`, unique non-Draft fill, and `omit-fill`; the authoring schema exposes the configurable known-Dreamsign site. |
