...
The bake is a pure function of the bundled cards, draft records, and Dream Avatar
...
2. **20 signature tides — one per signatured Dream Avatar.** Resolve the
   Dream Avatar's full signature onto the corpus and grow a 150-copy pool from all
   of those signature cards as the seed set. This is the deterministic centre of
   the pools `sigseed` grows for that Dream Avatar (whose per-run variety is a
...
4. **`tidePoolByDreamAvatar`.** Per Dream Avatar, two lists — **leads** (one drawn
   at random per run, always joined) and **fill** (the broad tail):
   - A **signatured** Dream Avatar has one lead candidate — its own signature tide
     ...
     would pull the pool toward another Dream Avatar's identity and is not used.
   - A **neutral** Dream Avatar has *every signature tide* as a lead candidate, so
     ...
     Dream Avatar. Leading with a whole coherent archetype (rather than a broad,
...
`data/tides3.jsonc` carries both the decklists and the per-Dream Avatar pools in
...
1. **Lead.** Draw one lead tide at random and always join it — a fixed choice for
   a signatured Dream Avatar (one candidate), a different coherent archetype each
...
Measured on the real-draft simulation (every Dream Avatar, full signature) with
`scripts/pool-metrics.mjs` at 100 seeds, alongside `sigseed`:

| Metric | `tides3` | `sigseed` |
| --- | --- | --- |
| Dream Avatar (signature-theme delivery) | 87.7 | 89.2 |
...
```bash
npm run pool-metrics -- --variant tides3 --seeds 200 --metric dreamAvatar
...
On the headline **dream avatar** metric — the question "does each pool deliver the
theme its Dream Avatar was built to play?" — `tides3` (87.7) is by far the closest
