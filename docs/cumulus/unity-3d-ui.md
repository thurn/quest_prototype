# Cumulus 3D User Interface for Unity 6.5

## Summary

This document defines a world-space, mesh-based user interface for Unity 6.5
that carries Cumulus's visual language into a fully three-dimensional scene.
...
The signature Cumulus liquid-glass surface is a hybrid material. Its shape,
...
that must preserve Unity 6.5 Tile-Only Mode use a lit, 72-percent-alpha Cumulus
...
- [Cumulus token source](../../src/cumulus/primitives/cumulus-tokens.css) is the
  authoritative visual vocabulary for the existing implementation.
- [Cumulus entity reveal interactions](entity-reveal-interactions.md) defines
...
Cumulus currently expresses glass through a browser backdrop filter. The
...
recognizably Cumulus through each of those states.
...
The system therefore needs a rendering boundary shared across all Cumulus glass
...
- Render Cumulus panels and controls as world-space meshes with meaningful
...
- Preserve Cumulus's semantic component and token discipline instead of exposing
...
The Unity material catalog mirrors Cumulus's semantic roles:
...
The standard glass target begins with the existing Cumulus values:
...
Shadow color is produced by the receiving scene and lighting model. The Cumulus
...
glass cost only when it renders Cumulus glass.

The default frame ordering maps to Unity 6.5's After Rendering Post Processing
injection point:

1. Shadow casters, including tangible Cumulus geometry, render into applicable
   light shadow maps.
...
6. A Cumulus depth-only pass writes the nearest visible panel silhouette.
7. Cumulus glass, controls, text, glyphs, and focus treatments render against
   that depth with bounded display-space lighting and manual backdrop
   composition.
8. Designated foreground world transparency that must cover Cumulus renders in
...
- Cumulus text, glyphs, motes, and highlights render with their owning Cumulus
...
- Other Cumulus glass uses the on-glass relationship when surfaces overlap.
...
The Cumulus depth pass uses depth writes with a less-than-or-equal test against
retained scene depth. It also records a stable panel-group owner for each Cumulus
...
injection point. Cumulus geometry and content remain outside temporal history and
...
source. Cumulus token colors are authored for that space. The final platform
...
irrelevant to Cumulus geometry and keeps camera motion blur from smearing
...
Cumulus visible pass as a spatial edge treatment.
...
- Deep recursive transmission is not required for Cumulus conformance.

## 3D Component System

### Semantic component ownership

Unity components expose the same strict roles as Cumulus rather than becoming a
...
Unity consumes a generated or validated projection of Cumulus's semantic tokens.
...
- Touch uses direct press behavior and the Cumulus reveal contract.
...
Visual press compresses the tangible object using Cumulus's shared press factor.
...
Cumulus's material-continuity rules apply in world space:
...
- Full Cumulus blur, saturation, tint, sheen, and rim.
...
lights per pixel. At most two additional lights cast shadows on Cumulus geometry.
...
`SceneGlass` and `OnGlass` share a dielectric GGX lighting module. Cumulus's
...
`CumulusGlassLightingProfile` authors both semantic roles; panel instances do not
...
- At 60 frames per second, total Cumulus glass GPU cost targets 2.0 milliseconds
...
stabilization. Cumulus glass targets 3.0 milliseconds or less of GPU time in the
...
This profile is an intentional semantic rendering of Cumulus glass, not a frozen
...
"Total Cumulus glass" is the GPU and CPU delta between that scene and the same
deterministic run with Cumulus visible, depth, shadow-caster, capture, blur, and
interaction work disabled. The delta includes Cumulus's added shadow-map work.
...
minus 1 degree. VSync is disabled. The synchronized Cumulus-enabled and disabled
...
- Number of shadow-casting Cumulus renderers.
...
Every shadow-casting light adds shadow-map work for applicable Cumulus geometry.
...
- Active Cumulus quality profile and the reason it was selected.
...
- Cumulus shadow casters and active shadow resolution tiers.
...
- PSO cache misses involving Cumulus shaders.
...
- Physically based transmission and refraction cannot reproduce Cumulus's broad,
...
- Recursive transmission adds sorting and memory beyond Cumulus's need; nested
...
- Existing Cumulus token roles and component semantics remain authoritative.
...
- Standard and popover glass interiors match Cumulus reference captures for
...
- Glass receives the main scene light and the profile's allowed additional
  lights.
- Tangible Cumulus geometry casts shadows at the intended semantic coverage.
