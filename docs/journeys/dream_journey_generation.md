...
- [Journey Master Design](journeys.md) High-level journey vision, Dream Atlas rules,
...
Dream Journeys are the part of journey mode where Dreamtides can be most
...
- Support every extreme journey-rule rewrite in V1.
...
available. It contains only the journey-state features the generator is allowed to
reason about.
...
- neutral or negative persistent journey changes should usually be represented as
  journey statuses
- positive effects whose main gameplay expression is harming or burdening the
  opponent during battles should also prefer journey statuses over Dreamsigns
...
shape supports mini-journeys, delayed payoff objects, and stored value without
...
4. persist the committed manifest in journey state
...
Read journey state and produce the Run Context Snapshot for this dreamscape. Also
