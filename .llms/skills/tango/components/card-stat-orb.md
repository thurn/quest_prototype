...
| `tintColor` | `TangoColor` = `"accent" \| "accent-bright" \| "accent-strong" \| "essence" \| "energy" \| "energy-bright" \| "spark" \| "points" \| "danger" \| "positive" \| "selected" \| "sale" \| "gold" \| "gold-light" \| "text-primary" \| "text-secondary" \| "text-muted" \| "text-faint" \| "text-on-accent" \| "white"` | no | — | Tint applied to the digit when the stat has been changed by a transfiguration (e.g. a bumped spark or halved energy cost). Only the number picks up the tint; the resource glyph behind it keeps its resource hue so a changed stat still reads as the same kind of resource. |
