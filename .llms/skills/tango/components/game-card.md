...
| `termDefinitions` | `"none" \| "card"` | no | — | Keyword-definition reveal behavior. Default `"card"` means this card owns its keyword InfoCards, positioned beside the card and top-aligned with it. Use `"none"` only when a parent surface renders those supplemental cards beside a larger main preview of this same card. |
