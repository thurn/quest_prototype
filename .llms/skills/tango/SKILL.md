...
| --- | --- | --- | --- | --- |
| Pressable | Primitives | 21 | [components/pressable.md](components/pressable.md) | The one press-feedback primitive. |
...
| Option List Item | Components | 1 | [components/option-list-item.md](components/option-list-item.md) | A compact selectable row for a named effect, its identifying glyph, and an essence price. |
...
| Dreamcaller Portrait | Components | 21 | [components/dreamcaller-portrait.md](components/dreamcaller-portrait.md) | The one way to render a dreamcaller's character art: the transparent full-body cutout standing on a tinted radial backdrop, in one of five fixed framings. |
...
| Rules Text | Components | 17 | [components/rules-text.md](components/rules-text.md) | Renders Dreamtides rules copy from card data — resource pips, ability carets, and glossary keywords styled in place — so ability text reads the same everywhere it appears. |
