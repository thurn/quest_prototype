...
import { validateAnthology } from "./dda-markdown-lib.mjs";
...
function usage() {
  return [
    "Usage: format-markdown.mjs (--write | --check) [--anthology <directory>]",
    "",
    "Formats and validates the flat DDA Markdown corpus.",
  ].join("\n");
}
...
function parseArguments(argumentsList) {
  ...
  return { anthologyDirectory, mode };
}
...
if (options?.help) {
  ...
  let validation = await validateAnthology(options.anthologyDirectory);
  if (validation.files.length === 0) {
    ...
    if (options.mode === "--write") {
      validation = await validateAnthology(options.anthologyDirectory);
    }
    ...
    if (
      ...
      process.stdout.write(
        `DDA Markdown ${options.mode === "--write" ? "formatted" : "checked"}.\n`,
      );
    }
  }
}
