---
name: tango-sweep
description: Recurring health sweep of the Tango design system — ratchet the integrity-check baselines toward zero, hunt the drift the lint suite cannot see (doc drift, doctrine drift, convergence debt, adoption problems), verify every finding against the tree, fix what is mechanical, and file the rest. Designed to run unattended on a schedule. Triggers on tango sweep, design system sweep, design system audit, tango health, design system health, /tango-sweep.
---

# Tango Sweep

The recurring, scaled-down form of the whole-system audit in
[docs/postmortems/2026-07-06-tango-system-audit.md](../../../docs/postmortems/2026-07-06-tango-system-audit.md)
and the fix program it produced
([docs/superpowers/specs/2026-07-07-tango-system-revisions-design.md](../../../docs/superpowers/specs/2026-07-07-tango-system-revisions-design.md)).
...
The write-time rails already exist and hold: the `tango/*` ESLint rules
(`eslint.config.js`) and the cross-file integrity tests
(`scripts/tango-*.test.mjs` — duplicate literals, orphan tokens, ghost
...
- Find the previous sweep report: the newest file in `docs/tango-sweeps/`.
...
- List the range's commits touching the design-system surface:
  `src/tango/`, `src/screens/tango_adapters/`, `src/tango/docs/`,
  `.llms/skills/tango*`, `eslint.config.js`, `scripts/tango-*`,
...
- `BASELINE` arrays in `scripts/tango-duplicate-literals.test.mjs`,
  `scripts/tango-ghost-components.test.mjs`,
  `scripts/tango-orphan-tokens.test.mjs`.
- The known-files list in `scripts/tango-legacy-ratchet.test.mjs`.
- Any per-rule allowlists / `no-restricted-imports` exceptions in
  `eslint.config.js`.
- Components marked `status: "incubating"` in `src/tango/docs/` demo
...
- **Adoption & catalog honesty.** Regenerate docs (`npm run tango-metadata
  && npm run tango-docs`), then read the generated "Real consumers" counts.
  ...
  `src/tango/components/` and `src/tango/primitives/` absent from the docs
  registry); blurbs whose claims a grep contradicts (a blurb saying "every X
  routes through it" is a checkable claim — check it).
- **Doctrine re-grep.** Find doctrine comments in `src/tango/` — "the ONE",
...
- **Token & demo drift.** Near-orphan tokens (one reader — is it load-bearing
  or vestigial?); token/literal mixes inside `src/tango/**/*.css`; demo
...
- Touched a component, demo entry, or the token stylesheet → run
  `npm run tango-metadata && npm run tango-docs` / `npm run tango-tokens`
...
Create `docs/tango-sweeps/YYYY-MM-DD.md` (today's date). It is both the
