...
export const BOOK_SEGMENT_PATTERN = /^[a-z0-9]+(?:_[a-z0-9]+)*$/;
...
function validateBookSegment(value, label) {
  const normalized = validateSingleLine(value, label);
  if (!BOOK_SEGMENT_PATTERN.test(normalized)) {
    throw new Error(
      `${label} must use lowercase words separated by underscores`,
    );
  }
  return normalized;
}
...
export async function buildImagePublication({
  ...
  const normalizedPart = validateBookSegment(part, "part");
  const normalizedChapter = validateBookSegment(chapter, "chapter");
