...
| The card pool | `data/tabula/cards_v2.toml` |
...
Each card in `data/tabula/cards_v2.toml` carries an `mtg-name` field naming the
...
```bash
grep -E '^(name|mtg-name) =' data/tabula/cards_v2.toml | paste - -
```
...
python3 - <<'PY'
import re
toml = open('data/tabula/cards_v2.toml').read()
...
python3 - <<'PY'
import re
names = set(re.findall(r'\nname = "(.*?)"', open('data/tabula/cards_v2.toml').read()))
...
- Every cited Dreamtides card resolves in `cards_v2.toml`.
