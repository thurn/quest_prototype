#!/usr/bin/env python3
"""Select unrepresented cards and create reproducible Exploration design requests."""
...
from mechanic_ideas import MechanicCatalogError, load_mechanic_catalog


SCRIPTS_DIR = Path(__file__).resolve().parent
SKILL_DIR = SCRIPTS_DIR.parent
REPO_ROOT = SCRIPTS_DIR.parents[3]
DEFAULT_CARDS_SOURCE = REPO_ROOT / "data/cards.ron"
DEFAULT_CARDS_COMPAT = REPO_ROOT / "data/cards.toml"
DEFAULT_DREAMSIGNS_SOURCE = REPO_ROOT / "data/dreamsigns.ron"
DEFAULT_DREAMSIGNS_COMPAT = REPO_ROOT / "data/dreamsigns.toml"
DEFAULT_EXPLORATION_SOURCE = REPO_ROOT / "data/exploration_site.ron"
DEFAULT_EXPLORATION_COMPAT = REPO_ROOT / "data/exploration_site.toml"
DEFAULT_TRANSFIGURATION_SOURCE = REPO_ROOT / "data/transfiguration_site.ron"
DEFAULT_TRANSFIGURATION_COMPAT = REPO_ROOT / "data/transfiguration_site.toml"
DEFAULT_MECHANIC_IDEAS = SKILL_DIR / "references/mechanic-ideas.json"
DEFAULT_EXPLORATION_MODEL = REPO_ROOT / "tools/game-data/src/models/exploration.rs"
DEFAULT_EFFECT_SCHEMA = REPO_ROOT / "scripts/exploration-effect-editor-schema.mjs"
...
SOURCE_KEYS = (
    ...
    "exploration_source",
    "exploration_compat",
    "transfiguration_source",
    "transfiguration_compat",
    "mechanic_ideas",
    "exploration_model",
    "effect_schema",
)
...
def canonical_uuid(
    ...
    *,
    ...
    if require_v4 and (parsed.version != 4 or parsed.variant != uuid.RFC_4122):
        raise SelectionError(f"{label} must be an RFC 4122 UUIDv4")
    return normalized
...
def load_cards(path: Path) -> list[dict[str, Any]]:
    try:
        ...
        raise SelectionError(f"Generated card catalog does not exist: {path}") from error
    except tomllib.TOMLDecodeError as error:
        raise SelectionError(f"Generated card catalog is invalid TOML: {path}: {error}") from error
...
def load_live_ids(path: Path) -> set[str]:
    try:
        ...
        raise SelectionError(f"Generated Exploration catalog does not exist: {path}") from error
    except tomllib.TOMLDecodeError as error:
        raise SelectionError(f"Generated Exploration catalog is invalid TOML: {path}: {error}") from error
...
def find_art(image_number: int, images_dir: Path) -> Path:
    ...
    trailing_number = re.compile(rf"(?<!\d){image_number}$")
    matches = sorted(
        path.resolve()
    ...
    if not matches:
        raise SelectionError(
            f"No full-size image ending in {image_number} before its extension exists in {images_dir}"
        )
    if len(matches) > 1:
        joined = "\n  ".join(str(path) for path in matches)
        raise SelectionError(f"Multiple full-size images match {image_number}:\n  {joined}")
    return matches[0]
...
def create_batch(
    *,
    ...
    if set(source_paths) != set(SOURCE_KEYS):
        raise SelectionError(f"source_paths must contain exactly {sorted(SOURCE_KEYS)}")

    resolved_sources = {key: path.resolve() for key, path in source_paths.items()}
    cards = load_cards(resolved_sources["cards_compat"])
    cards_by_id = {card["id"]: card for card in cards}
    live_ids = load_live_ids(resolved_sources["exploration_compat"])
    try:
        load_mechanic_catalog(
            resolved_sources["mechanic_ideas"],
            model_path=resolved_sources["exploration_model"],
        )
    except MechanicCatalogError as error:
        raise SelectionError(str(error)) from error
    source_digests = {key: sha256(path) for key, path in resolved_sources.items()}

    if requested_card_ids:
        normalized_requested = [canonical_uuid(value, "--card-id") for value in requested_card_ids]
        ...
        if represented:
            raise SelectionError(
                f"Card UUID(s) already represented in live Exploration: {', '.join(represented)}"
            )
        ...
        if batch_size > len(eligible):
            raise SelectionError(
                f"Requested {batch_size} cards but only {len(eligible)} are absent from live Exploration"
            )
    ...
    requests_dir.mkdir()
    results_dir.mkdir()
    repository = {key: str(path) for key, path in resolved_sources.items()}
