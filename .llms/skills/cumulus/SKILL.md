...
| Icon Button | Actions & Inputs | 19 | [components/icon-button.md](components/icon-button.md) | The compact glyph-only glass disc, with placement-aware recipes for scene media or an existing glass surface, and made fully round so it reads as one family with the filter/sort controls. |
...
| Rich Text | Text & Guidance | 10 | [components/rich-text.md](components/rich-text.md) | The design system's model for formatted copy. |
...
| Playing Card | Cards | 1 | [components/playing-card.md](components/playing-card.md) | The shared outlined playing-card face, with visible rank-and-suit, conventional face-down, and Four-Suit Reprise variants. |
