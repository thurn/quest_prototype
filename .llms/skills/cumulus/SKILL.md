...
| Card Feature Callout | Components | 1 | [components/card-feature-callout.md](components/card-feature-callout.md) | A compact speech-inspired glass label for teaching one semantic region of a full GameCard, with canonical energy and spark glyph treatments. |
...
| Segmented Control | Components | 7 | [components/segmented-control.md](components/segmented-control.md) | The compact tab and filter switch used for type filters, sort direction, and small mode toggles. |
...
| Radial Announcement | Components | 3 | [components/radial-announcement.md](components/radial-announcement.md) | The orbiting circular status moment for turn handoffs, wins, and failures. |
...
| Inline Glyph | Primitives | 8 | [components/inline-glyph.md](components/inline-glyph.md) | The Boxicons renderer for flowing text: a one-em square whose midpoint follows the surrounding font's capital height at every type size. |
...
| Entity Reference | Components | 2 | [components/entity-reference.md](components/entity-reference.md) | An inline, underlined card or Dreamsign name that reveals the canonical full entity on hover, keyboard focus, or touch hold through the shared coordinator. |
...
| Rules Text | Components | 14 | [components/rules-text.md](components/rules-text.md) | Renders Dreamtides rules copy from card data — resource pips, Unicode trigger markers, and glossary keywords styled in place — with definition cards adapted to the exact rules sentence. |
...
| Card Gallery Panel | Components | 9 | [components/card-gallery-panel.md](components/card-gallery-panel.md) | The shared card-browser surface: GlassPanel title and action chrome around a scrolling GameCard grid, framed as floating glass or a full-bleed alpha scrim. |
