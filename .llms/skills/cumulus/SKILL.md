...
| Icon Button | Components | 11 | [components/icon-button.md](components/icon-button.md) | The compact glyph-only glass disc — neutral or purple accent liquid glass, with placement-aware recipes for scene media or an existing glass surface, and made fully round so it reads as one family with the filter/sort controls. |
...
| Offer Tile | Components | 1 | [components/offer-tile.md](components/offer-tile.md) | The 200×200 rounded symbolic Dream Augury offer button: UUID-backed complete cards, dreamsign art, and operation marks gathered inside an iron fantasy frame without spelling out the complete offer. |
