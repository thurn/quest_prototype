...
| --- | --- | --- | --- | --- |
| Pressable | Primitives | 28 | [components/pressable.md](components/pressable.md) | The one press-feedback primitive. |
...
| Glass Button | Components | 19 | [components/glass-button.md](components/glass-button.md) | The labeled glass action — a bold text label with optional numerical essence cost on the shared liquid-glass surface, with neutral, danger, and purple accent treatments plus placement-aware recipes for media or an existing glass surface. |
...
| Glass Panel | Components | 8 | [components/glass-panel.md](components/glass-panel.md) | The shared liquid-glass content container: an optional structured header, a composed body, and an optional footer on the canonical floating material. |
| Glass Dialog | Components | 10 | [components/glass-dialog.md](components/glass-dialog.md) | The glass overlay shell: a modal dialog with a bounded, centered glass panel on desktop and a full-bleed frosted overlay on mobile, with a hairline-closed header (title, optional subtitle, and an optional glass close disc) over a scrolling body. |
...
| Corner Utility Menu | Components | 1 | [components/corner-utility-menu.md](components/corner-utility-menu.md) | The strict app-chrome command offering: a fixed icon trigger and a hierarchical inward-opening utility menu. |
| Context Action Menu | Components | 1 | [components/context-action-menu.md](components/context-action-menu.md) | The strict pointer/card command offering: a clamped desktop context menu and a responsive dialog sheet with the same typed hierarchy. |
| Speech Bubble | Components | 3 | [components/speech-bubble.md](components/speech-bubble.md) | A guide-dialog bubble for character-led site screens: the same frosted information material as an InfoCard, with a left arrow that points back to the speaker. |
...
| Segmented Control | Components | 4 | [components/segmented-control.md](components/segmented-control.md) | The compact tab and filter switch used for type filters, sort direction, and small mode toggles. |
| Select | Components | 6 | [components/select.md](components/select.md) | The compact dropdown control, and Cumulus's standard mobile filter/sort control: a button that shows a leading glyph and the current selection, and opens a menu on tap. |
| TextField | Components | 3 | [components/text-field.md](components/text-field.md) | The reusable labeled text and search input on Cumulus control chrome, with supporting and validation messaging. |
...
| NumberStepper | Components | 3 | [components/number-stepper.md](components/number-stepper.md) | A labeled, accessible decrement/value/increment row with optional canonical resource notation. |
| DisclosureSection | Components | 4 | [components/disclosure-section.md](components/disclosure-section.md) | A controlled Cumulus section for progressively revealing dense secondary information. |
...
| Game Card | Components | 27 | [components/game-card.md](components/game-card.md) | The playable card object — art, cost, stats, and rules text — rendered at any size and always resolved by UUID, never by name. |
