...
> **Guidance:** Use this when a screen presents a card collection as the primary task surface, such as the Starting Deck reveal, a card-selection site, or a searchable zone browser.
...
| `heightMode` | `CardGalleryHeightMode` = `"content" \| "fill"` | no | `content` | Transparent layout-wrapper height behavior. Defaults to `content`. `fill` may reserve caller-owned stage space for card fitting, but the floating glass surface still hugs its rendered header, grid, and footer and centers vertically within that reserved space. |
