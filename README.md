...
a Dream Avatar, a fixed package resolves, and the run proceeds through draft
...
- Optional: dream avatar and dreamsign portraits at
  `~/Documents/synty/dream-avatars/` and `~/Documents/dreamsigns/filtered/`.
  Missing portraits behave like missing card artwork.

The bundled card, dream avatar, and dreamsign TOML data live under `data/` in
...
emulator, adds `?game=<room-id>` to the URL, and opens the Dream Avatar selection
...
```
data/             Bundled TOML data (cards, dreamAvatars, dreamsigns)
