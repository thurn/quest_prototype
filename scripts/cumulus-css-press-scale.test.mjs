...
//
// CSS companion to the `cumulus/no-adhoc-press-scale` ESLint rule.
//
...
// test scans every src/cumulus CSS file's `transform:` declarations and fails on a
 ...
 * `[scaleCall, fileRelative]` (e.g. `["scale(1.03)", "src/cumulus/…/foo.css"]`).
...
/**
 * PURE core: scan every src/cumulus CSS file and return every non-compliant
...
export function findAdhocCssScales() {
  const offenders = [];
  for (const full of collectCssFiles(CUMULUS_ROOT)) {
...
describe("src/cumulus CSS has no ad-hoc press/hover scale literals", () => {
  it("every transform: scale() is scale(1), a var(--…), or baselined", () => {
    ...
    const message = unexpected
      .map(
        (o) =>
          `ADHOC CSS SCALE ${JSON.stringify(o.call)} in ${o.file} — use scale(var(--hover-scale)) / scale(var(--press-scale)) / scale(var(--node-hover-scale)), or add it to BASELINE in scripts/cumulus-css-press-scale.test.mjs`,
      )
      .join("\n");
