...
//
// Cross-file duplicate-literal integrity check for src/cumulus.
//
...
// This test scans every Cumulus source file for such literals and fails the
...
const CUMULUS_ROOT = resolve(ROOT, "src/cumulus");

// The generated mirror of cumulus-tokens.css: every token value it carries is a
...
/**
 * PURE core: scan `src/cumulus` and return every trimmed visual literal
...
export function findCrossFileDuplicates() {
  ...
  for (const fullPath of collectFiles(CUMULUS_ROOT)) {
...
describe("src/cumulus has no unreviewed cross-file duplicate visual literals", () => {
  it("every cross-file duplicate is baselined", () => {
    ...
    const message = unexpected
      .map(
        (dup) =>
          `DUPLICATE ${JSON.stringify(dup.literal)} in [${dup.files.join(", ")}] — either dedupe it into a shared source of truth or add it to BASELINE in scripts/cumulus-duplicate-literals.test.mjs`,
      )
      .join("\n");
