...
import {
  DREAM_AVATAR_EDITOR_SOURCE_PATHS,
  createDreamAvatarEditorApiMiddleware,
} from "./dream-avatar-editor-api.mjs";
...
function fixture() {
  const rootDir = mkdtempSync(join(tmpdir(), "dream-avatar-editor-api-"));
  roots.push(rootDir);
...
describe("DreamAvatar canonical RON editor API", () => {
  it("loads a revision and submits stable-id semantic field operations", async () => {
    ...
    const publishEdit = vi.fn(async ({ operations }) => {
      data.dreamAvatars[0].name = operations[0].value;
      return { sourceRevision: "revision-2" };
    });
    const middleware = createDreamAvatarEditorApiMiddleware({
      rootDir,
      loadData: () => data,
      revision: () => "revision-1",
      publishEdit,
    ...
    const loaded = await invoke(middleware, "GET", "/api/editor/dream-avatars");
    ...
    const saved = await invoke(
      middleware,
      "PATCH",
      `/api/editor/dream-avatars/${AVATAR_ID}`,
      {
        id: AVATAR_ID,
        field: "name",
        value: "  Edited Avatar  ",
    ...
    expect(saved.body).toMatchObject({
      dreamAvatar: { id: AVATAR_ID, name: "Edited Avatar" },
