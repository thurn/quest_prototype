...
describe("validateCardEdit", () => {
  ...
  it("normalizes and clamps art crop values", () => {
    // Pan is normalized to -1..1 (0 = centered, ±1 = image edge).
    expect(validateCardEdit("art", { x: 0.4, y: -0.6, scale: 2 })).toMatchObject({
      ok: true,
      value: { x: 0.4, y: -0.6, scale: 2 },
    });
    // Pan clamps to -1..1 and scale clamps to 1..5.
    expect(validateCardEdit("art", { x: 5, y: -5, scale: 9 })).toMatchObject({
      ok: true,
      value: { x: 1, y: -1, scale: 5 },
    });
    // Pan rounds to 3 decimals, scale to 2.
    expect(
      validateCardEdit("art", { x: 0.33333, y: -0.12678, scale: 1.234 }),
    ).toMatchObject({
      ok: true,
      value: { x: 0.333, y: -0.127, scale: 1.23 },
    });
  });
...
`;
  ...
  it("inserts an art crop table into a card that has none", () => {
    ...
    const patched = patchRenderedCardsToml(source, {
      cardId: FIRST_ID,
      field: "art",
      value: { x: 0.4, y: -0.6, scale: 1.3 },
    });
    const parsed = parse(patched.source);

    expect(parsed.cards[0].art).toEqual({ x: 0.4, y: -0.6, scale: 1.3 });
    expect(patched.source).toContain("art = { x = 0.4, y = -0.6, scale = 1.3 }");
  ...
  it("replaces an existing art crop table in place", () => {
    ...
    const replaced = patchRenderedCardsToml(inserted, {
      cardId: FIRST_ID,
      field: "art",
      value: { x: -0.25, y: 0.8, scale: 2 },
    });
    const parsed = parse(replaced.source);

    expect(parsed.cards[0].art).toEqual({ x: -0.25, y: 0.8, scale: 2 });
    expect(replaced.source).not.toContain("x = 0.4");
  });
