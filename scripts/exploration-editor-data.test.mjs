...
describe("exploration editor data", () => {
  ...
  it("publishes a closed code-owned schema without player copy", () => {
    ...
    expect(
      EXPLORATION_EFFECT_SCHEMAS.filter(({ kind }) =>
        [
    ...
    expect(
      EXPLORATION_EFFECT_SCHEMAS.filter((definition) =>
        ["free-next-shop", "lose-half-essence-and-free-purchases"].includes(
          definition.kind,
        ),
      ),
    ).toEqual([
    ...
    expect(
      EXPLORATION_EFFECT_SCHEMAS.filter((definition) =>
        ["change-card-type-selected", "replace-random-with-card"].includes(
          definition.kind,
        ),
      ),
    ).toEqual([
    ...
    expect(
      ...
      EXPLORATION_EFFECT_SCHEMAS.findIndex(
        ({ kind }) => kind === "replace-selected-with-card",
      ) + 1,
    );
    expect(
      EXPLORATION_EFFECT_SCHEMAS.findIndex(
        ({ kind }) => kind === "replace-selected",
      ),
    ).toBe(
      EXPLORATION_EFFECT_SCHEMAS.findIndex(
        ({ kind }) => kind === "replace-random-with-card",
    ...
    expect(
      EXPLORATION_EFFECT_SCHEMAS.find(
        (definition) =>
          definition.kind === "purge-selected-dreamsign-and-gain-random",
      ),
    ).toEqual(
      expect.objectContaining({
        canonicalMechanicId: "gain-dreamsign",
  ...
  it("normalizes exact Wave 8 contracts and rejects malformed fields", () => {
    ...
    const actions = [
      ...
      },
      {
        ...
        followupTitle: "Choose one",
        followupSubtitle: "Purge one card",
      },
    ];
    const normalized = actions.map((action) =>
      normalizeExplorationAction(action),
    );
    expect(
      normalized.map(({ canonicalMechanicId, selectionPolicyId }) => [
        canonicalMechanicId,
        selectionPolicyId,
      ]),
    ).toEqual([
  ...
  it("normalizes fixed site insertion and rejects malformed variants", () => {
    ...
    for (const [action, message] of [
      ...
      [
        { ...fixedSite, followupTitle: "Choose", followupSubtitle: "Choose" },
        /does not support a followup/u,
      ],
      [
        { ...fixedSite, effectText: "Add {site_type}" },
  ...
  it("normalizes the exact site-type chooser contract", () => {
    ...
    for (const [action, message] of [
      ...
      [{ ...chooser, followupSubtitle: "" }, /invalid followup override/u],
      [{ ...chooser, siteType: "Shop" }, /siteType does not apply/u],
      [{ ...chooser, count: 1 }, /count does not apply/u],
      [
        { ...chooser, effectText: "Choose {site_type}" },
        /presentation tokens/u,
      ],
    ]) {
      expect(() => normalizeExplorationAction(action)).toThrow(message);
    }
  ...
  it("rejects unknown action card and Dreamsign references before staging", () => {
    ...
    ).toThrow(/Unknown card reference/u);
    expect(() =>
      normalizeExplorationAction(
        {
    ...
    ).toThrow(/Unknown Dreamsign reference/u);
  });
  ...
  it("validates exact Nightmare Dreamsign editor fields before staging", () => {
    ...
    for (const [action, message] of [
      ...
      [
        { ...offered, dreamsignId: fixed.dreamsignId },
        /dreamsignId does not apply/u,
      ],
    ]) {
      expect(() => normalizeExplorationAction(action, references)).toThrow(
        message,
      );
    }
  });
  ...
  it("validates exact starter-card editor fields before staging", () => {
    ...
    for (const [action, message] of [
      ...
      [
        { ...replaceAll, cardId: "00000000-0000-4000-8000-000000000001" },
        /cardId does not apply/u,
      ],
      [
        { ...replaceAll, followupTitle: "Choose", followupSubtitle: "Choose" },
        /does not support a followup/u,
      ],
    ]) {
      expect(() => normalizeExplorationAction(action)).toThrow(message);
  ...
  it("normalizes automatic starter transfigurations and rejects foreign fields", () => {
    ...
    for (const [action, message] of [
      ...
      [
        { ...random, transfiguration: "Inspired" },
        /transfiguration does not apply/u,
      ],
      [{ ...all, count: 2 }, /count does not apply/u],
      [
        { ...all, cardId: "00000000-0000-4000-8000-000000000001" },
        /cardId does not apply/u,
      ],
      [
        { ...all, followupTitle: "Choose", followupSubtitle: "Choose" },
        /does not support a followup/u,
      ],
      [
        { ...all, effectText: "Transfigure {starter_card}" },
        /does not support presentation tokens/u,
      ],
      [
        { ...all, effectText: "Transfigure $DECK_CARD" },
        /does not support presentation tokens/u,
      ],
    ]) {
      expect(() => normalizeExplorationAction(action)).toThrow(message);
  ...
  it("normalizes chosen and automatic multi-card transfigurations", () => {
    ...
    for (const [action, message] of [
      ...
      [
        { ...random, followupTitle: "Choose", followupSubtitle: "Choose" },
        /does not support a followup/u,
      ],
      [
        { ...random, effectText: "Transfigure {deck_card}" },
        /does not support presentation tokens/u,
      ],
      [
        { ...random, transfiguration: "Kindled" },
        /transfiguration does not apply/u,
      ],
  ...
  it("normalizes counted replacement, fixed transfiguration, copy, and card-type effects", () => {
    ...
    for (const [action, message] of [
      ...
      [
        { ...copy, followupTitle: "Choose", followupSubtitle: "Choose" },
        /does not support a followup/u,
      ],
      [
  ...
  it("normalizes exact Wave7 deck-mutation fields and rejects missing or foreign fields", () => {
    ...
    for (const [action, message] of [
      [
        { ...replacement, predicate: undefined },
        /requires a supported non-Any predicate/u,
      ],
      [{ ...replacement, cardId: undefined }, /requires cardId/u],
      [{ ...replacement, count: 1 }, /count does not apply/u],
      [
        { ...replacement, followupTitle: "Choose", followupSubtitle: "Choose" },
        /does not support a followup/u,
      ],
      [
        { ...replacement, effectText: "Replace {deck_card}" },
      ...
      [
        { ...chosen, effectText: "Change {deck_card}" },
        /unsupported presentation token/u,
      ],
    ]) {
      expect(() => normalizeExplorationAction(action, references)).toThrow(
