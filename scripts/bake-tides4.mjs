...
//      cards, providing the generic tail that tops a pool up.
...
//      on-identity facets. At runtime, a null starter marks a signatureless
//      DreamAvatar; the pool builder then selects a random signatured DreamAvatar
//      and uses that Avatar's starter, facets, and neutral tail as one coherent
//      borrowed archetype.
//
...
// Production treats a null starter as the signatureless marker: it randomly
// selects a signatured DreamAvatar and consumes that borrowed entry's starter,
// facets, and neutral tail. The signatureless entry's complete facet library and
// broad tail remain fallback data for artifact consumers.
//
...
function renderMarkdown(json, dreamAvatars) {
  ...
  lines.push(
    ...
    "subset of a DreamAvatar's facets creates distinct coherent runs. At runtime,",
    "a signatureless DreamAvatar borrows a random signatured DreamAvatar's starter,",
    "facets, and neutral tail, so the resulting pool follows one coherent archetype.",
    "",
...
export function buildTides4({
  ...
  //   * a SIGNATURELESS DreamAvatar records the whole facet library and every
  //     broad tide as fallback data. Production recognizes its null starter and
  //     borrows a randomly selected signatured DreamAvatar's complete entry.
