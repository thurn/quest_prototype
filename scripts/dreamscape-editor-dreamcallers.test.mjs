...
function dreamscapes() {
  return [
    { id: "starter", name: "Starter", isStarter: true, dreamcallerIds: [] },
    { id: "alpha", name: "Alpha", isStarter: false, dreamcallerIds: [A1, A2, A3, A4] },
    { id: "beta", name: "Beta", isStarter: false, dreamcallerIds: [B1, B2, B3] },
  ];
}
...
describe("planDreamcallerAssignment — replace", () => {
  it("replaces a resident with an unassigned caller (no swap)", () => {
    const plan = planDreamcallerAssignment(dreamscapes(), CATALOG_IDS, {
      action: "replace",
      dreamscapeId: "beta",
      outId: B1,
      inId: POOL1,
...
describe("planDreamcallerAssignment — add", () => {
  ...
  it("rejects pulling a caller out of a region already at the minimum", () => {
    ...
    const plan2 = planDreamcallerAssignment(
      [
        { id: "alpha", name: "Alpha", isStarter: false, dreamcallerIds: [A1, A2, A3] },
        { id: "beta", name: "Beta", isStarter: false, dreamcallerIds: [B1, B2, B3] },
      ],
      CATALOG_IDS,
...
describe("planDreamcallerAssignment — remove and guards", () => {
  ...
  it("rejects assigning to the starter and rejects unknown callers", () => {
    expect(
      planDreamcallerAssignment(dreamscapes(), CATALOG_IDS, {
        action: "add",
...
describe("rewriteDreamcallerIds / applyDreamcallerChanges", () => {
  it("rewrites one region's array in the multiline name-comment style", () => {
    const next = rewriteDreamcallerIds(fixtureToml(), {
      dreamscapeId: "beta",
      ids: [POOL1, B2, B3],
      nameById: NAME_BY_ID,
    });
    ...
    expect(beta["dreamcaller-ids"]).toEqual([POOL1, B2, B3]);
  ...
  it("applies a two-region swap atomically and keeps the file valid", () => {
    ...
    const next = applyDreamcallerChanges(fixtureToml(), plan.changes, NAME_BY_ID);
    const parsed = parse(next);
    const alpha = parsed.dreamscapes.find((entry) => entry.id === "alpha");
    const beta = parsed.dreamscapes.find((entry) => entry.id === "beta");
    expect(alpha["dreamcaller-ids"]).toEqual([A1, A2, A3, B1]);
    expect(beta["dreamcaller-ids"]).toEqual([A4, B2, B3]);
  });
  ...
  it("collapses an emptied array to the inline form", () => {
    ...
    expect(next).toContain("dreamcaller-ids = []");
    expect(parse(next).dreamscapes.find((entry) => entry.id === "beta")["dreamcaller-ids"]).toEqual(
      [],
    );
  });
});
