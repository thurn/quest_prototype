...
function summaryFromRecord(record) {
  return {
    ...
    screenType: record?.journeyState?.screen?.type ?? "unknown",
  };
}
...
function writeRecordAtomic(fs, dir, fileName, record) {
  ensureDir(fs, dir);
  const tempDir = fs.mkdtempSync(join(tmpdir(), "journey-save-"));
...
/**
 * Creates a connect-style middleware serving the saved-journey endpoints:
 *
 * - `GET    /api/saved-journeys`            -> `{ saves: SavedJourneySummary[] }`
 * - `POST   /api/saved-journeys`            -> body `{ name, journeyState }`, saves
 * - `GET    /api/saved-journeys/<name>`     -> `{ name, savedAt, journeyState }`
 * - `DELETE /api/saved-journeys/<name>`     -> `{ ok: true }`
 *
 * `rootDir` is the repository root; saves live in `rootDir/saved-journeys`. A
 * `fileSystem` override is accepted for tests.
 */
export function createSavedJourneysApiMiddleware({
  rootDir,
  fileSystem = defaultFileSystem,
} = {}) {
  if (typeof rootDir !== "string" || rootDir === "") {
    throw new Error("createSavedJourneysApiMiddleware requires a rootDir.");
  }
  const savesDir = join(rootDir, SAVES_DIR_NAME);

  return function savedJourneysApiMiddleware(req, res, next) {
    ...
    if (req.method === "GET" && !isCollection) {
      ...
      if (found === null) {
        errorResponse(res, 404, "NOT_FOUND", `No saved journey named "${decodedName}".`);
        return;
      }
    ...
    if (req.method === "POST" && isCollection) {
      readRequestBody(req)
        .then((body) => {
          ...
          if (name === "") {
            errorResponse(res, 400, "INVALID_NAME", "A saved journey needs a name.");
            return;
          }
          ...
          if (parsed?.journeyState === undefined || parsed.journeyState === null) {
            errorResponse(res, 400, "INVALID_STATE", "Request body is missing journeyState.");
            return;
          }
