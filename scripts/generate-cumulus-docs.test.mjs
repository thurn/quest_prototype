...
describe("extractDemoDoc", () => {
  it("extracts id, title, blurb, guidance, details, props notes, group, docName, and usage", () => {
  ...
  it("rejects multi-sentence info callouts", () => {
    const source = DEMO_FIXTURE.replace(
      `callout: "Prefer a real component.",`,
      `callout: "Prefer a real component. Keep this box short.",`,
    );
  ...
  it("does not treat common abbreviations as sentence boundaries", () => {
    expect(
      sentenceCount("Use the shell (e.g. GlassPanel) for docked content."),
    ).toBe(1);
    expect(sentenceCount("See Dr. Smith for details.")).toBe(1);
  });

  it("requires details to be a non-empty literal array", () => {
    const source = DEMO_FIXTURE.replace(
      `details: [\n    "The fixture keeps its supporting explanation in ordinary prose.",\n    "Each item becomes a separate paragraph.",\n  ],`,
      `details: [],`,
    );
...
describe("renderComponentMarkdown", () => {
  ...
  it("renders blurb, callout, supporting paragraphs, props table, and usage snippets", () => {
    ...
    expect(markdown).toContain(
      "Required fields depend on the selected variant.",
    );
    ...
    expect(markdown).toContain(
      "GENERATED by scripts/generate-cumulus-docs.mjs",
    );
  });
...
describe("index rendering and splicing", () => {
  ...
  it("renders one index row per component with a first-sentence summary", () => {
    const index = renderIndexSection(
      docs,
      new Map(docs.map((d) => [d.title, 0])),
    );
  ...
  it("replaces only the text between the markers, repeatably", () => {
    ...
    const skill = `# Skill\n\n${INDEX_BEGIN_MARKER}\nold content\n${INDEX_END_MARKER}\n\nAfter.\n`;
    const once = spliceGeneratedIndex(
      skill,
      renderIndexSection(docs, countByTitle),
    );
    const twice = spliceGeneratedIndex(
      once,
      renderIndexSection(docs, countByTitle),
    );
