// Simulate draft pools for each v2 Dreamcaller and report aggregate statistics,
...
//
// For each Dreamcaller it runs N reproducible seeds (base-seed + i) and reports
...
//
// Usage:
//   node scripts/simulate-dreamcaller-pools.mjs                       # all, 200 seeds
//   node scripts/simulate-dreamcaller-pools.mjs --seeds 500
//   node scripts/simulate-dreamcaller-pools.mjs --dreamcaller "Kell Tarn"
//   node scripts/simulate-dreamcaller-pools.mjs --top 15
...
/**
 * Run `seeds` generations for one Dreamcaller and aggregate statistics over the
 * resulting pools.
 */
function simulateDreamcaller(dreamcaller, poolData, cardMeta, seeds, baseSeed) {
...
function main() {
  ...
  const filter = parseStringFlag(argv, "--dreamcaller");
  const asJson = argv.includes("--json");

  const poolData = buildPoolData(loadCards());
  const cardMeta = loadCardMeta();
  const allDreamcallers = loadDreamcallers();

  const targets =
    filter !== null
      ? [findDreamcaller(allDreamcallers, filter)].filter(Boolean)
      : allDreamcallers;

  if (targets.length === 0) {
    process.stderr.write(`# no Dreamcaller matched: ${String(filter)}\n`);
    process.exit(1);
  }

  const reports = targets.map((d) =>
    simulateDreamcaller(d, poolData, cardMeta, seeds, baseSeed),
  );
  ...
  process.stderr.write(
    `# simulating ${reports.length} Dreamcaller(s) × ${seeds} seeds (base ${baseSeed})\n`,
  );
