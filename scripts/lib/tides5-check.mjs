// Shared staleness check for the committed tides5 artifact (data/tides5.jsonc).
//
...
import { bakeTides5ArtifactText } from "../bake-tides5.mjs";

const HERE = dirname(fileURLToPath(import.meta.url));
export const ROOT = resolve(HERE, "..", "..");
export const ARTIFACT_REL = "data/tides5.jsonc";
export const OVERRIDES_REL = "data/tides5-overrides.jsonc";
 ...
 */
export function checkTides5({ rootDir = ROOT } = {}) {
  const artifactPath = resolve(rootDir, ARTIFACT_REL);
  if (!existsSync(artifactPath)) {
    throw new Error(
      `${ARTIFACT_REL} does not exist. Generate it with \`npm run bake-tides5\`.`,
    );
  }
...
export function formatStaleMessage(result) {
  ...
  lines.push("");
  lines.push("════════════════════════════════════════════════════════════════════════");
  lines.push("  data/tides5.jsonc is OUT OF SYNC with the tides5 bake.");
  lines.push("════════════════════════════════════════════════════════════════════════");
  lines.push("");
  lines.push("`data/tides5.jsonc` is a GENERATED file. It differs from what");
  lines.push("`npm run bake-tides5` produces from the current source data, so it must be");
  ...
  lines.push("    npm run bake-tides5    # rewrite data/tides5.jsonc + the markdown");
  lines.push("    git add data/tides5.jsonc docs/cards2/tides5_decklists.md");
  lines.push("");
  lines.push("────────────────────────────────────────────────────────────────────────");
  lines.push("  FIX 2 — you HAND-EDITED card lists in data/tides5.jsonc:");
  lines.push("────────────────────────────────────────────────────────────────────────");
  ...
  lines.push("layer instead:  data/tides5-overrides.jsonc");
  lines.push("");
