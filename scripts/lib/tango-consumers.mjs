// Consumer-count helper for the Tango component catalog.
//
// For every entry registered in src/tango/docs/registry.ts, this computes how
...
// NOT part of the doc site (src/tango/docs/) and NOT a test/spec. A component
...
// never disagree:
//   - scripts/tango-ghost-components.test.mjs, which fails the build when a NEW
...
//      names the demo file.
//   2. The demo's object literal (`export const <name>: TangoComponent = {…}`)
...
export function computeConsumerCounts() {
  ...
  const arrayInit = findExportedInitializer(registrySource, "TANGO_COMPONENTS");
  if (!arrayInit || !ts.isArrayLiteralExpression(arrayInit)) {
    throw new Error(
      "tango-consumers: could not find the TANGO_COMPONENTS array literal in registry.ts",
    );
  }
  ...
  return demoNames.map((demoName) => {
    ...
    if (!demoSpecifier) {
      throw new Error(
        `tango-consumers: registry entry "${demoName}" has no matching import in registry.ts`,
      );
    }
    const demoFile = resolveModule(REGISTRY_PATH, demoSpecifier);
    if (!demoFile) {
      throw new Error(
        `tango-consumers: could not resolve demo module "${demoSpecifier}" (from ${demoName}) to a file`,
      );
    }
    ...
    if (!objectLiteral || !ts.isObjectLiteralExpression(objectLiteral)) {
      throw new Error(
        `tango-consumers: demo "${demoName}" (${relative(ROOT, demoFile)}) does not export an object literal`,
      );
    }
    ...
    if (!docName) {
      throw new Error(
        `tango-consumers: demo "${demoName}" (${relative(ROOT, demoFile)}) has no string docName`,
      );
    }
    ...
    if (!sourceSpecifier) {
      throw new Error(
        `tango-consumers: could not resolve the source module for "${demoName}" ` +
    ...
    if (!moduleFile) {
      throw new Error(
        `tango-consumers: could not resolve source module "${sourceSpecifier}" ` +
