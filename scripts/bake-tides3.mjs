...
//   4. tidePoolByDreamcaller — per Dreamcaller, the tides a pool combines, split
//      into LEADS (one drawn at random per run, always joined) and FILL (broad
//      tail). A signatured Dreamcaller has one lead, its own signature tide; a
//      neutral Dreamcaller has every signature tide as a lead candidate, so each
...
function renderMarkdown(json, dreamcallers) {
  ...
  lines.push("");
  lines.push("| Dreamcaller | Lead tide(s) | Fill tides |");
  lines.push("| --- | --- | --- |");
  const nameOfTide = (id) => tideById.get(id)?.name ?? id;
  for (const dc of dreamcallers) {
    const entry = json.tidePoolByDreamcaller[dc.id] ?? { leads: [], fill: [] };
    const leads =
...
function run() {
  ...
  // tidePoolByDreamcaller — leads (one drawn at random per run) plus a broad fill
  ...
  // is what differs:
  //   * a SIGNATURED Dreamcaller has a single lead candidate — its own signature
  //     tide — so every pool leans the same way, on its own identity;
  //   * a NEUTRAL Dreamcaller has every signature tide as a lead candidate (the
  ...
  //     signatureless Dreamcaller. Leading with a coherent archetype (rather than
  ...
  for (const dc of dreamcallers) {
    ...
    if (ownSig) {
      ...
      const fill = neutralTides
        ...
        .map((x) => x.id);
      tidePoolByDreamcaller[dc.id] = { leads: [ownSig], fill };
    } else {
