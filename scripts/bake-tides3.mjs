...
// each pool live from a Dreamcaller's signature cards through the pick-affinity
...
//      synergy of `pickfit`).
//   2. 20 SIGNATURE tides — one per signatured Dreamcaller: grow `sigseed`'s
//      pool from the Dreamcaller's FULL resolved signature set, baked as a deck.
//      This is the deterministic centre of the pools `sigseed` grows for that
//      Dreamcaller (whose per-run variety is a random signature SUBSET).
...
//      for a signatureless Dreamcaller. They are the generic tail of a signatured
//      pool and the body of a neutral Dreamcaller's pool.
//   4. tidePoolByDreamcaller — per Dreamcaller, the ordered tide ids a pool
//      combines: the LEAD first (a signatured Dreamcaller's own signature tide;
//      a neutral Dreamcaller's own broad tide), then FILL tides (the closest
//      broad tides for a signatured Dreamcaller; a farthest-point spread of every
//      tide for a neutral one).
//
...
const TUNING = {
  // Copies in a signature tide — one signatured Dreamcaller's `sigseed` pool.
  ...
  // deal), and neutral Dreamcallers combine several into a broad pool.
  neutralTideSize: 30,
  ...
  // How many broad (neutral) tides a signatured Dreamcaller's pool lists as fill,
...
// by card id. Used to order a signatured Dreamcaller's broad fill tides.
...
// each signature tide is a Dreamcaller's full-signature \`sigseed\` pool baked as
...
// its first id is the lead (always joined), the rest fill (shuffled at runtime).
//
...
function renderMarkdown(json, dreamcallers) {
  ...
  lines.push(
    ...
    "copies of a card). `tides3` is the human-legible counterpart of `sigseed` —",
    "each signature tide is a Dreamcaller's full-signature `sigseed` pool baked as",
    "a deck; the neutral tides are the broad pools `sigseed` reduces to for a",
    "signatureless Dreamcaller.",
    "",
  ...
  lines.push("");
  lines.push("| Dreamcaller | Lead tide | Fill tides |");
  lines.push("| --- | --- | --- |");
  for (const dc of dreamcallers) {
    const pool = json.tidePoolByDreamcaller[dc.id] ?? [];
    const lead = pool[0] ? tideById.get(pool[0])?.name ?? pool[0] : "(none)";
    const fill = pool
      .slice(1)
      .map((id) => tideById.get(id)?.name ?? id)
      .join("; ");
    lines.push(`| ${dc.name} | ${lead} | ${fill || "(none)"} |`);
  }
  lines.push("");
...
function run() {
  ...
  const tides = [];
  // 20 SIGNATURE tides — one per signatured Dreamcaller, grown from its full
  // signature exactly as `sigseed` would (the deterministic all-signatures pool).
  const signatureTideByDreamcaller = new Map();
  const noSignal = [];
  let sigIdx = 0;
  for (const dc of dreamcallers) {
    ...
    signatureTideByDreamcaller.set(dc.id, id);
  }
  ...
  // tidePoolByDreamcaller. A signatured Dreamcaller leads with its own signature
  // tide and fills from the closest broad tides; a neutral Dreamcaller leads with
  // a broad tide (rotated so each has its own identity) and fills with a
  // farthest-point spread of every tide.
  const tidePoolByDreamcaller = {};
  for (const dc of dreamcallers) {
    const ownSig = signatureTideByDreamcaller.get(dc.id);
    if (ownSig) {
      const lead = tides.find((t) => t.id === ownSig);
      // Fill from the nearest broad (neutral) tides: they carry the generic tail
      // a `sigseed` pool's play-rate prior pulls in, without adding a second
      // theme (an allied signature tide would pull the pool off this
      // Dreamcaller's identity). One joins per pool (the rest give the shuffle
      // room for run-to-run variety in which broad cards land).
      const fill = neutralTides
        .map((t) => ({ id: t.id, s: tideCosine(lead, t) }))
        .sort((a, b) => b.s - a.s || (a.id < b.id ? -1 : 1))
        .slice(0, TUNING.signatureFillWidth)
        .map((x) => x.id);
      tidePoolByDreamcaller[dc.id] = [ownSig, ...fill];
    }
  }
  // Neutral Dreamcallers (no signature tide): assign in roster order, rotating
  // which broad tide leads, and fill with a farthest-point spread of every tide.
  const neutralDreamcallers = dreamcallers.filter(
    (dc) => !signatureTideByDreamcaller.has(dc.id),
  );
  const allTideIds = tides.map((t) => t.id);
  neutralDreamcallers.forEach((dc, i) => {
    const lead = neutralTides[i % neutralTides.length].id;
    // Farthest-point order over every tide for a broad fill, starting from the
    // lead so the closest-to-lead tides come last.
    const rest = allTideIds.filter((id) => id !== lead);
    const ordered = [];
    ...
    while (remaining.size > 0) {
      ...
      for (const id of remaining) {
        const t = tides.find((x) => x.id === id);
        let nearest = Infinity;
        for (const a of anchorList) {
          const at = tides.find((x) => x.id === a);
          const d = 1 - tideCosine(t, at);
          if (d < nearest) nearest = d;
        }
        if (nearest > bestDist || (nearest === bestDist && best !== null && id < best)) {
          bestDist = nearest;
          best = id;
        }
      }
      ...
      anchorList.push(best);
    }
    tidePoolByDreamcaller[dc.id] = [lead, ...ordered];
  });

  // Strip the bake-only `dreamcallerId` from the serialized tides (it is recorded
  ...
  // Render with the dreamcallerId-carrying tides for the owner column.
  writeFileSync(
    resolve(ROOT, docRel),
  ...
  if (noSignal.length > 0) {
    console.warn(`Signatured Dreamcallers with no corpus signature: ${noSignal.join(", ")}.`);
  }
