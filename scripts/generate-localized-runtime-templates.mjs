...
export function localizedCardSubtype(sourceText: string): LocalizedString | null {
  switch (sourceText) {
${cardSubtypes.map((subtype) => `    case ${JSON.stringify(subtype)}:\n      return tx(${JSON.stringify(subtype)}, "Player-facing Character subtype name from the canonical card catalog.");`).join("\n")}
    default:
      return null;
  }
}
...
export function localizedGuideDialogue(sourceText: string): LocalizedString | null {
  switch (sourceText) {
${staticGuideDialogue.map((line) => `    case ${JSON.stringify(line)}:\n      return tx(${JSON.stringify(line)}, "Player-facing Dream Guide dialogue from the canonical guide catalog.");`).join("\n")}
    default:
      return null;
  }
}
...
export function localizedDreamsignImageAlt(sourceText: string): LocalizedString | null {
  switch (sourceText) {
${dreamsignImageAlts.map((line) => `    case ${JSON.stringify(line)}:\n      return tx(${JSON.stringify(line)}, "Player-facing Dreamsign image description from the canonical Dreamsign catalog.");`).join("\n")}
    default:
      return null;
  }
}
