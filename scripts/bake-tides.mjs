...
//   3. FAVORED tides per DreamAvatar: the tides most IDF-cosine-similar to the
//      DreamAvatar's signature cards — the same probe `idf3` uses to find its
//      anchor decks — baked as an inspectable list. The runtime draws a subset
//      of these, so a DreamAvatar's pools stay on-identity without any hidden
//      runtime scoring.
//
...
const TUNING = {
  ...
  idfRankWeight: 2,
  // How many favored tides are baked per signatured DreamAvatar. The runtime
  // draws a subset (see `TIDES.favoredDraw` in variant-tides.ts), so a wider
  // list here means more pool-to-pool variety per DreamAvatar.
  favoredPerDreamAvatar: 4,
...
// IDF-cosine of a DreamAvatar's signature probe against a tide's card multiset
...
// the decklists; its `favoredTidesByDreamAvatar` is unused by `tides2`).
...
// decklists (public/decklists-data.json) and the DreamAvatar signatures — do
...
// DreamAvatar's tide pool, shuffling in the lead's allied tides until there are
// enough cards, then dealing the first 200 (never more than 2 copies of a card).
// Which tides are allied and which a DreamAvatar draws from live in the curated
...
// the DreamAvatar's favored tides with tides drawn at random until there are
// enough cards, then dealing the first 200 (never more than 2 copies of a
// card).
//
// Cards are keyed by stable cards_v2 UUID; \`name\` fields are informational,
// refreshed at bake time. \`favoredTidesByDreamAvatar\` is keyed by DreamAvatar
...
function serializeArtifact(json, header) {
  ...
  const favored = Object.entries(json.favoredTidesByDreamAvatar).map(
    ([dc, tides], i, arr) =>
...
// appends the relationship tables (allied tides, DreamAvatar tide pools) after
...
function renderMarkdown(json, dreamAvatars, tideStats, variant) {
  ...
  if (variant === "tides2") {
    ...
    lines.push(
      ...
      "lead tide from the DreamAvatar's tide pool, shuffling in the lead's allied",
      "tides until there are enough cards, then dealing the first 200 (never more",
      "than 2 copies of a card). Which tides are allied and which a DreamAvatar",
    ...
    lines.push(
      ...
      "DreamAvatar's favored tides with tides drawn at random until there are",
      "enough cards, then dealing the first 200 (never more than 2 copies of a",
      "card).",
      "",
    ...
    lines.push("## Favored tides by DreamAvatar");
    lines.push("");
    lines.push("| DreamAvatar | Favored tides |");
    lines.push("| --- | --- |");
    for (const dc of dreamAvatars) {
      const favored = json.favoredTidesByDreamAvatar[dc.id] ?? [];
...
function run() {
  ...
  // Favored tides per signatured DreamAvatar, by signature-probe cosine.
  // Resolve the DreamAvatar's signature card names to UUIDs so the probe
  // operates entirely in UUID space (matching the UUID-keyed IDF corpus).
  const archetypeTides = tides;
  const favoredTidesByDreamAvatar = {};
  const noSignal = [];
  for (const dc of dreamAvatars) {
    ...
    const scored = archetypeTides
      ...
      .slice(0, TUNING.favoredPerDreamAvatar);
    if (scored.length > 0) {
      favoredTidesByDreamAvatar[dc.id] = scored.map((x) => x.tide.id);
    }
  }
  ...
  writeFileSync(
    resolve(ROOT, docRel),
    renderMarkdown(json, dreamAvatars, tideStats, variant) + "\n",
  );
  ...
  console.log(
    `Favored tides baked for ${Object.keys(favoredTidesByDreamAvatar).length} ` +
      `of ${dreamAvatars.length} DreamAvatars.`,
  );
  if (noSignal.length > 0) {
    console.warn(`Signatured DreamAvatars with no usable probe: ${noSignal.join(", ")}.`);
  }
  if (dropped.size > 0) {
    console.warn(
