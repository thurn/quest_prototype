...
import {
  DEFAULT_AVATAR_TOML_PATH,
  TIDES_SOURCE_PATH,
  readEditorAvatars,
  readTideCatalog,
  validateAvatarEdit,
  validateTidePool,
} from "./avatar-editor-data.mjs";
import {
  sourceRevision,
...
const BASE_PATH = "/api/editor/avatars";
export const AVATAR_SOURCE_PATH = join("data", "avatars.ron");
export const AVATAR_EDITOR_SOURCE_PATHS = [
  AVATAR_SOURCE_PATH,
  TIDES_SOURCE_PATH,
];
...
function resolveRequestedSource(requested) {
  ...
  const normalized = requested
    ...
    .replace(/^data\//u, "")
    .replace(/\.toml$/iu, ".ron");
  if (normalized !== "avatars.ron") {
    return {
      ok: false,
      message: "The Avatar editor source must be data/avatars.ron.",
    };
  }
  return { ok: true };
}
...
function routeForRawPath(rawPath) {
  ...
  let avatarId;
  try {
    avatarId = decodeURIComponent(encodedSegment);
  } catch {
    return {
      ok: false,
      statusCode: 400,
      code: "INVALID_AVATAR_ID",
      message: "Route Avatar id must be a canonical UUIDv4.",
    };
  }
  if (!UUID_V4_PATTERN.test(avatarId)) {
    return {
      ok: false,
      statusCode: 400,
      code: "INVALID_AVATAR_ID",
  ...
  return { ok: true, resource: "avatar", avatarId };
}
...
function readEditorData(rootDir) {
  return {
    avatars: readEditorAvatars({ rootDir }),
    tides: readTideCatalog({ rootDir }),
  };
}

function confirmedRecord(data, avatarId) {
  return data.avatars.find(
...
function collectionResponse(rootDir, revision, loadData) {
  return {
    ...loadData(rootDir),
    sourceRevision: revision(rootDir, AVATAR_EDITOR_SOURCE_PATHS),
  };
}
...
async function handlePatch(
  ...
  if (body.id !== avatarId) {
    errorResponse(
      ...
      400,
      "AVATAR_ID_MISMATCH",
      "Route Avatar id must match request body id.",
      { routeId: avatarId, bodyId: body.id },
    );
    return;
  }
  const beforeData = loadData(rootDir);
  if (confirmedRecord(beforeData, avatarId) === undefined) {
    errorResponse(
      res,
      404,
      "AVATAR_NOT_FOUND",
  ...
  if (body.field === "tide-pool") {
    ...
    const validation = validateAvatarEdit(body.field, body.value);
    if (!validation.ok) {
      errorResponse(res, 400, "INVALID_EDIT", validation.message, {
  ...
  const confirmed = confirmedRecord(loadData(rootDir), avatarId);
  if (confirmed === undefined) {
    const error = new Error("Published Avatar could not be reloaded.");
  ...
  jsonResponse(res, 200, {
    ...
    sourceRevision:
      ...
      revision(rootDir, AVATAR_EDITOR_SOURCE_PATHS),
...
export function createAvatarEditorApiMiddleware({
  rootDir = ROOT,
  publishEdit = stageAndPublishGameDataEdit,
  revision = sourceRevision,
  loadData = readEditorData,
} = {}) {
  return async function avatarEditorApiMiddleware(req, res, next) {
    const rawPath = rawPathFromUrl(req.url);
    if (!isAvatarApiPath(rawPath)) {
      next();
      return;
    }
    try {
      ...
      if (req.method === "PATCH" && route.resource === "avatar") {
        await handlePatch(
          req,
          res,
          rootDir,
      ...
      errorResponse(
        ...
        error instanceof Error
          ? error.message
          : "Avatar editor transaction failed.",
        {
          datasetId: "avatars",
          source: AVATAR_SOURCE_PATH,
