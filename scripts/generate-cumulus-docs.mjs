// Exports an optional Markdown snapshot of the Cumulus design system.
//
//   node scripts/generate-cumulus-docs.mjs   # also: npm run cumulus-docs
//
// Writes one markdown file per documented Cumulus component, a design-token
// reference, and a component index under .generated/cumulus-docs/. The export
// is disposable; canonical component documentation lives in the same sources
// the /cumulus doc site renders:
//
//   - src/cumulus/docs/registry.ts          canonical component list + order
//   - src/cumulus/docs/demos/<id>.tsx       authored prose: blurb, callout,
//                                           details, usage
//   - src/cumulus/metadata/cumulus-metadata.json  docgen props (materialized by
//                                                 prepare-workspace)
...
//
// The generator owns its disposable components/ output directory and deletes
// any .md file it did not just write.
...
const EXPORT_DIR = resolve(ROOT, ".generated/cumulus-docs");
const COMPONENTS_OUT_DIR = join(EXPORT_DIR, "components");
const INDEX_OUT_PATH = join(EXPORT_DIR, "index.md");
const TOKENS_OUT_PATH = join(EXPORT_DIR, "tokens.md");
...
/**
 * Every optional Markdown snapshot file as an absolute-path → content map,
 * computed from canonical sources without touching disk. `main()` writes this
 * map and sweeps stale component files in the disposable export directory.
 */
export function computeDocOutputs() {
  ...
  files.set(
    INDEX_OUT_PATH,
    `# Cumulus Component Index\n\n${renderIndexSection(docs, countByTitle)}\n`,
  );
...
function main() {
  ...
  console.log(`Wrote component index to ${relative(ROOT, INDEX_OUT_PATH)}`);
  console.log(
