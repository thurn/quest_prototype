...
function fixtureSource() {
  const layers = LAYERS.map((name, index) => ({
    ...
    "node-count": {
      min: index === 0 || index === 6 ? 1 : 2,
      max: index === 0 || index === 6 ? 1 : 3,
    },
    ...(index === 0
      ? { "mandatory-sites": {} }
      : {
...
describe("compileAtlasData", () => {
  ...
  it("rejects unsupported schema versions and malformed layer sets", () => {
    ...
    expectFailure((source) => {
      source.layers[1].name = "one";
    }, /values must be unique/);
    expectFailure((source) => {
      source.layers[2]["node-count"] = { min: 4, max: 2 };
    }, /min must not exceed max/);
  });

  it("rejects invalid probabilities, weights, and mandatory capacity", () => {
    expectFailure((source) => {
      source["known-dreamsign"]["placement-probability"] = 2;
    }, /placement-probability/);
    expectFailure((source) => {
      source["fill-profiles"][0]["site-weights"].Essence = -1;
    }, /site-weights.Essence/);
    expectFailure((source) => {
      source.layers[1]["site-count"].max = 5;
    }, /exceed max/);
    expectFailure((source) => {
      source.layers[2]["mandatory-sites"].Battle = 1;
    }, /Battle is appended structurally/);
  });

  it("rejects unknown layer, site, and fill-profile references", () => {
    expectFailure((source) => {
      source.graph["bonus-reveal"]["eligible-layers"] = ["eight"];
    }, /unknown layer/);
    expectFailure((source) => {
      source.layers[2]["mandatory-sites"].Unknown = 1;
    }, /unknown site type/);
    expectFailure((source) => {
      source.layers[2]["fill-profile"] = "missing";
    }, /unknown profile/);
  });
