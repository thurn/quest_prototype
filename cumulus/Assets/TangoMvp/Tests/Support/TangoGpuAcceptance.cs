...
namespace TangoMvp.Tests.Support
{
    public enum TangoComparison
    {
        GreaterThanOrEqual,
        LessThanOrEqual,
        Equal,
    ...
    [Serializable]
    public sealed class TangoGpuAcceptanceResult
    {
    ...
    public static class TangoGpuAcceptance
    {
        ...
        public static TangoGpuAcceptanceResult Evaluate(
            string metricName,
            float measuredValue,
            ...
            string deviceName = "")
        {
            ...
            return new TangoGpuAcceptanceResult
            {
        ...
        public static bool AllPassed(IEnumerable<TangoGpuAcceptanceResult> results)
        {
            if (results == null)
            {
                return false;
            ...
            bool any = false;
            foreach (TangoGpuAcceptanceResult result in results)
            {
                any = true;
        ...
        public static string Serialize(IEnumerable<TangoGpuAcceptanceResult> results)
        {
            if (results == null)
            {
        ...
        private static bool Compare(float value, TangoComparison comparison, float threshold)
        {
            switch (comparison)
            {
                case TangoComparison.GreaterThanOrEqual:
                    return value >= threshold;
                case TangoComparison.LessThanOrEqual:
                    return value <= threshold;
                case TangoComparison.Equal:
                    return value == threshold;
                default:
        ...
        private static string ComparisonName(TangoComparison comparison)
        {
            switch (comparison)
            {
                case TangoComparison.GreaterThanOrEqual:
