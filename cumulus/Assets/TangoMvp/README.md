...
1. `shell-harness-self-tests` exercises stale/missing/malformed Unity evidence, timeout/process-group handling, provenance, exact GPU evidence, PNG decoding, and scope-guard negative controls.
...
| `blurEdgeEnergyRatioMaximum` | `<= 0.65` | Covered edges retain at most 65% of uncovered edge energy. Higher means blur is ineffective. |
| `blurEdgeEnergyRatioMinimum` | `>= 0.05` | The result retains bounded structure. Lower indicates a flat/blank/overblurred region. |
...
| Moving opaque object stays live through both panes | Metrics `liveBackdropDelta.LiveGlassA` and `.LiveGlassB`; `TangoGlassGpuTests.GlassLab_RendersLiveSharedBlurAndFailClosedFallbackEvidence`. |
