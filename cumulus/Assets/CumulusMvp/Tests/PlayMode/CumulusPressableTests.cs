...
{
    public sealed class CumulusPressableTests : InputTestFixture
    {
        ...
        {
            CumulusPressable pressable = CreatePressable("state-control", out Transform visual, out Collider collider);
        ...
        {
            CumulusPressable pressable = CreatePressable("confirm-button", out _, out _);
            int activations = 0;
            ...
            pressable.BeginPress();
            LogAssert.Expect(LogType.Log, "CumulusPressable activated: confirm-button");
        ...
        public IEnumerator VirtualMouse_ActivatesThroughPointerInteractor()
        {
            ...
            CumulusPressable pressable = CreatePressable("pointer-control", out _, out _);
            ...
            yield return null;
            LogAssert.Expect(LogType.Log, "CumulusPressable activated: pointer-control");
        ...
        public IEnumerator VirtualMouse_DragOffCancelsOriginalPress()
        {
            Mouse mouse = InputSystem.AddDevice<Mouse>();
            Camera camera = CreateCamera();
            CumulusPressable pressable = CreatePressable("cancel-control", out _, out _);
            CreateInteractor(camera);
            int activations = 0;
            pressable.Activated.AddListener(() => activations++);
            Physics.SyncTransforms();
        ...
        public IEnumerator VirtualMouse_DisabledCameraCancelsPressAndAllowsLaterActivation()
        {
            Mouse mouse = InputSystem.AddDevice<Mouse>();
            Camera camera = CreateCamera();
            CumulusPressable pressable = CreatePressable("disabled-camera-control", out Transform visual, out _);
            CreateInteractor(camera);
            int activations = 0;
            pressable.Activated.AddListener(() => activations++);
            Physics.SyncTransforms();
            ...
            yield return null;
            AssertVector(visual.localScale, new Vector3(1.2f, 0.8f, 1f) * 0.9f);
            LogAssert.Expect(LogType.Log, "CumulusPressable activated: disabled-camera-control");
            Release(mouse.leftButton);
            yield return null;
            Assert.That(activations, Is.EqualTo(1));
        }
        ...
        public IEnumerator VirtualMouse_RemovedDeviceCancelsPressAndAllowsLaterActivation()
        {
            Mouse mouse = InputSystem.AddDevice<Mouse>();
            Camera camera = CreateCamera();
            CumulusPressable pressable = CreatePressable("removed-mouse-control", out Transform visual, out _);
            CreateInteractor(camera);
            int activations = 0;
            pressable.Activated.AddListener(() => activations++);
            Physics.SyncTransforms();
            ...
            yield return null;
            AssertVector(visual.localScale, new Vector3(1.2f, 0.8f, 1f) * 0.9f);
            LogAssert.Expect(LogType.Log, "CumulusPressable activated: removed-mouse-control");
        ...
        public IEnumerator VirtualMouse_DisabledInteractorCancelsPressAndAllowsLaterActivation()
        {
            Mouse mouse = InputSystem.AddDevice<Mouse>();
            Camera camera = CreateCamera();
            CumulusPressable pressable = CreatePressable("disabled-interactor-control", out Transform visual, out _);
            CumulusPointerInteractor interactor = CreateInteractor(camera);
            int activations = 0;
            pressable.Activated.AddListener(() => activations++);
            Physics.SyncTransforms();
            yield return null;
            ...
            yield return null;
            AssertVector(visual.localScale, new Vector3(1.2f, 0.8f, 1f) * 0.9f);
            LogAssert.Expect(LogType.Log, "CumulusPressable activated: disabled-interactor-control");
            Release(mouse.leftButton);
            yield return null;
            Assert.That(activations, Is.EqualTo(1));
        }

        private CumulusPressable CreatePressable(string semanticId, out Transform visual, out Collider collider)
        {
            ...
            CumulusPressable pressable = root.AddComponent<CumulusPressable>();
        ...
        private CumulusPointerInteractor CreateInteractor(Camera camera)
        {
            CumulusPointerInteractor interactor = camera.gameObject.AddComponent<CumulusPointerInteractor>();
