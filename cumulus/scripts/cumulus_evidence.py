...
EXPECTED_METRICS = {
    ...
    **{f"downsamplePasses.{phase}": ("equal", 4.0) for phase in FACT_PHASES},
    **{f"upsamplePasses.{phase}": ("equal", 3.0) for phase in FACT_PHASES},
...
def main() -> int:
    ...
    try:
        ...
        print(f"cumulus-evidence: {error}", file=sys.stderr)
        return 1
    return 0
