...
`Web/main.tsx` imports the production `glassSurfaceStyle` and Cumulus token stylesheet, so the browser side measures the current web material directly. The Unity batch capture loads the same committed PNG resources and captures them through the configured URP renderer feature and `CumulusSceneGlass` material.
