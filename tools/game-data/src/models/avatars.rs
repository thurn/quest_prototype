...
struct CompatibilityCatalog {
    #[serde(rename = "avatar")]
    avatars: Vec<CompatibilityAvatar>,
        ...
        for (ids, expected, label) in [
            ...
            for id in ids {
                ...
                if !referenced.insert(*id) {
                    bail!("Avatar {} repeats Tide UUID {id}", avatar.id);
                }
            }
        }
    }
...
fn validate_focus(avatar: &AvatarDefinition) -> Result<()> {
    for (axis, value) in [
        ...
        if !value.is_finite() || !(0.0..=1.0).contains(&value) {
            bail!(
                "Avatar {} portrait focus {axis} must be finite and in [0, 1]",
                avatar.id
            );
        }
    }
...
fn validate_signature_ids(avatar_id: AvatarId, card_ids: &[CardId]) -> Result<()> {
    let mut unique = BTreeSet::new();
    for card_id in card_ids {
        if !unique.insert(*card_id) {
            bail!("Avatar {avatar_id} repeats signature card id {card_id}");
        }
    }
    Ok(())
}
...
pub fn validate_internal_metadata(
    ...
    for entry in metadata {
        if !avatar_ids.contains(&entry.avatar_id) {
            bail!(
                "internal avatar metadata references unknown Avatar {}",
                entry.avatar_id
            );
        }
        let avatar = avatars
...
fn validate_metadata_shape(metadata: &[AvatarMetadata]) -> Result<()> {
    ...
    for entry in metadata {
        if !metadata_ids.insert(entry.avatar_id) {
            bail!(
                "duplicate internal avatar metadata for Avatar {}",
                entry.avatar_id
            );
        }
    ...
    for avatar in avatars {
        for card_id in &avatar.signature_card_ids {
            if !known_card_ids.contains(&card_id.to_string()) {
                bail!(
                    "Avatar {} references unknown signature card {}",
                    avatar.id,
                    card_id
                );
            }
...
mod tests {
    ...
    fn synthetic_source() -> &'static str {
        r##"// Synthetic Avatar definitions.
...
"##
    ...
    fn lowers_ordered_records_ability_paragraphs_signatures_and_optional_tuning() {
        ...
        let avatars = output["avatar"].as_array().unwrap();
    ...
    fn rejects_unknown_fields_and_noncanonical_ids() {
        ...
        for invalid in [
            ...
            assert!(ron::from_str::<AvatarId>(&format!("\"{invalid}\"")).is_err());
        }
        let invalid_card =
    ...
    fn rejects_duplicate_identity_art_invalid_focus_text_signatures_and_metadata() {
        ...
        assert!(
            lower(duplicate_art)
                .unwrap_err()
                .to_string()
                .contains("duplicate Avatar portrait")
        );
