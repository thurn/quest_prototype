...
/**
 * Keeps Cumulus screen adapters (`src/screens/cumulus_adapters/*Adapter.tsx`) wiring-only.
 *
 * A migrated screen splits three ways: the pure Cumulus screen
 * (`src/cumulus/screens/`) renders a view-model; the pure view-model builder
 * (`src/screens/cumulus_adapters/*-view-model.ts`) maps domain data into the screen's
 ...
 *      view-model module, and — within `src/cumulus/` — only `src/cumulus/screens/`.
 *      An adapter mounts a screen; it never composes Cumulus components or
 ...
 *      adapter is layout or chrome, and both belong in the Cumulus screen.
 *
 * SCOPE. Fires only on `src/screens/cumulus_adapters/` files whose basename ends in
 ...
 * inputs, its own layer (the sibling view-model module), and the Cumulus screen
 * it mounts. Fail-closed — anything else (legacy UI in `src/components/` or
 * `src/screens/`, Cumulus components/primitives, feature modules) is an error.
 */
...
const rule = {
  meta: {
    ...
    docs: {
      description:
        "Keep Cumulus screen adapters wiring-only: one exported *Adapter component, no helper logic (it belongs in the *-view-model module), and Cumulus imports limited to src/cumulus/screens/.",
    },
    schema: [],
    messages: {
      cumulusImport:
        "An adapter may import from src/cumulus/ only under src/cumulus/screens/ ('{{source}}' resolves to '{{resolved}}'). Adapters mount a screen; composing Cumulus components is the screen's job, and view-model modules own any component types the view-model references.",
      moduleLogic:
      ...
      extraExport:
        "Adapters export exactly one thing: the *Adapter component. Move shared functions and constants to the screen's *-view-model module, and exported types to the Cumulus screen (view types) or the view-model module (builder types).",
      defaultExport:
      ...
      disallowedImport:
        "Adapters import only their wiring inputs (src/state, src/data, src/types, src/runtime, src/logging), the sibling view-model module, and the Cumulus screen they mount ('{{source}}' resolves to '{{resolved}}'). Legacy UI and feature modules stay out — an adapter wires, it doesn't compose.",
      intrinsicElement:
        "Adapters render exactly the Cumulus screen component (or null while state is unavailable); a `<{{tag}}>` here is layout or chrome, and both belong in the Cumulus screen (src/cumulus/screens/).",
    },
  },
  ...
  create(context) {
    ...
    function checkImportSource(node) {
      ...
      if (!allowed) {
        context.report({
          node,
          messageId: resolved.startsWith("src/cumulus/")
            ? "cumulusImport"
            : "disallowedImport",
