...
function chooseType(label: string): void {
  const trigger = document.querySelector<HTMLButtonElement>(
    'button[aria-label="Figment type"]',
  );
  act(() => trigger?.click());
  const option = [
...
describe("BattleFigmentCreator", () => {
  it("uses Cumulus dialog controls, exposes every catalog type, and dispatches a valid target", () => {
    ...
    expect(
      document.querySelector("[data-battle-figment-creator]"),
    ).not.toBeNull();
    act(() =>
      document
    ...
    expect(document.querySelectorAll('[role="option"]')).toHaveLength(
      FIGMENT_CATALOG_ENTRIES.length,
    );
    act(() =>
      [...document.querySelectorAll<HTMLButtonElement>('[role="option"]')]
        .find((element) => element.textContent?.includes("Shadow"))
        ?.click(),
    );
    act(() =>
      document
        .querySelector<HTMLButtonElement>(
          'button[aria-label="Figment battlefield slot"]',
        )
        ?.click(),
    );
    ...
    act(() =>
      document
        .querySelector<HTMLButtonElement>(
          'button[aria-label="Create more figments"]',
        )
        ?.click(),
    );
    act(() =>
    ...
    act(() =>
      document
        .querySelector<HTMLButtonElement>(
          '[data-testid="battle-figment-submit"]',
        )
        ?.click(),
    );
  ...
  it("keeps type-derived spark and keyword display in the Cumulus form", () => {
    ...
    chooseType("Ember");
    const spark = document.querySelector<HTMLInputElement>(
  ...
  it("labels figment choices by authored name when types share a subtype", () => {
    ...
    ).toEqual(["Warrior (✦1)", "Legionnaire (✦1)"]);
    act(() => root.unmount());
  });
  ...
  it("restores the last selected type when the creator is reopened", () => {
    ...
    document.body.append(host);
    const root = createRoot(host);
    let rememberedTypeId: CardId | undefined;
