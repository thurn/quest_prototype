...
describe("BattleCardNoteEditor", () => {
  it("uses a Cumulus dialog/form and submits the default next-turn expiry", () => {
    ...
    act(() => document.querySelector<HTMLButtonElement>('[data-testid="battle-note-add"]')?.click());
    const submitted = onSubmit.mock.calls[0]?.[0];
    expect(submitted).toMatchObject({ kind: "ADD_CARD_NOTE", battleCardId, noteId: "note-1", text: "remember this", expiry: { kind: "atStartOfTurn" } });
    act(() => root.unmount());
  });
  ...
  it("preserves manual and after-N expiry resolution through Cumulus controls", () => {
    ...
    act(() => document.querySelector<HTMLButtonElement>('[data-testid="battle-note-add"]')?.click());
    expect(onSubmit).toHaveBeenLastCalledWith(expect.objectContaining({ expiry: { kind: "manual" } }));
    act(() => root.unmount());
  });
});
