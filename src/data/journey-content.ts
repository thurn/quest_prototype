 ...
 * idf3 generator's numeric seed from the journey seed and DreamAvatar id so each
 ...
 * from `logs/journey-log.jsonl` filtered by the run's `gameId`. The tide variants
 ...
 * pool and the starter decklist. Deterministic per `(journeySeed, dreamAvatar.id)`.
 */
