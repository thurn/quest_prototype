...
describe("Blackjack rules", () => {
  it("values soft hands and natural blackjacks", () => {
    expect(blackjackHandValue([card("A"), card("K")])).toEqual({
      total: 21,
      isSoft: true,
      isBlackjack: true,
      isBust: false,
    });
    expect(blackjackHandValue([card("A"), card("A"), card("9")])).toMatchObject({
      total: 21,
      isSoft: true,
      isBlackjack: false,
    });
    expect(blackjackHandValue([card("A"), card("A"), card("K")])).toMatchObject({
      total: 12,
      isSoft: false,
      isBust: false,
    });
  ...
  it("resolves player, dealer, and shared natural blackjacks", () => {
    expect(blackjackOpeningOutcome(
      [card("A"), card("K")],
      [card("10"), card("9")],
    )).toBe("player-win");
    expect(blackjackOpeningOutcome(
