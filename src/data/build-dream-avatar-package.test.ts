...
describe("buildDreamAvatarPackage", () => {
  it("never lets starter cards into the draft pool", () => {
    const ctx = makeContext();
    const pkg = buildDreamAvatarPackage(makeDreamAvatar(), ctx, "seed-abc");
  ...
  it("is deterministic for the same inputs", () => {
    const ctx = makeContext();
    const a = buildDreamAvatarPackage(makeDreamAvatar(), ctx, "seed-abc");
    const b = buildDreamAvatarPackage(makeDreamAvatar(), ctx, "seed-abc");
  ...
  it("yields a pool even when the DreamAvatar has no signature cards", () => {
    const ctx = makeContext();
    const withEmpty = buildDreamAvatarPackage(
      makeDreamAvatar({ signatureCards: [] }),
      ctx,
      "seed-abc",
    );
    ...
    const withUndefined = buildDreamAvatarPackage(
      makeDreamAvatar({ signatureCards: undefined }),
      makeContext(),
      "seed-xyz",
    );
    expect(
...
describe("buildDreamAvatarPackage draft_pool_constructed logging", () => {
  beforeEach(() => {
    resetLog();
  });
  ...
  it("records the algorithm, seed, and size for every variant", () => {
    const ctx = makeContext();
    buildDreamAvatarPackage(makeDreamAvatar(), ctx, "seed-abc");
    const event = constructedEvent();
    expect(event.dreamAvatarId).toBe("dc-test");
  ...
  it("records the tide decks the pool was dealt from for tide variants", () => {
    // A synthetic tides2 artifact: three disjoint tides, a DreamAvatar pool with
    ...
    const tides2Relationships = {
      ...
      tidePoolByDreamAvatar: { "dc-test": ["tide-1"] },
    };
...
describe("buildDreamAvatarProvenance", () => {
  it("describes the exact same pool as buildDreamAvatarPackage", () => {
    const ctx = makeContext();
    const pkg = buildDreamAvatarPackage(makeDreamAvatar(), ctx, "seed-abc");
    const prov = buildDreamAvatarProvenance(makeDreamAvatar(), ctx, "seed-abc");
  ...
  it("is deterministic for the same inputs", () => {
    const a = buildDreamAvatarProvenance(makeDreamAvatar(), makeContext(), "seed-abc");
    const b = buildDreamAvatarProvenance(makeDreamAvatar(), makeContext(), "seed-abc");
    expect(a).toEqual(b);
  });
});

// A tides4 run context: four small tides (a signature floor, two facets, a broad
// tail) keyed by name through the artifact, plus a DreamAvatar pool combining
...
describe("buildDreamAvatarTides4Provenance", () => {
  ...
  it("returns null for a non-tides4 pool variant", () => {
    expect(
      buildDreamAvatarTides4Provenance(makeDreamAvatar(), makeContext(), "seed-abc"),
    ).toBeNull();
  });

  it("is deterministic for the same inputs", () => {
    const a = buildDreamAvatarTides4Provenance(
      makeDreamAvatar(),
      makeTides4Context(),
      "seed-t4",
    );
    const b = buildDreamAvatarTides4Provenance(
      makeDreamAvatar(),
      makeTides4Context(),
      "seed-t4",
    );
