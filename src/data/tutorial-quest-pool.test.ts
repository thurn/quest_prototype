...
import { buildTutorialQuestPackage } from "./tutorial-quest-package";
import { validateTutorialQuestPool } from "./tutorial-quest-pool";
...
describe("validateTutorialQuestPool", () => {
  it("accepts three distinct valor tides whose UUID copies fill the pool", () => {
    const pool = validateTutorialQuestPool(syntheticSource(), 4);
  ...
  it("rejects pool-wide duplicate card UUIDs", () => {
    ...
    expect(() => validateTutorialQuestPool(source, 4)).toThrow(
      /duplicates.*00000000-0000-4000-8000-000000000001/u,
    );
  });

  it("rejects a pool that does not match the normal quest size", () => {
    expect(() => validateTutorialQuestPool(syntheticSource(), 150)).toThrow(
      /normal quest pool size \(150\)/u,
    );
  });
});

describe("buildTutorialQuestPackage", () => {
  it("resolves UUIDs to a normal draft multiset and logs its tide provenance", () => {
    ...
    const pkg = buildTutorialQuestPackage(
