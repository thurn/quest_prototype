...
function ordinaryCatalogCandidates(
  ...
  const allowed =
    constraints.allowedCardUuids === undefined
      ? null
      : new Set(constraints.allowedCardUuids);
  const predicate = constraints.predicate ?? "any";
  const useDraftPool =
    constraints.cardScope !== "catalog" && context.draftPoolCardUuids.size > 0;
...
function cardCandidates(
  ...
  if (request.policyId === "fixed") {
    const card =
      constraints.fixedCardUuid === undefined
        ? undefined
...
function withoutCardContext(
  ...
  for (const [tideId, weight] of context.affinityIndex.cardVectors.get(
    cardUuid,
  ) ?? []) {
...
function deckEntryCandidates(
  ...
  if (constraints.fixedDeckEntryId !== undefined) {
    entries = entries.filter(
      ({ entry }) => entry.entryId === constraints.fixedDeckEntryId,
    );
  }
  if (entries.length === 0) {
    return failure(
  ...
  if (
    ...
    return failure(
      request,
      "no_legal_candidates",
      "deck is too small for purge policy",
    );
  ...
  for (const { entry, baseCard, effectiveCard } of context.effectiveDeckCards) {
    if (
      ...
      (constraints.starterOnly !== true &&
        ...
        effectiveCard.isStarter) ||
      (constraints.fixedDeckEntryId !== undefined &&
        entry.entryId !== constraints.fixedDeckEntryId)
    )
      continue;
    const affinity = cardAffinity(
      effectiveCard.id,
    ...
    for (const form of rewardTransfigurations(
      context.content.transfigurationData,
      baseCard,
    )) {
      if (
        (constraints.allowPerfected !== true && form === "Perfected") ||
        (constraints.fixedTransfiguration !== undefined &&
          form !== constraints.fixedTransfiguration) ||
...
function transfiguredCatalogCandidates(
  ...
  const result = built.flatMap((candidate): Candidate[] => {
    ...
    const ranked = rewardTransfigurations(
      context.content.transfigurationData,
      candidate.card,
    )
      .filter(
        (form) =>
          (request.constraints?.allowPerfected === true ||
            form !== "Perfected") &&
      ...
      .sort(
        (left, right) =>
          ...
          compareStableKeys(left.form, right.form),
      );
    const best = ranked[0];
    ...
    return [
      {
        ...
        components: {
          ...
          transfigurationBenefit: best.benefit,
        },
      },
    ];
  });
  ...
  if (request.mechanicId === "add-site") {
    ...
    if (
      ...
      return failure(
        request,
        "invalid_request",
        "add-site contains an unsupported site type",
      );
    }
...
function keyKind(request: RewardSelectionRequest): RewardCandidateKeyKind {
  switch (request.mechanicId) {
    ...
    case "shop-purchase-modifier":
      throw new Error("Mechanic does not select a reward candidate");
  }
}
...
function tuningFor(
  ...
  if (request.policyId === "fixed")
    return { fraction: 1, minimum: 1, values: {} };
...
function bindingsFor(
  ...
  const cards = selected.flatMap((candidate) =>
  ...
  return {
    ...
    packs:
      request.mechanicId === "pack-chooser" && packSize > 0
        ? Array.from({ length: request.count }, (_, index) =>
            cards
              .slice(index * packSize, (index + 1) * packSize)
              .map((card) => card.id),
          )
        : [],
  };
...
export function selectReward(
  ...
  if (
    ...
    return failure(
      request,
      "invalid_request",
      "unknown or incompatible mechanic/policy contract",
    );
  const required =
    request.mechanicId === "pack-chooser"
      ? request.count * (request.packSize ?? 0)
  ...
  const initialBandSize = selectionBandSize(
    candidates.length,
    tuning.fraction,
    tuning.minimum,
  );
  if (initialBandSize < required && request.upTo !== true) {
    return failure(
      ...
      `required ${String(required)} candidates but band contains ${String(initialBandSize)}`,
    );
  }
  const targetCount =
    request.upTo === true ? Math.min(required, initialBandSize) : required;
  ...
  const initialBandKeys = new Set(
    candidates.slice(0, initialBandSize).map((candidate) => candidate.key),
  );
  ...
  while (selected.length < targetCount && candidates.length > 0) {
    if (request.policyId === "card-bundle") {
      candidates = canonicalRank(
        candidates.map((candidate) => {
          if (candidate.card === undefined) return candidate;
          const ranked = ordinaryCardRank(
            context,
            candidate.card,
            temporaryContext,
          );
      ...
      request.policyId !== "card-bundle"
        ...
        : request.mechanicId === "pack-chooser"
          ...
          : selected.length === 0
            ? candidateStream
            : (bundleStream ?? candidateStream);
    const picked = sampleSelectionBand(candidates, size, () =>
      drawStream.draw(),
    );
    ...
    candidates = candidates.filter((candidate) => {
      ...
      return (
        ...
        picked.entryId === undefined ||
        candidate.entryId !== picked.entryId
      );
    });
  ...
  const traceRows = allCandidates.map(
    (candidate): RewardSelectionCandidateTrace => ({
      key: candidate.key,
  ...
  const candidateDigest = stableDigest(
    allCandidates.map(({ key, rank, components }) => ({
      key,
      rank,
      components,
    })),
  ...
  const streams = [candidateStream, packStream, bundleStream]
    ...
    .map((candidate) => ({
      purpose:
        candidate.saltParts[candidate.saltParts.length - 1] ?? "candidate",
  ...
  const trace = {
    ...
    purpose:
      primaryStream.saltParts[primaryStream.saltParts.length - 1] ??
      "candidate",
