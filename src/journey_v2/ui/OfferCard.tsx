import type { MerchantChoice, MerchantGameObject, MerchantOffer } from "../types";
...
/**
 * The game objects an offer shows face-up in its body. Direct-reward offers
 * show their own `gameObjects`; chooser offers (drafts) show every candidate's
 * objects so the player can see — and hover — the whole curated pool before
 * opening the chooser to pick one.
 */
function offerBodyObjects(offer: MerchantOffer): readonly MerchantGameObject[] {
  if (offer.choiceRequest !== undefined) {
    return offer.choiceRequest.candidates.flatMap(
      (candidate) => candidate.gameObjects,
    );
  }
  return offer.gameObjects;
}
  ...
  return (
    ...
    >
      ...
      <section className="min-w-0 overflow-y-auto pr-1">
        <MerchantGameObjectList objects={offerBodyObjects(offer)} compact />
      </section>
