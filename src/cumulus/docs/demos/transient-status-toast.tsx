import {
  ...
  type TransientStatusVariant,
} from "../../components/status/TransientStatusToast";
import type { CumulusComponent } from "../registry";

function TransientStatusToastDemo({ variant = "warning" }: { variant?: TransientStatusVariant }) {
  return <TransientStatusToast variant={variant} copy={{ title: "Action Not Applied", message: "Try again when the game is ready." }} onDismiss={() => undefined} />;
}

export const transientStatusToastDemo: CumulusComponent = {
  id: "transient-status-toast",
  title: "Transient Status Toast",
  blurb: "The fixed, safe-area-aware short-lived status surface for player notices that need a semantic severity and structured copy.",
...
<TransientStatusToast
  ...
  onDismiss={dismiss}
/>` }],
  demo: { defaultArgs: { variant: "warning" } },
};
