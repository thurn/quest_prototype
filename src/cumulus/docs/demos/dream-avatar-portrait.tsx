// Registry demo entry for DreamAvatarPortrait — the highest-adoption workhorse
// in the catalog. The component takes only primitives plus a `dreamAvatar`
// object slot, so the demo's `Component` is a showcase wrapper that renders the
// framings side by side against one real DreamAvatarVisual (curated
// `imageNumber` + name/title; cutout art resolves from `public/`). `dreamAvatar`
// is a ReactNode-free object prop seeded via `sampleContent`; `docName` still
// points at DreamAvatarPortrait so the props table reports its actual API.
...
/** Curated real dreamAvatar: cutout art ships at public/dream-avatars/cutout/0025.png. */
...
/**
 * Renders all six DreamAvatarPortrait framings against one real dreamAvatar:
  ...
  return (
    ...
    >
      ...
      <Cell label="panel · size 120">
        <DreamAvatarPortrait dreamAvatar={dreamAvatar} variant="panel" size={120} />
      </Cell>
      <Cell label="thumb · size 56">
        <DreamAvatarPortrait dreamAvatar={dreamAvatar} variant="thumb" size={56} />
      </Cell>
      <Cell label="standing">
      ...
      <Cell label="cutout">
        <Stage width={150} height={230}>
          <DreamAvatarPortrait dreamAvatar={dreamAvatar} variant="cutout" />
        </Stage>
      </Cell>
