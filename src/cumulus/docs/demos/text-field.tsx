...
function Demo({ label = "Search cards", placeholder = "Type a name…" }: { label?: string; placeholder?: string }) { const [value, setValue] = useState(""); return <TextField label={localizationTodo(label)} kind="search" value={value} placeholder={localizationTodo(placeholder)} onChange={setValue} onCommit={setValue} />; }
