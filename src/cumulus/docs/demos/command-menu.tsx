...
} from "../../components/overlay/CommandMenu";
...
const demoActions: readonly CommandMenuItem[] = [
  ...
  },
  {
    kind: "group",
    id: "saved",
    label: assertLocalized("Saved Journeys"),
    glyph: GLYPHS.chevronRight,
    actions: [
      {
        kind: "action",
        id: "load",
        label: assertLocalized("Load Firstlight"),
...
function CommandMenuDemo() {
  ...
  const actions = demoActions.map((action) =>
    action.kind === "action"
...
export const commandMenuDemo: CumulusComponent = {
  ...
  callout:
    "Use the appChrome model for journey chrome and the context model for an activated target.",
  details: [
