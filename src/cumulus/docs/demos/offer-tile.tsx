...
export const offerTileDemo: CumulusComponent = {
  ...
  callout:
    "Use the named offer kind that matches the category and pass every object surfaced by the generated offer. Fixed-target variants show their exact affected cards; chooser variants show every choice. Descriptions are derived centrally from the model, naming fixed groups of one or two cards, naming a fixed dreamsign or site, spelling quantities as words, and underlining specific card and dreamsign names in the InfoCard. Improvement offers state the exact operation and character-type destination. Every card uses the complete UUID-backed card face and its canonical proportional corner radius. Drafts arrange four complete cards in a two-by-two grid with the smallest spacing step in both directions. Trades reuse that grid behind the purge target, bundles use a large inset fan, and duplicate offers use a compact fan with the standard card-operation mark. Every composition stays inset from the iron frame. Inner objects stay decorative, the complete tile owns interaction plus its body-only InfoCard, and each complete framed tile drifts on its own stable phase.",
  group: "Components",
  docName: "OfferTile",
