...
export const richTextDemo: CumulusComponent = {
  ...
  callout:
    "Build values with the `richText` constructors — `richText.plain`, `richText.rules`, `richText.underline`, `richText.definitions`, `richText.note`, and `richText.stack` — and hand them to a copy slot (like `InfoCard.body`); reach for `RichTextView` only to render a standalone value inline.",
  details: [
...
/>`,
    ...
    {
      label: "Definition copy without a label",
      note: "Use the closed `definitionOnly` presentation for an authored explanation that reads as a complete sentence; the row omits its catalog term and colon while rules glyphs still render inline.",
