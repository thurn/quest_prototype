...
function Demo({ label = "Energy", value = 3 }: { label?: string; value?: number }) {
  ...
  return <NumberStepper label={label} value={current} resource="energy" decrementLabel={`Decrease ${label}`} incrementLabel={`Increase ${label}`} decrementDisabled={current <= 0} onDecrement={() => setCurrent((n) => Math.max(0, n - 1))} onIncrement={() => setCurrent((n) => n + 1)} />;
}
