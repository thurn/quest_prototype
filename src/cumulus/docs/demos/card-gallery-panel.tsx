...
export const cardGalleryPanelDemo: CumulusComponent = {
  ...
  callout:
    "Use this when a screen presents a card collection as the primary task surface, such as the Starting Deck reveal, a card-selection site, or a searchable zone browser.",
  details: [
    'A floating gallery hugs its header, toolbar, rendered card rows, and footer; blank glass inserted to fill a stage is not allowed. heightMode="fill" reserves only a transparent caller-owned fitting wrapper, vertically centers the content-sized glass within it, and never stretches the glass. The component derives material from frame geometry: floating is rounded glass and full-bleed is the edge-to-edge standard alpha scrim. It owns the header, optional mode/search/sort/filter toolbar, accessory slot, internal scroll, fixed grid modes, optional captions and footer actions, and mobile press-preview sizing with whole-card touch-circle clearance. Callers provide resolved card models keyed by entry id or UUID.',
  ],
  group: "Components",
