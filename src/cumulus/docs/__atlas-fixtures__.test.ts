...
describe("Atlas documentation fixtures", () => {
  it("uses shipped art for the unrevealed frame and boss presentation", () => {
    ...
    expect(
      fixtures.map((fixture) => fixture.item.unrevealedFrameRef),
    ).toEqual(
      fixtures.map(() => artRef.atlasAsset("Round_frame_main.png")),
    );
    expect(boss?.iconRef).toEqual(artRef.dreamscapeIcon("limbo"));
    expect(boss?.primary.sceneArt).toEqual(artRef.dreamscapeScene("limbo"));
    expect(boss?.primary.figureArt).toEqual(artRef.dreamGuide("apollyon"));
  });
});
