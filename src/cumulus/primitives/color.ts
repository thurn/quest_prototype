// Typed color values for the strict Cumulus API.
//
// A Cumulus component never takes a raw color string. Appearance is the design
...
/** True when a {@link CumulusColor} is a raw hex literal rather than a role name. */
export function isHexColor(color: CumulusColor): color is HexColor {
...
/**
 * A color a Cumulus component accepts: a named {@link ColorRole} from the palette,
 * or a {@link HexColor} literal for the data-driven case. Never a raw
 * `var(--x)` / CSS-color string.
 */
export type CumulusColor = ColorRole | HexColor;

/** Resolve a {@link CumulusColor} to the CSS color string to render with. */
export function resolveColor(color: CumulusColor): string {
 ...
 */
export function withAlpha(color: CumulusColor, alpha: number): string {
