...
describe("CardBrowserPanel", () => {
  it("owns the canonical mobile overlay recipe", () => {
    ...
    act(() => root.render(<CardBrowserPanel presentation="overlay" title="Starting Deck" subtitle="Your cards" cards={[{ entryId: "entry-a", model: model("Archive Sentry"), testId: "card-a" }]} />));
  ...
  it("owns the canonical desktop collection recipe even in a full-screen host", () => {
    ...
    const container = document.createElement("div"); document.body.append(container);
    const root = createRoot(container);
    act(() => root.render(<CardBrowserPanel presentation="fullScreen" title="Pool" cards={[]} />));
    const panel = container.querySelector<HTMLElement>("[data-gallery-role=browser]");
  ...
  it("prioritizes an enabled card double-tap over its delayed primary press", () => {
    ...
    const container = document.createElement("div"); document.body.append(container);
    const root = createRoot(container);
    act(() => root.render(<CardBrowserPanel title="Your Void" cards={[{ entryId: "physical-card", model: model("Physical"), testId: "physical-card" }]} onCardPress={activate} onCardDoubleTap={doubleTap} />));
  ...
  it("renders browser controls and physical card gestures", () => {
    ...
    act(() => root.render(<CumulusRoot><CardBrowserPanel
      ...
      toolbar={{
        segmented: { options: [{ value: "viewer", label: "Your Cards · 1" }, { value: "opponent", label: "Opponent Cards · 2" }], value: "viewer", onChange: ownerChange },
  ...
  it("supports a sort-only browser toolbar", () => {
    const container = document.createElement("div"); document.body.append(container);
    const root = createRoot(container);
    act(() => root.render(<CumulusRoot><CardBrowserPanel title="Your Void" cards={[]} toolbar={{ sort: { ariaLabel: "Sort cards", value: "current", options: [{ value: "current", label: "Current Order" }], onChange: vi.fn() } }} /></CumulusRoot>));
...
describe("CardPickerPanel", () => {
  it("derives count-aware columns and routes enabled card activation", () => {
    ...
    const container = document.createElement("div"); document.body.append(container);
    const root = createRoot(container);
    act(() => root.render(<CardPickerPanel title="Shop" cards={[
  ...
  it("keeps reserved entries in the count-aware grid", () => {
    const container = document.createElement("div"); document.body.append(container);
    const root = createRoot(container);
    act(() => root.render(<CardPickerPanel title="Shop" cards={[{ entryId: "reserved", model: model("Purchased"), reserved: true }]} />));
  ...
  it("reserves a decorative stacked-copy footprint before showing the copy", () => {
    ...
    const container = document.createElement("div"); document.body.append(container);
    const root = createRoot(container);
    const render = (shown: boolean) => <CumulusRoot><CardPickerPanel title="Duplication" cards={[
  ...
  it("uses the two-column compact recipe for mobile overlay choices", () => {
    const container = document.createElement("div"); document.body.append(container);
    const root = createRoot(container);
    act(() => root.render(<CardPickerPanel presentation="overlay" title="Choose" cards={Array.from({ length: 5 }, (_, index) => ({ entryId: String(index), model: model(String(index)) }))} />));
    const panel = container.querySelector<HTMLElement>("[data-gallery-role=picker]");
  ...
  it("keeps desktop overlays height-bounded while floating glass hugs content width", () => {
    ...
    const container = document.createElement("div"); document.body.append(container);
    const root = createRoot(container);
    act(() => root.render(<CardBrowserPanel presentation="overlay" title="Your Void" cards={[]} />));
    const panel = container.querySelector<HTMLElement>("[data-gallery-role=browser]");
