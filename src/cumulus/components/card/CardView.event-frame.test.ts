...
const CARD_VIEW_CSS = fileURLToPath(new URL("./CardView.css", import.meta.url));
...
describe("CardView event frame", () => {
  it("owns a complete violet treatment with canonical token fallbacks", () => {
    ...
    expect(css).toContain("--cv-event-surface: var(--surface-card, #1a1525);");
    expect(css).toContain("--cv-event-text: var(--text-on-card, #f6f6f5);");
