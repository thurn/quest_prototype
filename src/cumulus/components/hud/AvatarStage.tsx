// AvatarStage — full-body Avatar art that fills a caller-owned
...
export interface AvatarStageProps {
  /** The Avatar whose full-body art fills the stage. */
  avatar: AvatarVisual;
...
/** Full-body Avatar art for a caller-owned stage. */
export function AvatarStage({
  ...
  if (variant === "standing") {
    const glow = (
      <div
        aria-hidden="true"
        data-avatar-stage-backdrop="standing"
        style={{
          position: "absolute",
          inset: 0,
    ...
    if (broken) {
      return (
        <>
          {glow}
          <div
            data-avatar-stage-fallback="standing"
            style={{
              position: "absolute",
              inset: 0,
              display: "grid",
  ...
  if (variant === "cutout") {
    if (broken) {
      return (
        <div
          data-avatar-stage-fallback="cutout"
          style={{
            position: "absolute",
            inset: 0,
            display: "grid",
  ...
  const backdrop = (
    <div
      aria-hidden="true"
      data-avatar-stage-backdrop="fullBleed"
      style={{
        position: "absolute",
        inset: 0,
  ...
  if (broken) {
    return (
      <>
        {backdrop}
        <div
          data-avatar-stage-fallback="fullBleed"
          style={{
            position: "absolute",
            inset: 0,
            display: "grid",
