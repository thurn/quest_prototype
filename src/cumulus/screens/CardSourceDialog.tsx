...
export interface CardSourceDialogProps {
  isOpen: boolean;
  view: CardSourceView | null;
  onClose: () => void;
}
...
export function CardSourceDialog({
  ...
  view,
  onClose,
}: CardSourceDialogProps): ReactElement | null {
  useEffect(() => {
    if (!isOpen) return;
  ...
  return (
    ...
    >
      <GlassDialog
        ...
        onClose={onClose}
      >
        <div style={{ display: "grid", gap: token("--space-l") }}>
          {view.construction === null ? null : (
            <Narrative section={view.construction} initiallyExpanded />
          )}
          <Narrative section={view.cards} initiallyExpanded />
        </div>
      </GlassDialog>
    </div>
  );
      ...
      >
        {section.lines.map((line) =>
          line.card === null ? (
            ...
            <div
              key={line.id}
              data-card-source-card={line.card.cardId}
              style={{
                display: "grid",
