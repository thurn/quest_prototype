    ...
    <div className="cumulus" data-journey-debug-editor="" style={{ minHeight: "100vh" }}>
      <GlassDialog title="Edit Journey State" subtitle="Make explicit diagnostic changes to the active run." onClose={props.onClose} fullScreen>
        <div style={stackStyle}>
          ...
          <DisclosureSection title="Dreamsigns" summary={`${String(props.view.dreamsigns.length)} / ${String(props.view.maxDreamsigns)}`} expanded={expanded.dreamsigns ?? true} onExpandedChange={(value) => setExpanded((current) => ({ ...current, dreamsigns: value }))} testId="journey-debug-dreamsigns">
            <div style={sectionStyle}>
              {props.view.dreamsigns.length === 0 ? <p style={textStyle}>No dreamsigns yet.</p> : props.view.dreamsigns.map((dreamsign) => <div key={dreamsign.actionId} data-journey-debug-dreamsign={dreamsign.actionId} style={sectionStyle}><p style={textStyle}>{dreamsign.name}{dreamsign.isBane ? " · bane" : ""}</p><div style={{ display: "flex", flexWrap: "wrap", gap: token("--space-3") }}><GlassButton label={dreamsign.isBane ? "Unbane" : "Bane"} onPress={() => props.onToggleDreamsignBane(dreamsign.actionId)} placement="onGlass" testId={`journey-debug-bane-${dreamsign.actionId}`} /><GlassButton label="Remove" onPress={() => props.onRemoveDreamsign(dreamsign.actionId)} placement="onGlass" variant="danger" testId={`journey-debug-remove-dreamsign-${dreamsign.actionId}`} /></div></div>)}
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: token("--space-3") }}>{dreamsignMatches.map((option) => <GlassButton key={option.id} label={`Add ${option.name}`} onPress={() => props.onAddDreamsign(option.id)} disabled={!canAddDreamsign} placement="onGlass" testId={`journey-debug-add-dreamsign-${option.id}`} />)}</div>
            </div>
          </DisclosureSection>
          <DisclosureSection title="Deck" summary={`${String(props.view.deck.length)} entries`} expanded={expanded.deck ?? true} onExpandedChange={(value) => setExpanded((current) => ({ ...current, deck: value }))} testId="journey-debug-deck">
            <div style={stackStyle}>
              ...
              {matches.map((card) => <div key={card.cardId} style={{ display: "grid", gridTemplateColumns: "minmax(140px, 180px) minmax(0, 1fr)", alignItems: "center", gap: token("--space-4") }} data-journey-debug-card={card.cardId}><div style={{ width: "100%" }}><GameCard model={card.model} hideRulesText testId={`journey-debug-card-${card.cardId}`} /></div><div style={sectionStyle}><p style={textStyle}>{card.title}<br />{card.cardId}</p><div style={{ display: "flex", flexWrap: "wrap", gap: token("--space-3") }}><GlassButton label="Add" onPress={() => props.onAddCard(card.cardId, false)} placement="onGlass" testId={`journey-debug-add-card-${card.cardId}`} /><GlassButton label="Add as bane" onPress={() => props.onAddCard(card.cardId, true)} placement="onGlass" variant="danger" testId={`journey-debug-add-bane-${card.cardId}`} /></div></div></div>)}
