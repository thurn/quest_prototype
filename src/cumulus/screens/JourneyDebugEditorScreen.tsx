        ...
        <div style={stackStyle}>
          ...
          <DisclosureSection title="Dreamsigns" summary={`${String(props.view.dreamsigns.length)} / ${String(props.view.maxDreamsigns)}`} expanded={expanded.dreamsigns ?? true} onExpandedChange={(value) => setExpanded((current) => ({ ...current, dreamsigns: value }))} testId="journey-debug-dreamsigns">
            <div style={sectionStyle}>
              {props.view.dreamsigns.length === 0 ? <p style={textStyle}>No dreamsigns yet.</p> : props.view.dreamsigns.map((dreamsign) => <div key={dreamsign.actionId} data-journey-debug-dreamsign={dreamsign.actionId} style={sectionStyle}><p style={textStyle}>{dreamsign.name}{dreamsign.isNegative ? " · negative" : ""}</p><div style={{ display: "flex", flexWrap: "wrap", gap: token("--space-3") }}><GlassButton label={dreamsign.isNegative ? "Mark positive" : "Mark negative"} onPress={() => props.onToggleDreamsignNegative(dreamsign.actionId)} placement="onGlass" testId={`journey-debug-negative-${dreamsign.actionId}`} /><GlassButton label="Remove" onPress={() => props.onRemoveDreamsign(dreamsign.actionId)} placement="onGlass" variant="danger" testId={`journey-debug-remove-dreamsign-${dreamsign.actionId}`} /></div></div>)}
