...
const CARD: CardData = {
  ...
  spark: null,
  isFast: false,
  renderedText: "Nightmare is a Bane. Discover. Ephemeral.",
  imageNumber: 1,
  artOwned: false,
};
...
function pointer(
  ...
  target.dispatchEvent(
    new PointerEvent(type, { bubbles: true, pointerType: "mouse", pointerId }),
  );
}
...
describe("cross-family reveal competition", () => {
  it("enforces replacement, Escape suppression, unavailable activation, and consolidated definitions", async () => {
      ...
      root.render(
        <CumulusRoot>
          ...
          <SiteNode
            model={SITE}
            motion={false}
            onSelect={unavailableActivation}
          />
        </CumulusRoot>,
      ),
    );
    ...
    const glossary = container.querySelector<HTMLElement>(
      "[data-glossary-term]",
    )!;
    ...
    act(() => {
      pointer(dreamsign, "pointerover", 1);
    });
    expect(activeSources()).toEqual([dreamsign]);
    act(() => {
      pointer(dreamsign, "pointerout", 1);
    });
    expect(activeSources()).toEqual([glossary]);
    ...
    expect(
      document.querySelectorAll("[data-cumulus-reveal-group]"),
    ).toHaveLength(0);
    act(() => {
      pointer(dreamsign, "pointerover", 2);
      pointer(dreamsign, "pointerout", 2);
    });
    expect(activeSources()).toHaveLength(0);
    expect(
      document.querySelectorAll("[data-cumulus-reveal-group]"),
    ...
    await vi.waitFor(() =>
      expect(
        document.querySelector("[data-cumulus-reveal-group]"),
      ).not.toBeNull(),
    );
    ...
    const shownSecondaries = document.querySelectorAll(
