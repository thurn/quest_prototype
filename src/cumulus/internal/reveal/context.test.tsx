...
describe("Cumulus reveal coordinator root", () => {
  ...
  it("throws a clear error for nested roots", () => {
    expect(() => mount(<CumulusRoot><CumulusRoot><div /></CumulusRoot></CumulusRoot>)).toThrow(/CumulusRoot.*nested/i);
  });

  it("replaces the active source and dismisses it on unmount", () => {
    const { root, container } = mount(<CumulusRoot><Source id={UUID_A} label="A" /><Source id={UUID_B} label="B" /></CumulusRoot>);
    ...
    act(() => root.render(<CumulusRoot><Source id={UUID_A} label="A" /></CumulusRoot>));
    expect(getLogEntries().some((entry) => entry.event === "cumulus_entity_reveal_closed" && entry.dismissalReason === "source-unmount")).toBe(true);
  });
  ...
  it("captures nested non-bubbling scroll and native drag recognition", () => {
    const { container } = mount(<CumulusRoot><div data-scroll-container=""><Source id={UUID_A} /></div></CumulusRoot>);
    const button = container.querySelector("button")!;
  ...
  it("suppresses only a malformed source reveal and reports a diagnostic", () => {
    const { container } = mount(<CumulusRoot><Source id="not-a-uuid" label="Bad" /><Source id={UUID_A} label="Good" /></CumulusRoot>);
    ...
    expect(getLogEntries().some((entry) => entry.event === "cumulus_entity_reveal_invalid_source")).toBe(true);
  ...
  it("allows only the first active touch to activate", () => {
    ...
    const { container } = mount(<CumulusRoot><Source id={UUID_A} onActivate={activateA} /><Source id={UUID_B} onActivate={activateB} /></CumulusRoot>);
    const [a, b] = [...container.querySelectorAll("button")];
  ...
  it("keeps a touch reveal open when an ancestor pointer capture retargets the pointer", () => {
    ...
    act(() => {
      button.dispatchEvent(new PointerEvent("pointerdown", {
        bubbles: true, pointerType: "touch", pointerId: 11,
      }));
    });
    expect(button.dataset.revealActive).toBe("true");
  ...
  it("keeps duplicate UUID mounts registered independently when one unmounts", () => {
    const { root, container } = mount(<CumulusRoot><Source key="first" id={UUID_A} label="First" /><Source key="second" id={UUID_A} label="Second" /></CumulusRoot>);
    ...
    act(() => root.render(<CumulusRoot><Source key="second" id={UUID_A} label="Second" /></CumulusRoot>));
  ...
  it("rejects an incomplete GameCard registration instead of describing only its UUID", () => {
    ...
    const { container } = mount(<CumulusRoot><Source id={UUID_A} spec={incomplete} /></CumulusRoot>);
    const button = container.querySelector("button")!;
  ...
  it("mounts the one shared visual overlay and supplies measured source feedback", () => {
    ...
    expect(document.body.querySelectorAll(":scope > [data-cumulus-reveal-portal]")).toHaveLength(1);
  });

  it("keeps stationary readable sources unscaled", () => {
    const { container } = mount(<CumulusRoot><Source id={UUID_A} feedback="stationary" /></CumulusRoot>);
    const button = container.querySelector("button")!;
  ...
  it("keeps touch pending visual-only until the 30ms intent filter elapses", () => {
    ...
    expect(getLogEntries().filter((entry) => entry.event === "cumulus_entity_reveal_opened")).toHaveLength(0);
  ...
  it("uses the untransformed source rect captured at interaction start", () => {
    ...
    act(() => { button.dispatchEvent(new PointerEvent("pointerover", { bubbles: true, pointerType: "mouse" })); });
    const opened = getLogEntries().find((entry) => entry.event === "cumulus_entity_reveal_opened");
  ...
  it("preserves pen hover modality in the opened diagnostic", () => {
    ...
    expect(getLogEntries().find((entry) => entry.event === "cumulus_entity_reveal_opened")?.modality).toBe("pen");
  });
  ...
  it("pairs focus-hover-focus lifecycles and never closes a pre-measurement dismissal", () => {
    ...
    act(() => { button.dispatchEvent(new FocusEvent("focusout", { bubbles: true })); });
    expect(getLogEntries().filter((entry) => entry.event === "cumulus_entity_reveal_opened")).toHaveLength(3);
    expect(getLogEntries().filter((entry) => entry.event === "cumulus_entity_reveal_closed")).toHaveLength(3);
  ...
  it("recaptures the focused source when another source yields hover precedence", () => {
    const { container } = mount(<CumulusRoot><Source id={UUID_A} label="A" /><Source id={UUID_B} label="B" /></CumulusRoot>);
    const [a, b] = [...container.querySelectorAll("button")];
    ...
    const opens = getLogEntries().filter((entry) => entry.event === "cumulus_entity_reveal_opened");
    const closes = getLogEntries().filter((entry) => entry.event === "cumulus_entity_reveal_closed");
    ...
    const finalCloses = getLogEntries().filter((entry) => entry.event === "cumulus_entity_reveal_closed");
  ...
  it("dismisses on visualViewport resize and removes the listener on unmount", () => {
    ...
    const { container, root } = mount(<CumulusRoot><Source id={UUID_A} /></CumulusRoot>);
    const button = container.querySelector("button")!;
    button.getBoundingClientRect = () => DOMRect.fromRect({ x: 20, y: 220, width: 120, height: 60 });
    act(() => { button.dispatchEvent(new FocusEvent("focusin", { bubbles: true })); });
  ...
  it("cancels a GameCard return immediately when its source unmounts", () => {
    ...
    const { root, container } = mount(<CumulusRoot><Source id={UUID_A} spec={spec} /></CumulusRoot>);
    const button = container.querySelector("button")!;
    button.getBoundingClientRect = () => ({ x: 400, y: 250, left: 400, top: 250, right: 500, bottom: 300, width: 100, height: 50, toJSON: () => ({}) });
    act(() => { button.dispatchEvent(new PointerEvent("pointerover", { bubbles: true, pointerType: "mouse" })); });
    act(() => { button.dispatchEvent(new PointerEvent("pointerout", { bubbles: true, pointerType: "mouse" })); });
