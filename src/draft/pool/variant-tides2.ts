...
//
//   tides  — shuffle one of the avatar's favored tides with tides drawn at
//            random until the pool is full.
//   tides2 — draw a lead tide from the avatar's curated tide pool, then
...
//    together), and each DreamAvatar has a tide pool it draws from. We draw a
//    lead tide from your DreamAvatar's pool, shuffle it together with its allies
...
// each tide's allied tides, and each DreamAvatar's tide pool — are curated data
...
// DreamAvatar's tide pool (for the lead) or the lead's allies list (for the
...
/**
 * Build a pool by combining tide decks: draw a lead tide from the avatar's
 ...
 * keyed by cards_v2 UUID; the catalog index (`poolData.cardNameById`) gates
 ...
 * `poolData.tides2Relationships`. A DreamAvatar without a tide-pool entry (stale
...
export function generateTides2(
  ...
  // Lead selection: shuffle a copy of the avatar's tide pool and take the
  // first id. A missing entry (no DreamAvatar, stale data) falls back to a
  // shuffled draw over every tide so the variant still produces a pool.
  const leadPool =
    dreamAvatarId === undefined
      ? undefined
      : relationships.tidePoolByDreamAvatar[dreamAvatarId];
