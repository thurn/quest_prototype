...
export function buildCardSourceView(debug: CardSourceDebugState | null, tides: Tides4ProvenanceSummary | null, cardDatabase: ReadonlyMap<number, CardData>): CardSourceView | null {
  ...
  if (tides !== null) return { title: debug.screenLabel, subtitle: `${surface(debug.surface)} dealt from your DreamAvatar's tides.`, construction: { id: "construction", title: "How this pool was built", lines: plain(["Signature tide", ...tides.tides.filter((tide) => tide.role === "signature").map((tide) => tide.displayName), "Theme tides", `${String(tides.facetDrawnCount)} of ${String(tides.facetAvailableCount)} theme tides were drawn at random.`, "Deal", `Built to ${String(tides.dealSize)} cards at a ${String(tides.cap)}-copy cap.`]) }, cards: { id: "cards", title: "The cards in front of you", lines: lines((entry) => tideLine(entry.cardNumber, entry.cardName, tides)) } };
...
function tideLine(number: number, name: string, provenance: Tides4ProvenanceSummary): string { const source = provenance.cardProvenanceByNumber[String(number)]; const tide = provenance.tides.find((value) => value.id === source?.primaryTideId); if (tide === undefined) return `${name}: Pool card.`; const label = tide.displayName; return tide.role === "signature" ? `${name}: From your signature tide ${label}.` : tide.role === "facet" ? `${name}: From the theme tide ${label}, one of the ${String(provenance.facetDrawnCount)} drawn at random this run.` : `${name}: From the tide ${label}.`; }
