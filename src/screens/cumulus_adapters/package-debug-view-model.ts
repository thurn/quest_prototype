...
export function buildPackageDebugView(draftState: DraftState | null, cardDatabase: ReadonlyMap<number, CardData>, resolvedPackage: ResolvedDreamAvatarPackage | null, remainingDreamsignPool: readonly string[], dreamsignTemplates: readonly DreamsignTemplate[]): PackageDebugView {
  ...
  return {
    ...
    remainingDreamsigns: (pkg?.remainingDreamsigns ?? []).map((entry) => ({ id: entry.id, label: assertLocalized(entry.name) })),
    spentDreamsigns: (pkg?.spentDreamsigns ?? []).map((entry) => ({ id: entry.id, label: assertLocalized(entry.name) })),
    currentOffer: (draft?.currentOffer ?? []).map((card) => ({ id: card.id, label: assertLocalized(card.name) })),
    topRemainingCards: (draft?.topRemainingCards ?? []).map((card) => ({ id: `card:${String(card.cardNumber)}`, label: assertLocalized(`${card.name} ×${String(card.copiesRemaining)}`) })),
  };
}

function value(id: string, label: string, amount: number) { return { id, label: assertLocalized(label), value: assertLocalized(String(amount)) }; }
