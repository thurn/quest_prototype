...
export function RevealOverlay({ active, onPlaced }: RevealOverlayProps) {
  ...
  useLayoutEffect(() => {
    if (active === null || viewport?.layout !== "desktop" || active.spec.primary.kind !== "gameCard" || decision === null || decision.pressInPlace) return;
    const previousVisibility = active.element.style.visibility;
    active.element.style.visibility = "hidden";
    return () => { active.element.style.visibility = previousVisibility; };
  }, [active, decision, viewport?.layout]);
