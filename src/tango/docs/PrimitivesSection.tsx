// PrimitivesSection — the Primitives section of the /tango overview (Task
// 6.2 of the Tango design-system plan). A generated-from-tokens gallery of
// every design token in tango-tokens.css: color, typography, radius,
...
// IntroSection and above the component catalog in `TangoApp.tsx`'s
// `Overview`.
//
// Boundary: `src/tango/**` may not import `scripts/lib/tango-css-tokens.mjs`
// (a build-time Node module, forbidden by the Tango isolation ESLint rule —
...
// tango-tokens.css that `scripts/generate-tango-tokens.mjs` produces via the
...
// token to tango-tokens.css, rerun `npm run tango-tokens`, and it appears
// here automatically.
//
// Dogfoods Tango tokens for its own chrome (headings, layout, spacing) via
// `token(...)`, matching IntroSection.tsx and TangoApp.tsx — the only raw
 ...
 * mirroring `groupComponents` in TangoApp.tsx. */
...
const MOTION_STYLE_ELEMENT_ID = "tango-primitives-motion-kf";

/** Inserts the shared drift keyframes + reduced-motion guard into
 * document.head, once per document, the first time PrimitivesSection mounts. */
function ensureMotionKeyframes(): void {
  ...
  styleElement.textContent =
    ...
    "@keyframes tangoPrimitivesDrift{0%,100%{transform:translateX(0)}50%{transform:translateX(48px)}}" +
    ".tango-primitives-dot{animation-name:tangoPrimitivesDrift;animation-iteration-count:infinite}" +
    "@media(prefers-reduced-motion:reduce){.tango-primitives-dot{animation-play-state:paused}}";
  ...
  return (
    <div style={specimenTileStyle}>
      <div style={motionTrackStyle}>
        <div className="tango-primitives-dot" style={dotStyle} />
      </div>
      <p style={specimenNameStyle}>{name}</p>
      <p style={specimenValueStyle}>{entry.value}</p>
    </div>
    ...
    <div style={specimenTileStyle}>
      <div style={{ ...motionTrackStyle, display: "flex", alignItems: "center" }}>
        {Array.from({ length: STAGGER_DEMO_COUNT }, (_, index) => (
          <div
            key={index}
            className="tango-primitives-dot"
            style={{
              ...motionDotStyle,
              position: "relative",
              top: 0,
...
// elsewhere in the product (grep'd from src/battle and src/tango), rather
  ...
  return (
    <section id="tango-toc-tokens" aria-labelledby="tango-primitives-heading" style={{ marginBottom: token("--space-10") }}>
      ...
      <p style={leadStyle}>
        ...
        properties declared in tango-tokens.css, via the generated{" "}
