// TableOfContents — the floating left-hand rail on the /tango overview. Lists
...
//
// Chrome dogfoods Tango tokens (via the CSS classes in table-of-contents.css),
...
/**
 * The floating table of contents for the /tango overview. Renders a fixed rail
...
export function TableOfContents({ entries }: { entries: TocEntry[] }): ReactElement | null {
  ...
  return (
    <nav className="tango-toc" aria-label="Table of contents">
      <p className="tango-toc__eyebrow">On this page</p>
      <ul className="tango-toc__list">
        {entries.map((entry) => {
          const classes = ["tango-toc__item"];
          if (entry.depth === 1) {
            classes.push("tango-toc__item--child");
          }
          if (entry.id === activeId) {
            classes.push("tango-toc__item--active");
          } else if (entry.id === parentId) {
            classes.push("tango-toc__item--trail");
          }
          return (
            <li key={entry.id}>
              <button
