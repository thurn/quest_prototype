...
describe("GameCard reveal contract", () => {
  it("registers canonical UUID semantics and derives de-duplicated glossary secondaries", () => {
    ...
    const description = document.querySelector("[data-tango-reveal-descriptions]")?.textContent ?? "";
  ...
  it("uses a reading copy below 340px and leaves a complete wide source in place", async () => {
    ...
    expect(document.querySelector<HTMLElement>('[data-tango-reveal-card="primary"]')?.style.width).toBe("340px");
    ...
    await act(async () => { await new Promise((resolve) => setTimeout(resolve, 0)); });
    remeasure();
    await vi.waitFor(() => expect(document.querySelectorAll('[data-tango-reveal-card="secondary"]')).toHaveLength(1));
    expect(wideSource?.style.opacity).not.toBe("0");
    expect(document.querySelector('[data-tango-reveal-card="primary"]')).toBeNull();
    expect(document.querySelectorAll('[data-tango-reveal-card="secondary"]')).toHaveLength(1);
  ...
  it("keeps hidden-rules cards eligible for a complete popup", async () => {
    ...
    remeasure();
    await vi.waitFor(() => expect(document.querySelector('[data-tango-reveal-card="primary"]')).not.toBeNull());
    expect(document.querySelector('[data-tango-reveal-card="primary"]')?.textContent).toContain("Discard a bane");
    act(() => root.unmount());
  });
