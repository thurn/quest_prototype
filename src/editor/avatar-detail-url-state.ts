// URL synchronisation for the Avatar editor's full-screen detail view.
//
// Opening an Avatar's detail screen records the selected id in a `detail`
...
import { avatarIdFromUnknown } from "../types/identifiers";
...
interface AvatarDetailHistoryState {
  avatarDetailPushed?: boolean;
}

export function parseDetailIdFromUrl(search: string): AvatarId | null {
  const value = new URLSearchParams(search).get(DETAIL_PARAM);
  return value === null || value === "" ? null : avatarIdFromUnknown(value);
}

function buildDetailUrl(detailId: AvatarId | null): string {
...
export function pushDetailIdInUrl(detailId: AvatarId) {
  const state: AvatarDetailHistoryState = { avatarDetailPushed: true };
  window.history.pushState(state, "", buildDetailUrl(detailId));
}

export function replaceDetailIdInUrl(detailId: AvatarId | null) {
...
export function isPushedDetailHistoryEntry(): boolean {
  const state = window.history.state as AvatarDetailHistoryState | null;
  return (
    state !== null && typeof state === "object" && state.avatarDetailPushed === true
  );
}
