...
import { DEFAULT_DREAM_AVATAR_DISPLAY_STATE } from "./dream-avatar-editor-url-state";
import type { DreamAvatarDisplayState } from "./dream-avatar-types";

interface DreamAvatarEditorToolbarProps {
  displayState: DreamAvatarDisplayState;
  visibleCount: number;
  totalCount: number;
  onDisplayStateChange: (state: DreamAvatarDisplayState) => void;
}

const DREAM_AVATAR_SORT_OPTIONS: ReadonlyArray<CardBrowserSortOption> = [
...
export default function DreamAvatarEditorToolbar({
  displayState,
  visibleCount,
  totalCount,
  onDisplayStateChange,
}: DreamAvatarEditorToolbarProps) {
  const handlePatch = (patch: Partial<CardBrowserToolbarValues>) => {
    onDisplayStateChange({ ...displayState, ...patch } as DreamAvatarDisplayState);
  };

  const clearFilters = () => {
    onDisplayStateChange({
      ...displayState,
      searchText: DEFAULT_DREAM_AVATAR_DISPLAY_STATE.searchText,
      searchScope: DEFAULT_DREAM_AVATAR_DISPLAY_STATE.searchScope,
      sort: DEFAULT_DREAM_AVATAR_DISPLAY_STATE.sort,
      dir: DEFAULT_DREAM_AVATAR_DISPLAY_STATE.dir,
    });
  };
  ...
  return (
    <CardBrowserToolbar
      ...
      className="dream-avatar-editor-toolbar"
      values={displayState}
      onPatch={handlePatch}
      subtypeOptions={[]}
      visibleCount={visibleCount}
      totalCount={totalCount}
      sortOptions={DREAM_AVATAR_SORT_OPTIONS}
