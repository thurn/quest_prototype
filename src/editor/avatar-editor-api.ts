...
import type {
  LoadEditorAvatarsResponse,
  SaveEditorAvatarFieldRequest,
  SaveEditorAvatarFieldResponse,
  SaveEditorAvatarTidePoolRequest,
} from "./avatar-types";
...
async function readJsonResponse<T>(response: Response): Promise<T> {
  ...
  if (text.trim() !== "") {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Avatar editor API request failed with ${response.status}`);
      }
      throw error;
    }
  }
    ...
    throw new EditorApiRequestError({
      ...
      message:
        typeof apiError?.message === "string"
          ? apiError.message
          : `Avatar editor API request failed with ${response.status}`,
...
export async function loadEditorAvatars(
  signal?: AbortSignal,
): Promise<LoadEditorAvatarsResponse> {
  const response = await fetch(withTomlParam("/api/editor/avatars"), {
    headers: { Accept: "application/json" },
    signal,
  });
  const body = await readRevisionedJsonResponse<LoadEditorAvatarsResponse>(response);
  confirmSourceRevision(SOURCE, body);
  return body;
}

export async function saveEditorAvatarField(
  request: SaveEditorAvatarFieldRequest,
): Promise<SaveEditorAvatarFieldResponse> {
  return queueSourceSave(SOURCE, async () => {
    const response = await fetch(withTomlParam(`/api/editor/avatars/${request.id}`), {
      method: "PATCH",
      headers: { Accept: "application/json", "Content-Type": "application/json" },
      body: JSON.stringify(withExpectedSourceRevision(SOURCE, request)),
    });
    const body = await readRevisionedJsonResponse<SaveEditorAvatarFieldResponse>(response);
    confirmSourceRevision(SOURCE, body);
    return body;
  });
}

export async function saveEditorAvatarTidePool(
