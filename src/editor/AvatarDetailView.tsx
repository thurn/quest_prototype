...
export default function AvatarDetailView({
  avatar,
  tides,
  journeyContent,
  journeyContentError,
  ...
  const starterTides = useMemo(
    () =>
      avatar.tidePool.starter === null
        ? []
        : resolveTides([avatar.tidePool.starter], tideById),
    [avatar.tidePool.starter, tideById],
  );
  const facetTides = useMemo(
    () => resolveTides(avatar.tidePool.facets, tideById),
    [avatar.tidePool.facets, tideById],
  );
  const neutralTides = useMemo(
    () => resolveTides(avatar.tidePool.neutral, tideById),
    [avatar.tidePool.neutral, tideById],
  );
  ...
  // Resolve the Avatar's signature cards to renderable CardData. Resolution
  ...
  const signature = useMemo<SignatureResolution>(() => {
    ...
    const match = journeyContent.avatars.find(
      (entry) => entry.id === avatar.id,
    );
    ...
    return { cards, unresolved };
  }, [journeyContent, avatar.id, cardsByUuid]);
  ...
  // Drop the open tide selection whenever the screen switches Avatars so a
  // stale id (one not in the new Avatar's pool) never lingers.
  useEffect(() => {
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            <span style={{ color: "#fbbf24", fontWeight: 800 }}>
              {avatar.startingEssence}
            </span>
          </div>
        </div>
        <button
          type="button"
          data-avatar-detail-close=""
          onClick={onClose}
          style={{
            flexShrink: 0,
            border: "1px solid rgba(247, 241, 223, 0.28)",
        ...
        <section style={{ marginBottom: 28 }}>
          ...
          {journeyContentError !== null ? (
            ...
            <>
              ...
              <div style={{ containerType: "inline-size" }}>
                <div
                  data-avatar-signature-grid={avatar.id}
                  style={{
                    display: "grid",
                    gridTemplateColumns: SIZE_PRESETS.large.columns,
                    gap: SIZE_PRESETS.large.gap,
