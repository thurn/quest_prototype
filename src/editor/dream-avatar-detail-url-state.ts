// URL synchronisation for the DreamAvatar editor's full-screen detail view.
//
// Opening a DreamAvatar's detail screen records the selected id in a `detail`
...
interface DreamAvatarDetailHistoryState {
...
export function isPushedDetailHistoryEntry(): boolean {
  const state = window.history.state as DreamAvatarDetailHistoryState | null;
  return (
    state !== null && typeof state === "object" && state.dreamAvatarDetailPushed === true
  );
}
