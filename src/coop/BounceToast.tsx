...
export const PARTNER_CONFLICT_MESSAGE: LocalizedString = tx(
  ...
  "[coop] Transient status after another player wins a compare-and-swap race for shared game state.",
);
...
export const INVALID_ACTION_MESSAGE: LocalizedString = tx(
  ...
  "[coop] Transient status after the shared-game rules reject the player's action in the current state.",
);
...
export const APPEND_FAILED_MESSAGE: LocalizedString = tx(
  ...
  "[coop] Transient status after a shared-game action could not be appended to the room log.",
);
...
export const PENDING_DROPPED_MESSAGE: LocalizedString = tx(
  ...
  "[coop] Transient status after reconnection discards shared-game actions that were never confirmed.",
);
  ...
  switch (reason) {
    ...
    case "prompt_pending":
      return tx(
        ...
        "[coop] Transient status when another shared-game choice must be resolved before this action.",
      );
    case "unknown_conflict":
      return tx(
        "Action not applied: the game changed before it was received. Try again.",
        "[coop] Transient status after an unclassified shared-game state conflict rejects the player's action.",
      );
    case "observer_read_only":
      return tx(
        "Action not applied: this playtest is controlled from another browser.",
        "[coop] Transient status when a read-only playtest observer attempts a player action.",
      );
    case "fold_error":
    case "malformed_event":
      return tx(
        "Action not applied because of an internal error. Please try again.",
        "[coop] Transient status after an internal fold or malformed shared-game event prevents the player's action.",
      );
    case "invalid_action":
    default:
