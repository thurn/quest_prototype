// Journey-log sink for the coop event-sourcing client.
//
 ...
 * RTDB `rooms/{id}/logs` buffer AND the journey-log.jsonl mirror. Single-writer
 ...
 */
export interface JourneyLogSinkHandle extends CoopLogRecorder {
...
/**
 * Install the journey-log sink for a ready room:
 ...
 *    node (true `seq`) and journey-log.jsonl (console + dev server).
 */
export function installJourneyLogSink(
  database: Database,
  options: JourneyLogSinkOptions,
): JourneyLogSinkHandle {
  ...
  const recorder = createCoopLogRecorder({
    ...
    // journey-log.jsonl (console + dev server), verbatim so its true `seq`
    // survives. Single-writer gating lives in the recorder.
    emit: createCoopEmit(buffered, createJourneyLogMirror()),
  });
