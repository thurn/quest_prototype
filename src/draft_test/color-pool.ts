...
// Knobs for the `merged` variant. It draws from the merged archetype lists
// (`PoolData.mergedLists`, built offline in `scripts/setup-assets.mjs`): one
// list per drafted archetype, holding the cards that recur across that
// archetype's real decks. Run time is a trivial weighted pick — roll a primary
// archetype (theme-weighted), keep the on-color lists, and shuffle a few into
// the pool — with no IDF, cosine, spine, or softmax. The defaults below
// reproduce the `decklists` variant's output (see
// `docs/cards2/draft_pool_algorithms.md`). Falls back to `default` when no
// merged lists are bundled. Grouped here so tuning is a one-stop edit.
...
// Knobs for the `idf2` variant. It is `idf` in every respect — same corpus, same
// IDF weighting, same best-first whole-deck union — EXCEPT that the starter
// decklist is not drawn uniformly at random. The real corpus is lopsided: a
// popular archetype is recorded as dozens of near-duplicate decks while a fringe
// one has only a handful, so a uniform starter draw lands in the big clusters far
// more often and the player keeps getting the same kind of pool ("spirit animals
...
// mass and over-represented archetypes are drawn far less. `scripts/idf-starter-
// diversity-experiment.mjs` validates this: at the production setting it lifts the
// effective number of distinct archetypes drawn from ~221 to ~552 and cuts the
// top archetype's share from ~4.1% to ~1.2%, while pool coherence barely moves.
