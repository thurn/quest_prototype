...
/**
 * End-to-end journey-flow integration coverage.
 *
 * This is the battle-mode-style backbone for the journey run: it drives the REAL
 * journey state actions (`startJourneyFromDreamAvatar`, `completeJourneySite`,
...
/** Maximum journey deck size the design allows during battles (doc rule). */
const MAX_JOURNEY_DECK_SIZE = 50;
...
/**
 * A {@link JourneyContent} built entirely from the live compiled bundles plus the
 * shared pool-context helper, so the journey start path runs against the real
...
function buildContext(content: JourneyContent): AtlasBuildContext {
  return {
...
/** The node the journey state currently sits in. */
function currentNode(state: JourneyState): DreamscapeNode {
 ...
 * the real `completeJourneySite` action. Asserts the Battle stays gated by
 * `canVisitSite` until all non-Battle sites are done (the Battle-last rule), and
 * returns the resulting state.
 */
function completeNonBattleSites(state: JourneyState): JourneyState {
  let next = state;
  ...
  for (let index = 0; index < nonBattle.length; index += 1) {
    ...
    next = completeJourneySite(next, site.id);
  }
...
function winBattleAndAdvance(
...
): { next: JourneyState; wasFinalBoss: boolean } {
  ...
  expect(init.playerDeckOrder.length).toBeGreaterThanOrEqual(
    Math.min(MIN_BATTLE_DECK_SIZE, MAX_JOURNEY_DECK_SIZE),
  );
  // The journey deck itself stays within the 50-card battle ceiling.
  expect(state.deck.length).toBeLessThanOrEqual(MAX_JOURNEY_DECK_SIZE);

  // Victory: mark Battle visited and advance the atlas (mirrors the bridge).
  const afterBattleSite = completeJourneySite(state, battle.id);
...
describe("journey flow (end to end)", () => {
  it("plays a full 7-layer run start -> sites -> battle -> advance -> boss victory", () => {
    ...
    let state = startJourneyFromDreamAvatar({
    ...
    // Journey start lands the player in the starting dreamscape on the dreamscape
  ...
  it("pads a small starter deck up to the battle minimum at the first battle", () => {
    const content = makeJourneyContent();
    const state = startJourneyFromDreamAvatar({
      prev: createDefaultState(),
    ...
    // is padded up to it while the journey deck itself is left unchanged.
  ...
  it("ends the run on a defeat without advancing the atlas", () => {
    ...
    expect(defeated.screen).toEqual({ type: "journeyFailed" });
