...
import { randomRewardsPlugin } from "./index";

describe("random_rewards shape", () => {
  it("produces a structurally valid filled journey on a populated fixture", () => {
    const ctx = buildFixtureContext();
    const filled = randomRewardsPlugin.fill({
      context: ctx,
      drawContext: FIXTURE_DRAW_CONTEXT,
      stage: "mid",
    });

    const bounds = randomRewardsPlugin.definition.rootOptionCount;
  ...
  it("fills no-cost reward options with visible shared reward envelopes", () => {
    ...
    for (const option of filled.options) {
      ...
      if (!rewardEntry) {
        throw new Error("expected random_rewards reward envelope to resolve");
      }
