...
import { chooseYourLossPlugin } from "./index";

describe("choose_your_loss shape", () => {
  it("produces a structurally valid filled journey on a populated fixture", () => {
    const ctx = buildFixtureContext();
    const filled = chooseYourLossPlugin.fill({
      context: ctx,
      drawContext: FIXTURE_DRAW_CONTEXT,
      stage: "mid",
    });

    const bounds = chooseYourLossPlugin.definition.rootOptionCount;
