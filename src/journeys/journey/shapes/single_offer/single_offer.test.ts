...
import { singleOfferPlugin } from "./index";

describe("single_offer shape", () => {
  it("produces a structurally valid filled journey on a populated fixture", () => {
    const ctx = buildFixtureContext();
    const filled = singleOfferPlugin.fill({
      context: ctx,
      drawContext: FIXTURE_DRAW_CONTEXT,
      stage: "mid",
    });

    const bounds = singleOfferPlugin.definition.rootOptionCount;
