...
import { rewardAfterTriggerPlugin } from "./index";

describe("reward_after_trigger shape", () => {
  it("produces a structurally valid filled journey on a populated fixture", () => {
    const ctx = buildFixtureContext();
    const filled = rewardAfterTriggerPlugin.fill({
      context: ctx,
      drawContext: FIXTURE_DRAW_CONTEXT,
      stage: "mid",
    });

    const bounds = rewardAfterTriggerPlugin.definition.rootOptionCount;
