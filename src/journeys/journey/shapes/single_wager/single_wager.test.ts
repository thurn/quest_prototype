...
import { singleWagerPlugin } from "./index";

describe("single_wager shape", () => {
  it("produces a structurally valid filled journey on a populated fixture", () => {
    const ctx = buildFixtureContext();
    const filled = singleWagerPlugin.fill({
      context: ctx,
      drawContext: FIXTURE_DRAW_CONTEXT,
      stage: "mid",
    });

    const bounds = singleWagerPlugin.definition.rootOptionCount;
