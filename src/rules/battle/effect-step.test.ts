...
function makeInstance(battleCardId: string, sparkDelta: number): BattleCardInstance {
...
describe("erodeEdits", () => {
  it("returns a single ERODE edit with the given side and count", () => {
    expect(erodeEdits("enemy", 3)).toEqual([{ kind: "ERODE", side: "enemy", count: 3 }]);
  });

  it("carries the side through for the player", () => {
    expect(erodeEdits("player", 1)).toEqual([{ kind: "ERODE", side: "player", count: 1 }]);
  });
});

describe("addGainedSparkEdits", () => {
  it("accumulates onto the instance's current sparkDelta rather than overwriting", () => {
    const state = makeState({ instances: [makeInstance("c1", 2)] });
    expect(addGainedSparkEdits("c1", 3, state)).toEqual([
      { kind: "SET_CARD_SPARK_DELTA", battleCardId: "c1", value: 5 },
    ]);
  });

  it("starts from a zero sparkDelta correctly", () => {
    const state = makeState({ instances: [makeInstance("c1", 0)] });
    expect(addGainedSparkEdits("c1", 4, state)).toEqual([
      { kind: "SET_CARD_SPARK_DELTA", battleCardId: "c1", value: 4 },
    ]);
  });

  it("returns [] when the instance is absent", () => {
    const state = makeState({ instances: [] });
    expect(addGainedSparkEdits("missing", 3, state)).toEqual([]);
  });
});
...
describe("discardHandEdits", () => {
  it("emits exactly one DISCARD_CARD per hand card in hand order", () => {
    ...
    expect(discardHandEdits("player", state)).toEqual([
      { kind: "DISCARD_CARD", battleCardId: "h1" },
      { kind: "DISCARD_CARD", battleCardId: "h2" },
      { kind: "DISCARD_CARD", battleCardId: "h3" },
    ]);
  });
  ...
  it("reads from the requested side's hand only", () => {
    ...
    expect(discardHandEdits("enemy", state)).toEqual([
      { kind: "DISCARD_CARD", battleCardId: "e1" },
      { kind: "DISCARD_CARD", battleCardId: "e2" },
    ]);
  });
