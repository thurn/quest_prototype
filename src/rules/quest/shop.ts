...
//
// This module owns the quest events that spend at a shop, resolve a Dream
...
// relocates the DOMAIN MATH of a legacy quest mutation
...
// events resolve an encounter generated from async-loaded quest content. The
...
export function buyShopSlot(
  ...
  const price = discountedSlotPrice(
    slot,
    quest.shopModifiers.essenceDiscountPercent,
  );
  if (price > quest.essence) return null;
  ...
  let next: QuestState = {
    ...quest,
    essence: clampEssence(quest.essence - price, quest.essenceCap),
  };
...
export function rerollShop(
  ...
  const useFreeReroll = quest.shopModifiers.freeRerolls > 0;
  const cost = useFreeReroll ? 0 : finiteNumber(payload.essenceCost) ?? 0;
  if (!useFreeReroll && cost > quest.essence) return null;

  const provider = getSiteContentProvider();
  const generated = provider?.rerollShop?.({ quest, site, rng: ctx.rng });
  if (generated === undefined || generated === null) return null;

  const shopModifiers = useFreeReroll
    ? {
        ...quest.shopModifiers,
        freeRerolls: quest.shopModifiers.freeRerolls - 1,
      }
    : quest.shopModifiers;
  const essence = useFreeReroll
    ? quest.essence
    : clampEssence(quest.essence - cost, quest.essenceCap);
  ...
  return {
    ...
    siteRuntime: { ...quest.siteRuntime, [siteId]: nextRuntime },
  };
}
...
export function grantFreeRerolls(
  ...
  return {
    ...
    shopModifiers: {
      ...quest.shopModifiers,
      freeRerolls: quest.shopModifiers.freeRerolls + count,
    },
  };
}
  ...
  return {
    ...
    shopModifiers: {
      ...
      essenceDiscountPercent:
        quest.shopModifiers.essenceDiscountPercent + percent,
    },
  };
}
...
export function acceptMerchantOffer(
  ...
  return resolveMerchant(quest, payload, ctx, "accept");
}
...
export function declineMerchant(
  ...
  return resolveMerchant(quest, payload, ctx, "decline");
}
...
export function pushBattleModifier(
  ...
  return { ...quest, battleModifiers: [...quest.battleModifiers, modifier] };
}
...
export function pushTemporaryBaneGrant(
  ...
  return {
    ...quest,
    deck,
    battleModifiers: [...quest.battleModifiers, modifier],
  };
}
...
export function banSiteType(
  ...
  return {
    ...quest,
    dreamscapeModifiers: [...quest.dreamscapeModifiers, modifier],
  };
}
...
export function replaceSiteType(
  ...
  const node = quest.atlas.nodes[nodeId];
  ...
  return {
    ...
    atlas: {
      ...quest.atlas,
      nodes: { ...quest.atlas.nodes, [nodeId]: { ...node, sites } },
    },
  };
}
...
export function updateAtlas(
  ...
  return { ...quest, atlas: atlas as DreamAtlas };
}
...
export function setCardSourceDebug(
  ...
  if (state === null) return { ...quest, cardSourceDebug: null };
  if (typeof state !== "object" || Array.isArray(state)) return null;
  return {
    ...quest,
    cardSourceDebug: state as QuestState["cardSourceDebug"],
  };
}
